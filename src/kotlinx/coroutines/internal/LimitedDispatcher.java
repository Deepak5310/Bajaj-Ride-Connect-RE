...
public final class LimitedDispatcher extends CoroutineDispatcher implements Delay {
    ...
    public void mo3947scheduleResumeAfterDelay(long timeMillis, CancellableContinuation<? super Unit> continuation) {
        this.$$delegate_0.mo3947scheduleResumeAfterDelay(timeMillis, continuation);
    }
        ...
        public void run() {
            ...
            while (true) {
                ...
                if (i >= 16 && LimitedDispatcher.this.dispatcher.isDispatchNeeded(LimitedDispatcher.this)) {
                    LimitedDispatcher.this.dispatcher.mo3946dispatch(LimitedDispatcher.this, this);
                    return;
                }
            }
        }
    ...
    public void mo3946dispatch(CoroutineContext context, Runnable block) {
        Runnable runnableObtainTaskOrDeallocateWorker;
        this.queue.addLast(block);
        if (runningWorkers$volatile$FU.get(this) >= this.parallelism || !tryAllocateWorker() || (runnableObtainTaskOrDeallocateWorker = obtainTaskOrDeallocateWorker()) == null) {
            return;
        }
        this.dispatcher.mo3946dispatch(this, new Worker(runnableObtainTaskOrDeallocateWorker));
    }
