...
public final class BroadcastChannelImpl<E> extends BufferedChannel<E> implements BroadcastChannel<E> {
        ...
        try {
            ...
            if (!isClosedForSend() || this.lastConflatedElement != BroadcastChannelKt.NO_ELEMENT) {
                if (this.lastConflatedElement != BroadcastChannelKt.NO_ELEMENT) {
                    subscriberConflated.mo3880trySendJP2dKIU(getValue());
                }
    ...
    public Object mo3880trySendJP2dKIU(E element) {
        ...
        try {
            ...
            if (!(list instanceof Collection) || !list.isEmpty()) {
                ...
                while (it2.hasNext()) {
                    if (((BufferedChannel) it2.next()).shouldSendSuspend$kotlinx_coroutines_core()) {
                        Object objM3909failurePtdJZtk = ChannelResult.INSTANCE.m3909failurePtdJZtk();
                        reentrantLock.unlock();
                        return objM3909failurePtdJZtk;
                    }
                }
            }
            if (this.capacity == -1) {
            ...
            while (it3.hasNext()) {
                ((BufferedChannel) it3.next()).mo3880trySendJP2dKIU(element);
            }
            Object objM3910successJP2dKIU = ChannelResult.INSTANCE.m3910successJP2dKIU(Unit.INSTANCE);
            reentrantLock.unlock();
            return objM3910successJP2dKIU;
        } catch (Throwable th) {
            reentrantLock.unlock();
            throw th;
        }
    ...
    public boolean close(Throwable cause) {
        ...
        try {
            ...
            while (it2.hasNext()) {
                ((BufferedChannel) it2.next()).close(cause);
            }
            List<? extends BufferedChannel<E>> list = this.subscribers;
            ArrayList arrayList = new ArrayList();
            Iterator<T> it3 = list.iterator();
            while (it3.hasNext()) {
                BufferedChannel bufferedChannel = (Object) it3.next();
    ...
    public boolean cancelImpl$kotlinx_coroutines_core(Throwable cause) {
        ...
        try {
            Iterator<T> it2 = this.subscribers.iterator();
            while (it2.hasNext()) {
                ((BufferedChannel) it2.next()).cancelImpl$kotlinx_coroutines_core(cause);
            }
            this.lastConflatedElement = BroadcastChannelKt.NO_ELEMENT;
