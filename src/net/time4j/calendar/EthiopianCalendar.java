...
public final class EthiopianCalendar extends Calendrical<Unit, EthiopianCalendar> implements LocalizedPatternSupport {
    ...
    private static class Transformer implements EraYearMonthDaySystem<EthiopianCalendar> {
        ...
        public EthiopianCalendar transform(long j) {
            try {
                ...
                int i = 1;
                int iSafeCast2 = MathUtils.safeCast(MathUtils.floorDivide(j - ((long) MathUtils.safeCast(transform(new EthiopianCalendar(iSafeCast, i, i)))), 30)) + 1;
    ...
    private static class Merger implements ChronoMerger<EthiopianCalendar> {
        ...
        public EthiopianCalendar createFrom2(ChronoEntity<?> chronoEntity, AttributeQuery attributeQuery, boolean z, boolean z2) {
            ...
            if (i == Integer.MIN_VALUE) {
                chronoEntity.with(ValidationElement.ERROR_MESSAGE, "Missing Ethiopian year.");
                return null;
            }
            if (!chronoEntity.contains(EthiopianCalendar.ERA)) {
                chronoEntity.with(ValidationElement.ERROR_MESSAGE, "Missing Ethiopian era.");
            }
            ...
            if (chronoEntity.contains(EthiopianCalendar.MONTH_OF_YEAR)) {
                ...
                if (i2 != Integer.MIN_VALUE) {
                    ...
                    chronoEntity.with(ValidationElement.ERROR_MESSAGE, "Invalid Ethiopian date.");
                }
            } else {
        ...
        public long between(EthiopianCalendar ethiopianCalendar, EthiopianCalendar ethiopianCalendar2) {
            ...
            if (i == 1) {
                ...
                if (i == 2) {
                    ...
                    if (jYmValue <= 0 || ethiopianCalendar2.edom >= ethiopianCalendar.edom) {
                        return (jYmValue >= 0 || ethiopianCalendar2.edom <= ethiopianCalendar.edom) ? jYmValue : jYmValue + 1;
                    }
                    return jYmValue - 1;
                }
                if (i != 3) {
    ...
    /* JADX INFO: renamed from: net.time4j.calendar.EthiopianCalendar$2, reason: invalid class name */
