...
public final class ChineseCalendar extends EastAsianCalendar<Unit, ChineseCalendar> implements LocalizedPatternSupport {
    ...
    private static class YearOfEraRule implements IntElementRule<ChineseCalendar> {
        ...
        public int getInt(ChineseCalendar chineseCalendar) {
            ...
            if (relatedGregorianYear < 1662) {
                return relatedGregorianYear - 1643;
            }
            if (relatedGregorianYear < 1723) {
                return relatedGregorianYear - 1661;
            }
            if (relatedGregorianYear < 1736) {
                return relatedGregorianYear - 1722;
            }
            if (relatedGregorianYear < 1796) {
                return relatedGregorianYear - 1735;
            }
            if (relatedGregorianYear < 1821) {
                return relatedGregorianYear - 1795;
            }
            if (relatedGregorianYear < 1851) {
                return relatedGregorianYear - 1820;
            }
            if (relatedGregorianYear < 1862) {
                return relatedGregorianYear - 1850;
            }
            if (relatedGregorianYear < 1875) {
                return relatedGregorianYear - 1861;
            }
            if (relatedGregorianYear < 1909) {
                return relatedGregorianYear - 1874;
            }
            return chineseCalendar.getDaysSinceEpochUTC() < -21873 ? relatedGregorianYear - 1908 : relatedGregorianYear + 2698;
        }
        ...
        public ChineseCalendar createFrom(ChronoEntity<?> chronoEntity, AttributeQuery attributeQuery, boolean z, boolean z2) {
                    ...
                    if (i3 == Integer.MIN_VALUE) {
                        if (chronoEntity.contains(ChineseCalendar.ERA)) {
                            ...
                            if (chineseEra.isQingDynasty()) {
                                eastAsianYearForGregorian = cyclicYear.inQingDynasty(chineseEra);
                            } else {
                        ...
                        eastAsianYearForGregorian = cyclicYear.inCycle(i3);
                    }
                } else if (!chronoEntity.contains(ChineseCalendar.ERA) || (i = chronoEntity.getInt(ChineseCalendar.YEAR_OF_ERA)) == Integer.MIN_VALUE) {
                    eastAsianYearForGregorian = null;
                } else {
                    eastAsianYearForGregorian = EastAsianYear.forGregorian((((ChineseEra) chronoEntity.get(ChineseCalendar.ERA)).getStartAsGregorianYear() + i) - 1);
                }
            } else {
            ...
            if (chronoEntity.contains(ChineseCalendar.MONTH_OF_YEAR)) {
                ...
                if (i4 != Integer.MIN_VALUE) {
                    return ChineseCalendar.of(eastAsianYearForGregorian, eastAsianMonth, i4);
                }
            } else {
                int i5 = chronoEntity.getInt(ChineseCalendar.DAY_OF_YEAR);
                if (i5 != Integer.MIN_VALUE && i5 >= 1) {
                    return (ChineseCalendar) ChineseCalendar.of(eastAsianYearForGregorian, EastAsianMonth.valueOf(1), 1).plus(i5 - 1, Unit.DAYS);
                }
            }
            return null;
        }
