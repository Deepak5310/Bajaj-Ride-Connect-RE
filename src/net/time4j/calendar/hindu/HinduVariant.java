        ...
        while (stringTokenizer.hasMoreTokens()) {
            ...
            switch (i) {
                ...
                case 6:
                    ...
                    z = z && dDoubleValue == UJJAIN.getLongitude();
                    break;
                case 7:
    ...
    static class ModernHinduCS extends HinduCS {
        ...
        public long getMaximumSinceUTC() {
            if (this.max == Long.MAX_VALUE) {
                HinduCalendar hinduCalendarCreateNewYear = createNewYear(NavigationConstants.UI_HANDLER_PROGRESS);
                if (this.variant.isPurnimanta()) {
        ...
        static double hPrecession(double d) {
            ...
            return modulo((modulo(((((((((double) (-1)) * 6.0E-6d) / 3600.0d) * centuryJ2000) + 3.0864722222222223E-4d) * centuryJ2000) + 1.3969712777777776d) * centuryJ2000, 360.0d) + dModulo2) - Math.toDegrees(Math.atan2(Math.cos(Math.toRadians(dModulo)) * Math.sin(Math.toRadians(dModulo2)), Math.cos(Math.toRadians(dModulo2)))), 360.0d);
        }
        ...
        private static HinduCalendar hSolarFromFixed(long j, HinduVariant hinduVariant) {
            ...
            if (hinduVariant.useModernAstronomy()) {
                ...
                jModulo = (jTransform - 3) - ((long) ((int) modulo(Math.floor(hSiderealSolarLongitude(dApply)), 30.0d)));
                while (hSiderealZodiac(longFunctionHCritical.apply(jModulo)) != iHZodiac) {
                    jModulo++;
                }
            } else {
                iHZodiac = hZodiac(dApply);
                jModulo = (jTransform - 3) - ((long) ((int) modulo(Math.floor(hSolarLongitude(dApply)), 30.0d)));
        ...
        private static long hFixedFromLunar(int i, HinduMonth hinduMonth, HinduDay hinduDay, HinduVariant hinduVariant) {
            ...
            long jFloor = (long) Math.floor(d - ((modulo(((dHSolarLongitude / 360.0d) - (((double) (value - 1)) / 12.0d)) + 0.5d, 1.0d) - 0.5d) * SIDEREAL_YEAR));
            ...
            long jModulo = (14 + j) - ((long) ((int) modulo((hLunarDayFromMoment(j + 0.25d, hinduVariant) - value2) + 15, 30.0d)));
            while (true) {
