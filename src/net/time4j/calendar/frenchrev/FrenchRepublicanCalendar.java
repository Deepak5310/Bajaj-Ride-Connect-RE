...
public final class FrenchRepublicanCalendar extends Calendrical<Unit, FrenchRepublicanCalendar> {
    ...
    @FormattableElement(alt = "c", dynamic = true, format = WeatherCriteria.UNIT_CELSIUS)
    ...
    private static class DayOfDecadeAccess extends BasicElement<DayOfDecade> implements TextElement<DayOfDecade>, ElementRule<FrenchRepublicanCalendar, DayOfDecade> {
        ...
        private TextAccessor accessor(Locale locale, AttributeQuery attributeQuery) {
            ...
            if (textWidth == TextWidth.NARROW) {
                ...
                str = outputContext == OutputContext.FORMAT ? "w" : ExifInterface.LONGITUDE_WEST;
            }
            return CalendarText.getInstance("frenchrev", locale).getTextForms(name(), getType(), str);
        }
    }
    ...
    private static class Merger implements ChronoMerger<FrenchRepublicanCalendar> {
        ...
        public FrenchRepublicanCalendar createFrom2(ChronoEntity<?> chronoEntity, AttributeQuery attributeQuery, boolean z, boolean z2) {
            ...
            if (i2 < 1 || i2 > FrenchRepublicanCalendar.MAX_YEAR) {
                chronoEntity.with(ValidationElement.ERROR_MESSAGE, "Republican year out of range: " + i2);
                return null;
            }
            if (chronoEntity.contains(FrenchRepublicanCalendar.MONTH_OF_YEAR)) {
                ...
                int value2 = chronoEntity.getInt(FrenchRepublicanCalendar.DAY_OF_MONTH);
                if (value2 == Integer.MIN_VALUE && chronoEntity.contains(FrenchRepublicanCalendar.DAY_OF_DECADE) && (i = chronoEntity.getInt(FrenchRepublicanCalendar.DECADE_OF_MONTH)) != Integer.MIN_VALUE) {
                    value2 = ((DayOfDecade) chronoEntity.get(FrenchRepublicanCalendar.DAY_OF_DECADE)).getValue() + ((i - 1) * 10);
                }
                if (value2 != Integer.MIN_VALUE) {
                    if (value2 >= 1 && value2 <= 30) {
                        frenchRepublicanCalendar = FrenchRepublicanCalendar.of(i2, value, value2);
                    } else {
                ...
                int value3 = ((Sansculottides) chronoEntity.get(FrenchRepublicanCalendar.SANSCULOTTIDES)).getValue() + 360;
                if (value3 == 6 && !frenchRepublicanAlgorithm.isLeapYear(i2)) {
                    chronoEntity.with(ValidationElement.ERROR_MESSAGE, "Republican date is no leap year.");
                } else {
                    frenchRepublicanCalendar = new FrenchRepublicanCalendar(i2, value3);
                }
            } else {
                int i3 = chronoEntity.getInt(FrenchRepublicanCalendar.DAY_OF_YEAR);
                if (i3 != Integer.MIN_VALUE) {
                    if (i3 >= 1) {
                        if (i3 <= (frenchRepublicanAlgorithm.isLeapYear(i2) ? 366 : 365)) {
                            frenchRepublicanCalendar = new FrenchRepublicanCalendar(i2, i3);
                        } else {
                            chronoEntity.with(ValidationElement.ERROR_MESSAGE, "Invalid republican date.");
                        }
                    } else {
    ...
    private static class FUnitRule implements UnitRule<FrenchRepublicanCalendar> {
        ...
        public long between(FrenchRepublicanCalendar frenchRepublicanCalendar, FrenchRepublicanCalendar frenchRepublicanCalendar2) {
            ...
            if (i == 2) {
                ...
                if (jYmValue <= 0 || dayOfMonth2 >= dayOfMonth) {
                    return (jYmValue >= 0 || dayOfMonth2 <= dayOfMonth) ? jYmValue : jYmValue + 1;
                }
                return jYmValue - 1;
            }
            if (i != 3) {
            ...
            if (jDecValue <= 0 || value2 >= value) {
                return (jDecValue >= 0 || value2 <= value) ? jDecValue : jDecValue + 1;
            }
            return jDecValue - 1;
        }
    ...
    /* JADX INFO: renamed from: net.time4j.calendar.frenchrev.FrenchRepublicanCalendar$1, reason: invalid class name */
