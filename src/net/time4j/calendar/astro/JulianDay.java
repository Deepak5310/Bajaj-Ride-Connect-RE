...
public final class JulianDay implements Serializable {
    ...
    public static JulianDay ofEphemerisTime(CalendarDate calendarDate, PlainTime plainTime, ZonalOffset zonalOffset) {
        return new JulianDay(((EpochDays.JULIAN_DAY_NUMBER.transform(calendarDate.getDaysSinceEpochUTC(), EpochDays.UTC) - 0.5d) + (((Long) plainTime.get(PlainTime.NANO_OF_DAY)).longValue() / 8.64E13d)) - (((double) zonalOffset.getIntegralAmount()) / 86400.0d), TimeScale.TT);
    }
    ...
    static double getValue(Moment moment, TimeScale timeScale) {
        return ((moment.getElapsedTime(timeScale) + jdOffset(timeScale)) + (((double) moment.getNanosecond(timeScale)) / 1.0E9d)) / 86400.0d;
    }

    /* JADX INFO: renamed from: net.time4j.calendar.astro.JulianDay$1, reason: invalid class name */
