...
public enum MoonPhase {
    ...
    private static final Moment ZERO_REF = PlainTimestamp.of(Constants.MAX_URL_LENGTH, 1, 6, 18, 13, 42).atUTC();
    ...
    public Moment after(Moment moment) {
        ...
        Moment momentAtLunation = atLunation(estimatedLunations);
        int i = estimatedLunations;
        while (!momentAtLunation.isAfter((UniversalTime) moment)) {
            i++;
            momentAtLunation = atLunation(i);
        }
        if (i <= estimatedLunations) {
            while (true) {
                i--;
                Moment momentAtLunation2 = atLunation(i);
                if (!momentAtLunation2.isAfter((UniversalTime) moment)) {
                    break;
                }
                momentAtLunation = momentAtLunation2;
            }
        }
        return momentAtLunation;
    }
    ...
    public Moment atOrAfter(Moment moment) {
        ...
        Moment momentAtLunation = atLunation(estimatedLunations);
        int i = estimatedLunations;
        while (momentAtLunation.isBefore((UniversalTime) moment)) {
            i++;
            momentAtLunation = atLunation(i);
        }
        if (i <= estimatedLunations) {
            while (true) {
                i--;
                Moment momentAtLunation2 = atLunation(i);
                if (momentAtLunation2.isBefore((UniversalTime) moment)) {
                    break;
                }
                momentAtLunation = momentAtLunation2;
            }
    ...
    public Moment before(Moment moment) {
        ...
        if (i >= estimatedLunations) {
            while (momentAtLunation.plus(29L, TimeUnit.DAYS).isBefore((UniversalTime) moment)) {
                i++;
                Moment momentAtLunation2 = atLunation(i);
                if (!momentAtLunation2.isBefore((UniversalTime) moment)) {
                    break;
    ...
    public Moment atLunation(int i) {
        ...
        double dSin = ((((MEAN_SYNODIC_MONTH * d) + 2451550.09766d) + (((((7.3E-10d * d2) - 1.5E-7d) * d2) + 1.5437E-4d) * d3)) - (Math.sin(Math.toRadians((124.7746d - (1.56375588d * d)) + (((2.15E-6d * d2) + 0.0020672d) * d3))) * 1.7E-4d)) + periodic24(d4, d5, d6, d7);
        if (this == FIRST_QUARTER) {
            dSin += corrQuarter(d4, d5, d6, d7);
        ...
        for (int i2 = 0; i2 < 28; i2 += 2) {
            dSin += dArr[i2 + 1] * Math.sin(Math.toRadians(dArr[i2]));
        }
        return (Moment) JulianDay.ofEphemerisTime(dSin).toMoment().with(Moment.PRECISION, TimeUnit.SECONDS);
    }
    ...
    public static double getIllumination(Moment moment, int i) {
        ...
        double dCos = (cos(((((((180.0d - meanElongation) - (sin(meanAnomalyOfMoon) * 6.289d)) + (sin(meanAnomalyOfSun) * 2.1d)) - (sin(d - meanAnomalyOfMoon) * 1.274d)) - (sin(d) * 0.658d)) - (sin(meanAnomalyOfMoon * 2.0d) * 0.214d)) - (sin(meanElongation) * 0.11d)) + 1.0d) / 2.0d;
        double d2 = FACTORS[i];
        double d3 = dCos * d2;
        if (d2 - d3 <= 0.5d) {
            return 1.0d;
    ...
    private int getEstimatedLunations(Moment moment) {
        return MathUtils.safeCast(Math.round((ZERO_REF.until(moment, TimeUnit.DAYS) / MEAN_SYNODIC_MONTH) - (((double) this.phase) / 360.0d)));
    }
    ...
    private double periodic24(double d, double d2, double d3, double d4) {
        ...
        for (int i = 23; i >= 0; i--) {
            ...
            dSin += d5 * sin((((double) iArr2[i]) * d2) + (((double) iArr3[i]) * d3) + (((double) iArr4[i]) * d4));
        }
        return dSin;
    }
