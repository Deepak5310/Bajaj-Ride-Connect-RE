...
public enum StdSolarCalculator implements SolarTime.Calculator {
    SIMPLE { // from class: net.time4j.calendar.astro.StdSolarCalculator.1
        ...
        private Moment event(CalendarDate calendarDate, double d, double d2, double d3, boolean z) {
            ...
            double d5 = (((degrees / 15.0d) + dFloor) - (dayOfYear * 0.06571d)) - 6.622d;
            if (Double.compare(0.0d, d5) > 0) {
                d5 += 24.0d;
            } else if (Double.compare(24.0d, d5) <= 0) {
                d5 -= 24.0d;
            }
            long jLongValue = (((Long) gregorian.get(EpochDays.UTC)).longValue() * 86400) + ((long) ((int) Math.floor((d5 - d4) * 3600.0d)));
            TimeScale timeScale = TimeScale.UT;
            if (!LeapSeconds.getInstance().isEnabled()) {
                jLongValue += 63072000;
                timeScale = TimeScale.POSIX;
            }
            return (Moment) Moment.of(Math.round(jLongValue / 60.0d) * 60, timeScale).with(Moment.PRECISION, TimeUnit.MINUTES);
        }
    },
    NOAA { // from class: net.time4j.calendar.astro.StdSolarCalculator.2
        ...
        private Moment event(boolean z, CalendarDate calendarDate, double d, double d2, double d3) {
            ...
            long jFloor = (long) Math.floor(dLocalHourAngle2);
            return (Moment) momentFromLocalEvent.plus(jFloor, TimeUnit.SECONDS).plus((int) ((dLocalHourAngle2 - jFloor) * 1.0E9d), TimeUnit.NANOSECONDS).with(Moment.PRECISION, TimeUnit.SECONDS);
        }
    ...
    CC { // from class: net.time4j.calendar.astro.StdSolarCalculator.3
        ...
        private Moment event(boolean z, CalendarDate calendarDate, double d, double d2, double d3) {
            ...
            return (Moment) JulianDay.ofEphemerisTime(dMomentOfDepression - integralAmount).toMoment().with(Moment.PRECISION, TimeUnit.SECONDS);
        }
        ...
        private double approxMomentOfDepression(double d, double d2, double d3, double d4, boolean z) {
            ...
            double degrees = (((double) (z ? -1 : 1)) * ((((Math.toDegrees(Math.asin(dSineOffset)) / 360.0d) + 0.5d) % 1.0d) - 0.25d)) + jFloor + 0.5d;
    ...
    TIME4J { // from class: net.time4j.calendar.astro.StdSolarCalculator.4
        ...
        public double getGeodeticAngle(double d, int i) {
            ...
            return Math.toDegrees(Math.acos(dSqrt / (((double) i) + dSqrt)));
        }
        ...
        private Moment event(boolean z, CalendarDate calendarDate, double d, double d2, double d3) {
            ...
            while (true) {
                ...
                if (Math.abs(dLocalHourAngle - d4) < 15.0d) {
                    long jFloor = (long) Math.floor(dLocalHourAngle);
                    return (Moment) momentFromLocalEvent.plus(jFloor, TimeUnit.SECONDS).plus((int) ((dLocalHourAngle - jFloor) * 1.0E9d), TimeUnit.NANOSECONDS).with(Moment.PRECISION, TimeUnit.SECONDS);
                }
                d4 = dLocalHourAngle;
            }
        }
    ...
    private static final int[] DG_X = {403406, 195207, 119433, 112392, 3891, 2819, 1721, 660, 350, 334, 314, 268, 242, 234, 158, NikonType2MakernoteDirectory.TAG_LENS, 129, 114, 99, 93, 86, 78, 72, 68, 64, 46, 38, 37, 32, 29, 28, 27, 27, 25, 24, 21, 21, 20, 18, 17, 14, 13, 13, 13, 12, 10, 10, 10, 10};
    ...
    public static double apparentSolarLongitude(double d, double d2) {
        ...
        for (int length = DG_X.length - 1; length >= 0; length--) {
            dSin += ((double) DG_X[length]) * Math.sin(Math.toRadians(DG_Y[length] + (DG_Z[length] * d)));
        }
