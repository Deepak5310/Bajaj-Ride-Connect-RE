...
public final class SolarTime implements GeoLocation, Serializable {
    ...
    public static ChronoFunction<Moment, PlainTimestamp> apparentAt(final ZonalOffset zonalOffset) {
        return new ChronoFunction<Moment, PlainTimestamp>() { // from class: net.time4j.calendar.astro.SolarTime.12
            ...
            public PlainTimestamp apply(Moment moment) {
                PlainTimestamp plainTimestampOnAverage = SolarTime.onAverage(moment, zonalOffset);
                double dEquationOfTime = SolarTime.equationOfTime(moment);
                long jFloor = (long) Math.floor(dEquationOfTime);
                return plainTimestampOnAverage.plus(jFloor, ClockUnit.SECONDS).plus((int) ((dEquationOfTime - jFloor) * 1.0E9d), ClockUnit.NANOS);
            }
        };
    }
    ...
    public static ChronoFunction<Moment, PlainTimestamp> apparentAt(final ZonalOffset zonalOffset, final String str) {
        return new ChronoFunction<Moment, PlainTimestamp>() { // from class: net.time4j.calendar.astro.SolarTime.13
            ...
            public PlainTimestamp apply(Moment moment) {
                PlainTimestamp plainTimestampOnAverage = SolarTime.onAverage(moment, zonalOffset);
                double dEquationOfTime = SolarTime.equationOfTime(moment, str);
                long jFloor = (long) Math.floor(dEquationOfTime);
                return plainTimestampOnAverage.plus(jFloor, ClockUnit.SECONDS).plus((int) ((dEquationOfTime - jFloor) * 1.0E9d), ClockUnit.NANOS);
            }
        };
    ...
    static Moment fromLocalEvent(CalendarDate calendarDate, int i, double d, String str) {
        ...
        double daysSinceEpochUTC = ((calendarDate.getDaysSinceEpochUTC() * 86400) + ((long) (i * 3600))) - (d * 240.0d);
        ...
        double dEquationOfTime = calculator.equationOfTime(JulianDay.getValue(momentOf, TimeScale.TT));
        long jFloor2 = (long) Math.floor(dEquationOfTime);
        double dEquationOfTime2 = calculator.equationOfTime(JulianDay.getValue(momentOf.minus(jFloor2, TimeUnit.SECONDS).minus((int) ((dEquationOfTime - jFloor2) * 1.0E9d), TimeUnit.NANOSECONDS), TimeScale.TT));
        long jFloor3 = (long) Math.floor(dEquationOfTime2);
        return momentOf.minus(jFloor3, TimeUnit.SECONDS).minus((int) ((dEquationOfTime2 - jFloor3) * 1.0E9d), TimeUnit.NANOSECONDS);
    }
    ...
    private Moment timeOfShadow(long j, long j2, double d) {
        ...
        double elevation = SunPosition.at(momentOf, this).getElevation();
        if (Math.abs(elevation - d) < ARC_MIN) {
            return momentOf;
        }
        if (Double.compare(d, elevation) > 0.0d) {
