...
public final class HebrewCalendar extends Calendrical<Unit, HebrewCalendar> implements LocalizedPatternSupport {
    ...
    private static double hcMolad(int i, HebrewMonth hebrewMonth) {
        ...
        return (FIXED_EPOCH - 0.033796296296296297d) + (((double) ((i2 - 7) + MathUtils.floorDivide((i * 235) - 234, 19))) * 29.53059413580247d);
    }
    ...
    private static class Transformer implements EraYearMonthDaySystem<HebrewCalendar> {
        ...
        public HebrewCalendar transform(long j) {
            ...
            for (int i3 = 1; i3 < 13; i3++) {
                if (i3 != 6 || zIsLeapYear) {
                    long jLengthOfMonth = jHcNewYear - ((long) HebrewCalendar.lengthOfMonth(i, HebrewMonth.valueOf(i3)));
                    if (jLengthOfMonth <= 0) {
                        break;
                    }
                    i2 = i3 + 1;
        ...
        public long transform(HebrewCalendar hebrewCalendar) {
            long jTransform = (EpochDays.UTC.transform(HebrewCalendar.hcNewYear(hebrewCalendar.year), EpochDays.RATA_DIE) + ((long) hebrewCalendar.dom)) - 1;
            boolean zIsLeapYear = HebrewCalendar.isLeapYear(hebrewCalendar.year);
            int value = hebrewCalendar.month.getValue();
            for (int i = 1; i < value; i++) {
                if (zIsLeapYear || i != 6) {
                    jTransform += (long) HebrewCalendar.lengthOfMonth(hebrewCalendar.year, HebrewMonth.valueOf(i));
                }
            }
            return jTransform;
        }
    ...
    private static class HebrewUnitRule implements UnitRule<HebrewCalendar> {
        ...
        /* JADX WARN: Code duplicated, block: B:36:0x0084 A[PHI: r5
        ...
        public HebrewCalendar addTo(HebrewCalendar hebrewCalendar, long j) {
            ...
            if (i2 == 1) {
                int iSafeCast = MathUtils.safeCast(MathUtils.safeAdd(hebrewCalendar.year, j));
                if (iSafeCast >= 1 && iSafeCast <= 9999) {
                    HebrewMonth hebrewMonth = hebrewCalendar.month;
                    if (hebrewMonth == HebrewMonth.ADAR_I && !HebrewCalendar.isLeapYear(iSafeCast)) {
                        hebrewMonth = HebrewMonth.SHEVAT;
                    }
                    return new HebrewCalendar(iSafeCast, hebrewMonth, Math.min(hebrewCalendar.dom, HebrewCalendar.lengthOfMonth(iSafeCast, hebrewMonth)));
                }
            ...
            int value = hebrewCalendar.month.getValue();
            for (long jAbs = Math.abs(j); jAbs > 0; jAbs--) {
                if (j > 0) {
                    i = value + 1;
            ...
            HebrewMonth hebrewMonthValueOf = HebrewMonth.valueOf(value);
            return HebrewCalendar.of(i3, hebrewMonthValueOf, Math.min(hebrewCalendar.dom, HebrewCalendar.lengthOfMonth(i3, hebrewMonthValueOf)));
        }
    ...
    /* JADX INFO: renamed from: net.time4j.calendar.HebrewCalendar$3, reason: invalid class name */
