...
public class WeekdayRule<D extends CalendarDate> implements ElementRule<D, Weekday> {
    ...
    public Weekday getMinimum(D d) {
        ...
        if ((d.getDaysSinceEpochUTC() + 1) - ((long) getValue((CalendarDate) d).getValue(this.stdWeekmodel)) < calendarSystemApply.getMinimumSinceUTC()) {
    ...
    public Weekday getMaximum(D d) {
        CalendarSystem<D> calendarSystemApply = this.calsysFunc.apply(d);
        if ((d.getDaysSinceEpochUTC() + 7) - ((long) getValue((CalendarDate) d).getValue(this.stdWeekmodel)) > calendarSystemApply.getMaximumSinceUTC()) {
    ...
    public boolean isValid2(D d, Weekday weekday) {
        ...
        long daysSinceEpochUTC = (d.getDaysSinceEpochUTC() + ((long) weekday.getValue(this.stdWeekmodel))) - ((long) getValue((CalendarDate) d).getValue(this.stdWeekmodel));
        CalendarSystem<D> calendarSystemApply = this.calsysFunc.apply(d);
    ...
    public D withValue2(D d, Weekday weekday, boolean z) {
        ...
        int value = getValue((CalendarDate) d).getValue(this.stdWeekmodel);
