...
abstract class EastAsianCS<D extends EastAsianCalendar<?, D>> implements CalendarSystem<D> {
    ...
    public final D transform(long j) {
        ...
        long jFloor = (long) Math.floor((1.5d - (((double) i) / 12.0d)) + ((j - EPOCH_CHINESE) / MEAN_TROPICAL_YEAR));
    ...
    final long newYear(int i, int i2) {
        return newYearOnOrBefore((long) Math.floor(EPOCH_CHINESE + ((((double) (((i - 1) * 60) + i2)) - 0.5d) * MEAN_TROPICAL_YEAR)));
    }
    ...
    private long firstDayOfMonth(int i, int i2, EastAsianMonth eastAsianMonth) {
        long jNewMoonOnOrAfter = newMoonOnOrAfter(newYear(i, i2) + ((long) ((eastAsianMonth.getNumber() - 1) * 29)));
