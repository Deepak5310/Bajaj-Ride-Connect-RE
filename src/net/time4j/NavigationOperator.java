...
final class NavigationOperator<V extends Enum<V>> extends ElementOperator<PlainDate> {
    ...
    public <T extends ChronoEntity<T>> T doApply(T t) {
        ...
        if (t.contains(PlainDate.CALENDAR_DATE)) {
            ...
            int iDelta = delta(iOrdinal);
            return iDelta == iOrdinal ? t : (T) t.with(PlainDate.CALENDAR_DATE, plainDate.plus(iDelta - iOrdinal, plainDate.getChronology().getBaseUnit(getElement())));
        }
        switch (getType()) {
            case 9:
                str = "setToNext";
