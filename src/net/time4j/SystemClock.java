...
public final class SystemClock implements TimeSource<Moment> {
    ...
    static {
        ...
        Iterator it2 = ResourceLoader.getInstance().services(TickProvider.class).iterator();
        do {
            if (!it2.hasNext()) {
                stdTickProvider = null;
                break;
            }
            stdTickProvider = (TickProvider) it2.next();
        } while (!property.equals(stdTickProvider.getPlatform()));
    ...
    public long currentTimeInMillis() {
        if (this.monotonic || MONOTON_MODE) {
            long jUtcNanos = utcNanos();
            return MathUtils.safeMultiply(LeapSeconds.getInstance().strip(MathUtils.floorDivide(jUtcNanos, 1000000000)), 1000L) + ((long) MathUtils.floorModulo(jUtcNanos, 1000000));
        }
    ...
    public long currentTimeInMicros() {
        if (this.monotonic || MONOTON_MODE) {
            long jUtcNanos = utcNanos();
            return MathUtils.safeMultiply(LeapSeconds.getInstance().strip(MathUtils.floorDivide(jUtcNanos, 1000000000)), 1000000L) + ((long) MathUtils.floorModulo(jUtcNanos, 1000));
        }
    ...
    public long realTimeInMicros() {
        ...
        long jCurrentTimeMillis = System.currentTimeMillis();
        return MathUtils.safeMultiply(LeapSeconds.getInstance().enhance(MathUtils.floorDivide(jCurrentTimeMillis, 1000)), 1000000L) + ((long) (MathUtils.floorModulo(jCurrentTimeMillis, 1000) * 1000));
    }
    ...
    public SystemClock synchronizedWith(TimeSource<?> timeSource) {
        Moment momentFrom = Moment.from(timeSource.currentTime());
        return new SystemClock(this.monotonic, MathUtils.safeSubtract(MathUtils.safeMultiply(momentFrom.getElapsedTime(TimeScale.UTC), 1000000000L) + ((long) momentFrom.getNanosecond(TimeScale.UTC)), MONOTON_MODE ? System.nanoTime() : PROVIDER.getNanos()));
    }
    ...
    private static long calibrate() {
        ...
        return MathUtils.safeSubtract(MathUtils.safeMultiply(LeapSeconds.getInstance().enhance(MathUtils.floorDivide(jCurrentTimeMillis, 1000)), 1000000000L) + ((long) (MathUtils.floorModulo(jCurrentTimeMillis, 1000) * 1000000)), jNanoTime);
    }
