...
public final class CalendarText {
    ...
    static {
        ...
        if (it2.hasNext()) {
            isoTextProviderSPI = (FormatPatternProvider) it2.next();
        } else {
    ...
    private CalendarText(String str, Locale locale, TextProvider textProvider) throws Throwable {
        ...
        while (i2 < length) {
            ...
            for (int i3 = i; i3 < length2; i3++) {
                ...
                enumMap2.put(outputContext, new TextAccessor(textProvider.quarters(str, locale, textWidth, outputContext)));
            }
        ...
        for (TextWidth textWidth2 : TextWidth.values()) {
            ...
            for (OutputContext outputContext2 : OutputContext.values()) {
                enumMap4.put(outputContext2, new TextAccessor(textProvider.weekdays(str, locale, textWidth2, outputContext2)));
            }
        ...
        for (TextWidth textWidth3 : TextWidth.values()) {
            enumMap5.put(textWidth3, new TextAccessor(textProvider.eras(str, locale, textWidth3)));
        }
        ...
        for (TextWidth textWidth4 : TextWidth.values()) {
            ...
            for (OutputContext outputContext3 : OutputContext.values()) {
                enumMap7.put(outputContext3, new TextAccessor(textProvider.meridiems(str, locale, textWidth4, outputContext3)));
            }
        ...
        try {
            PropertyBundle propertyBundleLoad = PropertyBundle.load("calendar/names/" + str + RemoteSettings.FORWARD_SLASH_STRING + str, locale);
    ...
    public static CalendarText getInstance(String str, Locale locale) {
        ...
        if (locale.getLanguage().isEmpty() && str.equals(ISO_CALENDAR_TYPE)) {
            ...
            Iterator it2 = ResourceLoader.getInstance().services(TextProvider.class).iterator();
            while (true) {
                if (!it2.hasNext()) {
                    textProvider = null;
                    break;
                }
                TextProvider textProvider2 = (TextProvider) it2.next();
    ...
    public String toString() {
        return this.provider + "(" + this.calendarType + RemoteSettings.FORWARD_SLASH_STRING + this.locale + ")";
    }
    ...
    private static Map<TextWidth, Map<OutputContext, TextAccessor>> getMonths(String str, Locale locale, TextProvider textProvider, boolean z) {
        ...
        int i = 0;
        while (i < length) {
            TextWidth textWidth = textWidthArrValues[i];
            EnumMap enumMap2 = new EnumMap(OutputContext.class);
            OutputContext[] outputContextArrValues = OutputContext.values();
            int length2 = outputContextArrValues.length;
            boolean z3 = z2;
            int i2 = 0;
            while (i2 < length2) {
                OutputContext outputContext = outputContextArrValues[i2];
                int i3 = i2;
                String[] strArrMonths = textProvider.months(str, locale, textWidth, outputContext, z);
                if (z && !z3) {
                    z3 = !Arrays.equals(textProvider.months(str, locale, textWidth, outputContext, false), strArrMonths);
                }
                enumMap2.put(outputContext, new TextAccessor(strArrMonths));
                i2 = i3 + 1;
                length2 = length2;
                outputContextArrValues = outputContextArrValues;
                enumMap2 = enumMap2;
                textWidth = textWidth;
            }
            enumMap.put(textWidth, enumMap2);
    ...
    /* JADX INFO: renamed from: net.time4j.format.CalendarText$1, reason: invalid class name */
