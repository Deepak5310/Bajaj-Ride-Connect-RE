...
public class Iso8601Format {
    ...
    private static ChronoFormatter<PlainDate> calendarFormat(boolean z) {
        ChronoFormatter.Builder builderAddInteger = ChronoFormatter.setUp(PlainDate.class, Locale.ROOT).startSection(Attributes.NUMBER_SYSTEM, NumberSystem.ARABIC).startSection(Attributes.ZERO_DIGIT, '0').addInteger(PlainDate.YEAR, 4, 9, SignPolicy.SHOW_WHEN_BIG_NUMBER);
        if (z) {
    ...
    private static ChronoFormatter<PlainDate> weekdateFormat(boolean z) {
        ChronoFormatter.Builder builderAddInteger = ChronoFormatter.setUp(PlainDate.class, Locale.ROOT).startSection(Attributes.NUMBER_SYSTEM, NumberSystem.ARABIC).startSection(Attributes.ZERO_DIGIT, '0').addInteger(PlainDate.YEAR_OF_WEEKDATE, 4, 9, SignPolicy.SHOW_WHEN_BIG_NUMBER);
        if (z) {
            builderAddInteger.addLiteral('-');
        }
