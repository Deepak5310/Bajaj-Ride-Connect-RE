    ...
    public void parse(CharSequence charSequence, ParseLog parseLog, AttributeQuery attributeQuery, ParsedEntity<?> parsedEntity, boolean z) {
        ...
        if (this.reserved <= 0 || iIntValue > 0) {
            ...
            for (int i3 = position; i3 < length; i3++) {
                int iCharAt = charSequence.charAt(i3) - cCharValue;
                if (iCharAt < 0 || iCharAt > 9) {
                    break;
                }
                i2++;
            }
            iMin = Math.min(9, i2 - this.reserved);
        }
        int i4 = position + 1;
        ...
        while (i5 < iMin2) {
            int i6 = iMin2;
            int iCharAt2 = charSequence.charAt(i5) - cCharValue;
            if (iCharAt2 < 0 || iCharAt2 > i) {
                if (!z2) {
                    break;
                }
        ...
        int length2 = indicator.length() + i5;
        if (length2 >= length) {
            parseLog.setError(i5, "Missing or wrong ordinal indicator for: " + this.element.name());
            return;
        }
        String string = charSequence.subSequence(i5, length2).toString();
        if (string.equals(indicator)) {
            i5 = length2;
        } else if (!leniency.isLax()) {
            parseLog.setError(i5, "Wrong ordinal indicator for: " + this.element.name() + " (expected=" + indicator + ", found=" + string + ")");
            return;
        }
