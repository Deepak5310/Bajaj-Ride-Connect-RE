...
final class TimezoneNameProcessor implements FormatProcessor<TZID> {
    ...
    public void parse(CharSequence charSequence, ParseLog parseLog, AttributeQuery attributeQuery, ParsedEntity<?> parsedEntity, boolean z) {
        ...
        if (size <= 1 || leniency.isLax()) {
            ...
            if (!(tzid instanceof ZonalOffset)) {
                Iterator<TZID> it2 = arrayList.iterator();
                while (true) {
                    if (it2.hasNext()) {
                        TZID next = it2.next();
                ...
                if (!z2) {
                    for (TZID tzid2 : listResolveUsingPreferred2) {
                        if (tzid2.canonical().equals(tzid.canonical())) {
                            listResolveUsingPreferred = Collections.emptyList();
                            listResolveUsingPreferred2 = Collections.singletonList(tzid2);
                            z2 = true;
                            break;
                        }
                    }
    ...
    private List<TZID> resolveUsingPreferred(List<TZID> list, Locale locale, Leniency leniency) {
        ...
        Iterator<TZID> it2 = list.iterator();
        while (true) {
            z = false;
            if (!it2.hasNext()) {
                break;
            }
            String strCanonical = it2.next().canonical();
            ...
            for (TZID tzid : preferredIDs) {
                if (tzid.canonical().equals(strCanonical)) {
