...
final class SPX implements Externalizable {
    ...
    private static int toTimeOfDayR(int i) {
        switch (i) {
            ...
            case 2:
                return 3600;
            case 3:
                return TurnByTurnManager.MSG_NAV_INFO;
            case 4:
                return 10800;
            case 5:
                return 79200;
    ...
    private static List<ZonalTransition> readTransitions(ObjectInput objectInput) throws IOException {
        ...
        while (i4 < i2) {
            ...
            if (timeOfDayT == -1) {
                ...
                j = ((((((long) ((((b & 3) << 16) | ((objectInput.readByte() & 255) << 8)) | (objectInput.readByte() & 255))) * 86400) + POSIX_TIME_1825) + ((long) timeOfDayT)) - 7200) - ((long) offset2);
            }
            if (j <= j2) {
            ...
            if (i5 != 1) {
                if (i5 != 2) {
                    offset = i5 != 3 ? readOffset(objectInput) : TurnByTurnManager.MSG_NAV_INFO;
                } else {
                    offset = 3600;
                }
                i = offset;
    ...
    private static int readOffset(DataInput dataInput) throws IOException {
        ...
        return b == 127 ? dataInput.readInt() : b * 900;
    }
    ...
    private static DaylightSavingRule readLastDayOfWeekPattern(DataInput dataInput) throws IOException {
        ...
        return new LastWeekdayPattern(monthValueOf, weekdayValueOf, i2 == 31 ? dataInput.readInt() : i2 * 3600, offsetIndicator, savings);
    }
    ...
    private static Object readRuleBasedTransitionModel(ObjectInput objectInput) throws IOException, ClassNotFoundException {
        ...
        if (i == 255) {
            ...
            j = (((long) ((i << 16) + ((objectInput.readByte() & 255) << 8) + (255 & objectInput.readByte()))) * 900) + POSIX_TIME_1825;
        }
    ...
    private static int writeTransition(ZonalTransition zonalTransition, int i, DataOutput dataOutput) throws IOException {
        ...
        if (daylightSavingOffset != 0) {
            if (daylightSavingOffset != 3600) {
                i2 = daylightSavingOffset != 7200 ? 0 : 3;
            } else {
                i2 = 2;
            }
        }
        byte b2 = (byte) (b | (i2 << 5));
        long posixTime = zonalTransition.getPosixTime() + ((long) i);
        long j = 7200 + posixTime;
        if (j >= POSIX_TIME_1825 && j < 18067104000L) {
            timeIndexT = toTimeIndexT(MathUtils.floorModulo(j, DateTimeConstants.SECONDS_PER_DAY));
        }
