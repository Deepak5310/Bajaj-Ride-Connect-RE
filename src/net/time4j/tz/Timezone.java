...
public abstract class Timezone implements Serializable {
    ...
    static {
        ...
        Iterator<Class<? extends TZID>> it2 = listEmptyList.iterator();
        while (it2.hasNext()) {
            for (TZID tzid : (TZID[]) it2.next().getEnumConstants()) {
    ...
    public static List<TZID> getAvailableIDs(String str) {
        if (!str.equals("INCLUDE_ALIAS")) {
            ...
            Iterator<String> it2 = provider.getAvailableIDs().iterator();
            while (it2.hasNext()) {
                arrayList.add(resolve(it2.next()));
            }
    ...
    public static Set<TZID> getPreferredIDs(Locale locale, boolean z, String str) {
        ...
        Iterator<String> it2 = specificZoneNameRepository.getPreferredIDs(locale, z).iterator();
        while (it2.hasNext()) {
            hashSet.add(resolve(it2.next()));
        }
    ...
    public static TZID normalize(String str) {
        ...
        if (!strSubstring.isEmpty() && !strSubstring.equals(NAME_DEFAULT) && !strSubstring.equals("WINDOWS") && !strSubstring.equals("MILITARY") && (zoneModelProvider = PROVIDERS.get(strSubstring)) == null) {
            if (strSubstring.equals(NAME_TZDB)) {
                str2 = "TZDB provider not available: ";
            } else {
                str2 = "Timezone model provider not registered: ";
            }
            throw new IllegalArgumentException(str2 + str);
        }
        Map<String, String> aliases = zoneModelProvider.getAliases();
        while (true) {
            String str3 = aliases.get(strSubstring2);
            if (str3 == null) {
                break;
            }
            strSubstring2 = str3;
    ...
    public static String getProviderInfo() {
        ...
        Iterator<String> it2 = PROVIDERS.keySet().iterator();
        while (it2.hasNext()) {
            ZoneModelProvider zoneModelProvider = PROVIDERS.get(it2.next());
    ...
    private static Timezone getTZ(TZID tzid, String str, boolean z) {
        ...
        if (!z2 && (zoneModelProvider = PROVIDERS.get(strSubstring2)) == null) {
            ...
            if (strSubstring2.equals(NAME_TZDB)) {
                str2 = "TZDB provider not available: ";
            } else {
                str2 = "Timezone model provider not registered: ";
            }
    ...
    private static void fillEtcetera(Map<String, TZID> map) {
        ...
        map.put("Etc/GMT-2", ZonalOffset.ofTotalSeconds(TurnByTurnManager.MSG_NAV_INFO));
    ...
    private static class ZonalKeys {
        ...
        ZonalKeys() {
            ...
            Iterator it2 = Timezone.PROVIDERS.entrySet().iterator();
            while (it2.hasNext()) {
                ZoneModelProvider zoneModelProvider = (ZoneModelProvider) ((Map.Entry) it2.next()).getValue();
                if (zoneModelProvider != Timezone.PLATFORM_PROVIDER || Timezone.DEFAULT_PROVIDER == Timezone.PLATFORM_PROVIDER) {
                    Iterator<String> it3 = zoneModelProvider.getAvailableIDs().iterator();
                    while (it3.hasNext()) {
                        TZID tzidResolve = Timezone.resolve(it3.next());
                        if (!arrayList.contains(tzidResolve)) {
                            arrayList.add(tzidResolve);
                        }
                    }
                    arrayList2.addAll(arrayList);
                    Iterator<String> it4 = zoneModelProvider.getAliases().keySet().iterator();
                    while (it4.hasNext()) {
                        TZID tzidResolve2 = Timezone.resolve(it4.next());
