...
public class WinZoneProviderSPI implements ZoneModelProvider, ZoneNameProvider {
    ...
    static {
        ...
        WIN_NAME_VERSION = mapLoadData.get("VERSION").keySet().iterator().next();
        mapLoadData.remove("VERSION");
    ...
    public Set<String> getAvailableIDs() {
        ...
        Iterator<TZID> it2 = Timezone.getAvailableIDs("DEFAULT").iterator();
        while (it2.hasNext()) {
            hashSet.add("WINDOWS~" + it2.next().canonical());
        }
    ...
    private static Map<String, Map<String, String>> loadData() throws Throwable {
        ...
        try {
            try {
                ...
                ObjectInputStream objectInputStream2 = new ObjectInputStream(inputStreamLoad);
                try {
                    String utf = objectInputStream2.readUTF();
                    HashMap map = new HashMap((Map) cast(objectInputStream2.readObject()));
                    map.put("VERSION", Collections.singletonMap(utf, utf));
                    try {
