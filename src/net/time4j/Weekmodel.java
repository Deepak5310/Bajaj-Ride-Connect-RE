...
public final class Weekmodel implements Serializable {
    ...
    static {
        Iterator it2 = ResourceLoader.getInstance().services(WeekdataProvider.class).iterator();
        LOCALIZED_WEEKDATA = it2.hasNext() ? (WeekdataProvider) it2.next() : null;
    }
        ...
        public boolean parseFromInt(ChronoEntity<?> chronoEntity, int i) {
            for (Weekday weekday : Weekday.values()) {
                if (weekday.getValue(Weekmodel.this) == i) {
                    chronoEntity.with(this, weekday);
                    return true;
                }
            }
            return false;
    ...
    private static class DRule<T extends ChronoEntity<T>> implements ElementRule<T, Weekday> {
        ...
        public T withValue2(T t, Weekday weekday, boolean z) {
            ...
            return (T) t.with(PlainDate.CALENDAR_DATE, plainDate.withDaysSinceUTC((daysSinceUTC + ((long) weekday.getValue(this.element.getModel()))) - ((long) dayOfWeek.getValue(this.element.getModel()))));
        }
    ...
    private static class CWRule<T extends ChronoEntity<T>> implements ElementRule<T, Integer> {
        ...
        public T withValue2(T t, Integer num, boolean z) {
            PlainDate plainDate = (PlainDate) t.get(PlainDate.CALENDAR_DATE);
            if (num == null || (!z && !isValid2((ChronoEntity) t, num))) {
    ...
    private static class BWRule<T extends ChronoEntity<T>> implements ElementRule<T, Integer> {
        ...
        private ChronoElement<?> getChild(T t, boolean z) {
            ...
            int iIntValue = getValue((ChronoEntity) t).intValue();
            if (z) {
                if (iIntValue >= (this.owner.isYearRelated() ? 52 : 4)) {
                    PlainDate plainDate2 = (PlainDate) plainDate.with(navigableElementLocalDayOfWeek, t.getMaximum(navigableElementLocalDayOfWeek));
                    if (this.owner.isYearRelated()) {
                ...
                PlainDate plainDate3 = (PlainDate) plainDate.with(navigableElementLocalDayOfWeek, t.getMinimum(navigableElementLocalDayOfWeek));
                if (this.owner.isYearRelated()) {
        ...
        private int getWeek(PlainDate plainDate, int i) {
            ...
            int value = Weekmodel.getDayOfWeek((plainDate.getDaysSinceUTC() - ((long) dayOfMonth)) + 1).getValue(this.owner.getModel());
