...
public final class Moment extends TimePoint<TimeUnit, Moment> implements UniversalTime, Temporal<UniversalTime> {
    ...
    /* JADX WARN: Code duplicated, block: B:45:0x011b A[PHI: r1 r3
      0x011b: PHI (r1v2 long) = (r1v0 long), (r1v0 long), (r1v9 long), (r1v10 long) binds: [B:8:0x0025, B:44:0x0119, B:38:0x00de, B:39:0x00e0] A[DONT_GENERATE, DONT_INLINE]
      0x011b: PHI (r3v1 int) = (r3v0 int), (r3v0 int), (r3v24 int), (r3v25 int) binds: [B:8:0x0025, B:44:0x0119, B:38:0x00de, B:39:0x00e0] A[DONT_GENERATE, DONT_INLINE]] */
    private Moment(long j, int i, TimeScale timeScale) {
        int nanos;
        long jFloor;
        long jSafeSubtract = j;
        int iSafeSubtract = i;
        if (timeScale == TimeScale.POSIX) {
            this.posixTime = jSafeSubtract;
            this.fraction = iSafeSubtract;
        } else {
            LeapSeconds leapSeconds = LeapSeconds.getInstance();
            if (leapSeconds.isEnabled()) {
                if (timeScale == TimeScale.UTC) {
                    nanos = iSafeSubtract;
                    jFloor = jSafeSubtract;
                } else if (timeScale == TimeScale.TAI) {
                    if (jSafeSubtract < 0) {
                        throw new IllegalArgumentException("TAI not supported before 1958-01-01: " + jSafeSubtract);
                    }
                    if (jSafeSubtract < UTC_TAI_DELTA) {
                        long jSafeAdd = MathUtils.safeAdd(jSafeSubtract, -441763168L);
                        int iSafeAdd = MathUtils.safeAdd(iSafeSubtract, 184000000);
                        if (iSafeAdd >= 1000000000) {
                            jSafeAdd = MathUtils.safeAdd(jSafeAdd, 1L);
                            iSafeAdd = MathUtils.safeSubtract(iSafeAdd, 1000000000);
                        }
                        double d = jSafeAdd + (((double) iSafeAdd) / 1.0E9d);
                        double dDeltaT = d - TimeScale.deltaT(PlainDate.of(MathUtils.floorDivide((long) (d - 42.184d), DateTimeConstants.SECONDS_PER_DAY), EpochDays.UTC));
                        jFloor = (long) Math.floor(dDeltaT);
                        nanos = toNanos(dDeltaT, jFloor);
                    } else {
                        nanos = iSafeSubtract;
                        jFloor = MathUtils.safeSubtract(jSafeSubtract, 441763210L);
                    }
                } else if (timeScale == TimeScale.GPS) {
                    long jSafeAdd2 = MathUtils.safeAdd(jSafeSubtract, UTC_GPS_DELTA);
                    if (jSafeAdd2 < UTC_GPS_DELTA) {
                        throw new IllegalArgumentException("GPS not supported before 1980-01-06: " + jSafeSubtract);
                    }
                    nanos = iSafeSubtract;
                    jFloor = jSafeAdd2;
                } else if (timeScale == TimeScale.TT) {
                    if (jSafeSubtract < 42 || (jSafeSubtract == 42 && iSafeSubtract < 184000000)) {
                        double d2 = jSafeSubtract + (((double) iSafeSubtract) / 1.0E9d);
                        double dDeltaT2 = d2 - TimeScale.deltaT(PlainDate.of(MathUtils.floorDivide((long) (d2 - 42.184d), DateTimeConstants.SECONDS_PER_DAY), EpochDays.UTC));
                        jFloor = (long) Math.floor(dDeltaT2);
                        nanos = toNanos(dDeltaT2, jFloor);
                    } else {
                        jSafeSubtract = MathUtils.safeSubtract(jSafeSubtract, 42L);
                        iSafeSubtract = MathUtils.safeSubtract(iSafeSubtract, 184000000);
                        if (iSafeSubtract < 0) {
                            jSafeSubtract = MathUtils.safeSubtract(jSafeSubtract, 1L);
                            iSafeSubtract = MathUtils.safeAdd(iSafeSubtract, 1000000000);
                        }
                        nanos = iSafeSubtract;
                        jFloor = jSafeSubtract;
                    }
                    ...
                    if (jSafeSubtract < 0) {
                        ...
                        double dDeltaT3 = ((jSafeSubtract + (((double) iSafeSubtract) / 1.0E9d)) + TimeScale.deltaT(PlainDate.of(MathUtils.floorDivide(jSafeSubtract, DateTimeConstants.SECONDS_PER_DAY), EpochDays.UTC))) - 42.184d;
                        jFloor = (long) Math.floor(dDeltaT3);
                        nanos = toNanos(dDeltaT3, jFloor);
                    }
                }
                long jStrip = leapSeconds.strip(jFloor);
                long jEnhance = jFloor - leapSeconds.enhance(jStrip);
                this.posixTime = jStrip;
                if (jEnhance == 0 || jStrip == MAX_LIMIT) {
                    this.fraction = nanos;
                } else if (jEnhance == 1) {
                    this.fraction = 1073741824 | nanos;
                } else {
                    throw new IllegalStateException("Cannot handle leap shift of " + jSafeSubtract + ".");
                }
                iSafeSubtract = nanos;
            } else {
        ...
        switch (AnonymousClass1.$SwitchMap$net$time4j$scale$TimeScale[timeScale.ordinal()]) {
            ...
            case 3:
                if (getElapsedTimeUTC() < 0) {
                    double dDeltaT = TimeScale.deltaT(getDateUTC()) + (this.posixTime - POSIX_UTC_DELTA) + (((double) getNanosecond()) / 1.0E9d);
            ...
            case 5:
                if (this.posixTime < POSIX_UTC_DELTA) {
                    double dDeltaT2 = TimeScale.deltaT(getDateUTC()) + (this.posixTime - POSIX_UTC_DELTA) + (((double) getNanosecond()) / 1.0E9d);
    ...
    public boolean equals(Object obj) {
        ...
        return getNanosecond() == moment.getNanosecond();
    }
    ...
    public PlainDate getDateUTC() {
        return PlainDate.of(MathUtils.floorDivide(this.posixTime, DateTimeConstants.SECONDS_PER_DAY), EpochDays.UNIX);
    }
    ...
    private double getModernUT() {
        double elapsedTimeUTC = ((getElapsedTimeUTC() + 42.184d) + (((double) getNanosecond()) / 1.0E9d)) - TimeScale.deltaT(getDateUTC());
        long jFloor = (long) Math.floor(elapsedTimeUTC);
        return Double.compare(1.0E9d - ((elapsedTimeUTC - ((double) jFloor)) * 1.0E9d), 1.0d) < 0 ? jFloor + 1 : elapsedTimeUTC;
    }
    ...
    private static int getTimeOfDay(Moment moment) {
        return MathUtils.floorModulo(moment.posixTime, DateTimeConstants.SECONDS_PER_DAY);
    }
    ...
    private static class TimeUnitRule implements UnitRule<Moment> {
        ...
        public long between(Moment moment, Moment moment2) {
            ...
            if (this.unit.compareTo(TimeUnit.SECONDS) >= 0) {
                ...
                jSafeAdd = MathUtils.safeAdd(MathUtils.safeMultiply(MathUtils.safeSubtract(moment2.getPosixTime(), moment.getPosixTime()), 1000000000L), moment2.getNanosecond() - moment.getNanosecond());
            }
    ...
    /* JADX INFO: renamed from: net.time4j.Moment$1, reason: invalid class name */
    ...
    private static class GlobalTimeLine implements TimeLine<Moment> {
        ...
        public Moment stepForward(Moment moment) {
            try {
                ...
                return moment.plus(1L, TimeUnit.NANOSECONDS);
            } catch (ArithmeticException unused) {
                return null;
            }
        }
        ...
        public Moment stepBackwards(Moment moment) {
            try {
                ...
                return moment.minus(1L, TimeUnit.NANOSECONDS);
            } catch (ArithmeticException unused) {
                return null;
            }
        }
    ...
    private static class PrecisionRule implements ElementRule<Moment, TimeUnit> {
        ...
        public TimeUnit getValue(Moment moment) {
            ...
            if (nanosecond == 0) {
                ...
                if (MathUtils.floorModulo(j, DateTimeConstants.SECONDS_PER_DAY) == 0) {
        ...
        public Moment withValue2(Moment moment, TimeUnit timeUnit, boolean z) {
            ...
            switch (AnonymousClass1.$SwitchMap$java$util$concurrent$TimeUnit[timeUnit.ordinal()]) {
                case 1:
                    return Moment.of(MathUtils.floorDivide(moment.posixTime, DateTimeConstants.SECONDS_PER_DAY) * 86400, TimeScale.POSIX);
                case 2:
