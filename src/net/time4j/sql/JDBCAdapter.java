...
public abstract class JDBCAdapter<S, T> extends TemporalType<S, T> {
    ...
    private static class SqlDateRule extends JDBCAdapter<Date, PlainDate> {
        ...
        public PlainDate translate(Date date) {
            ...
            return (PlainDate) PlainDate.axis().getCalendarSystem().transform(MathUtils.floorDivide(time, DateTimeConstants.MILLIS_PER_DAY) - 730);
        }
        ...
        public Date from(PlainDate plainDate) {
            ...
            long jSafeMultiply = MathUtils.safeMultiply(((Long) plainDate.get(EpochDays.UNIX)).longValue(), DateUtils.MILLIS_PER_DAY);
            if (!JDBCAdapter.WITH_SQL_UTC_CONVERSION) {
                jSafeMultiply -= (long) (Timezone.ofSystem().getOffset(plainDate, PlainTime.of(0)).getIntegralAmount() * 1000);
            }
    ...
    private static class SqlTimeRule extends JDBCAdapter<Time, PlainTime> {
        ...
        public PlainTime translate(Time time) {
            ...
            return (PlainTime) PlainTime.midnightAtStartOfDay().with((ChronoElement<Integer>) PlainTime.MILLI_OF_DAY, MathUtils.floorModulo(time2, DateTimeConstants.MILLIS_PER_DAY));
        }
        ...
        public Time from(PlainTime plainTime) {
            ...
            if (!JDBCAdapter.WITH_SQL_UTC_CONVERSION) {
                integralAmount -= (long) (Timezone.ofSystem().getOffset(JDBCAdapter.UNIX_DATE, plainTime).getIntegralAmount() * 1000);
            }
    ...
    private static class SqlTimestampRule extends JDBCAdapter<Timestamp, PlainTimestamp> {
        ...
        public PlainTimestamp translate(Timestamp timestamp) {
            ...
            return (PlainTimestamp) PlainTimestamp.of(PlainDate.of(MathUtils.floorDivide(time, DateTimeConstants.MILLIS_PER_DAY), EpochDays.UNIX), PlainTime.of(0).plus(MathUtils.floorModulo(time, DateTimeConstants.MILLIS_PER_DAY), ClockUnit.MILLIS)).with((ChronoElement<Integer>) PlainTime.NANO_OF_SECOND, timestamp.getNanos());
        }

        @Override // net.time4j.TemporalType, net.time4j.engine.Converter
        public Timestamp from(PlainTimestamp plainTimestamp) {
            long jSafeMultiply = MathUtils.safeMultiply(((Long) plainTimestamp.getCalendarDate().get(EpochDays.UNIX)).longValue(), DateUtils.MILLIS_PER_DAY);
            long integralAmount = plainTimestamp.getInt(PlainTime.MILLI_OF_DAY);
            if (!JDBCAdapter.WITH_SQL_UTC_CONVERSION) {
                integralAmount -= (long) (Timezone.ofSystem().getOffset(plainTimestamp, plainTimestamp).getIntegralAmount() * 1000);
            }
