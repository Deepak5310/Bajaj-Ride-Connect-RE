...
final class SupportedSurfaceCombination {
    ...
    private static final Size DEFAULT_SIZE = new Size(OlympusMakernoteDirectory.TAG_PREVIEW_IMAGE, StreamingImageHandler.TFT_HEIGHT);
    private static final Size ZERO_SIZE = new Size(0, 0);
    private static final Size QUALITY_1080P_SIZE = new Size(1920, PhotoshopDirectory.TAG_COUNT_INFORMATION);
    private static final Size QUALITY_480P_SIZE = new Size(720, StreamingImageHandler.TFT_HEIGHT);
    ...
    boolean checkSupported(List<SurfaceConfig> list) {
        ...
        while (it2.hasNext() && !(zIsSupported = it2.next().isSupported(list))) {
        }
        return zIsSupported;
    }
    ...
    Map<UseCaseConfig<?>, Size> getSuggestedResolutions(List<SurfaceConfig> list, List<UseCaseConfig<?>> list2) {
        ...
        while (it2.hasNext()) {
            arrayList.add(transformSurfaceConfig(it2.next().getInputFormat(), new Size(OlympusMakernoteDirectory.TAG_PREVIEW_IMAGE, StreamingImageHandler.TFT_HEIGHT)));
        }
    ...
    private List<Integer> getUseCasesPriorityOrder(List<UseCaseConfig<?>> list) {
        ...
        ArrayList arrayList2 = new ArrayList();
        Iterator<UseCaseConfig<?>> it2 = list.iterator();
        while (it2.hasNext()) {
    ...
    private void generateSurfaceSizeDefinition() {
        this.mSurfaceSizeDefinition = SurfaceSizeDefinition.create(new Size(OlympusMakernoteDirectory.TAG_PREVIEW_IMAGE, StreamingImageHandler.TFT_HEIGHT), this.mDisplayInfoManager.getPreviewSize(), getRecordSize());
    }
