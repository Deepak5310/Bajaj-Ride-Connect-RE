...
public class MediaRouteDynamicControllerDialog extends AppCompatDialog {
    ...
    public MediaRouteDynamicControllerDialog(Context context, int i) {
        ...
        this.mHandler = new Handler() { // from class: androidx.mediarouter.app.MediaRouteDynamicControllerDialog.1
            ...
            public void handleMessage(Message message) {
                ...
                } else if (i2 == 2 && MediaRouteDynamicControllerDialog.this.mRouteForVolumeUpdatingByUser != null) {
                    MediaRouteDynamicControllerDialog.this.mRouteForVolumeUpdatingByUser = null;
    ...
    private final class RecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
        ...
        boolean isGroupVolumeNeeded() {
            return MediaRouteDynamicControllerDialog.this.mEnableGroupVolumeUX && MediaRouteDynamicControllerDialog.this.mSelectedRoute.getMemberRoutes().size() > 1;
        }
        ...
        void mayUpdateGroupVolume(MediaRouter.RouteInfo routeInfo, boolean z) {
            ...
            if (routeInfo.isGroup()) {
                Iterator<MediaRouter.RouteInfo> it2 = routeInfo.getMemberRoutes().iterator();
                while (it2.hasNext()) {
                    if (memberRoutes.contains(it2.next()) != z) {
                        iMax += z ? 1 : -1;
                    }
                }
            } else {
            ...
            boolean z2 = MediaRouteDynamicControllerDialog.this.mEnableGroupVolumeUX && iMax >= 2;
            if (zIsGroupVolumeNeeded != z2) {
                RecyclerView.ViewHolder viewHolderFindViewHolderForAdapterPosition = MediaRouteDynamicControllerDialog.this.mRecyclerView.findViewHolderForAdapterPosition(0);
        ...
        void updateItems() {
            ...
            if (!MediaRouteDynamicControllerDialog.this.mMemberRoutes.isEmpty()) {
                Iterator<MediaRouter.RouteInfo> it2 = MediaRouteDynamicControllerDialog.this.mMemberRoutes.iterator();
                while (it2.hasNext()) {
                    this.mItems.add(new Item(it2.next(), 3));
                }
            } else {
            ...
            if (!MediaRouteDynamicControllerDialog.this.mTransferableRoutes.isEmpty()) {
                for (MediaRouter.RouteInfo routeInfo2 : MediaRouteDynamicControllerDialog.this.mTransferableRoutes) {
                    if (MediaRouteDynamicControllerDialog.this.mSelectedRoute != routeInfo2) {
                        if (!z) {
