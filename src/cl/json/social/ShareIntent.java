...
public abstract class ShareIntent {
        ...
        if (!listQueryIntentActivities.isEmpty()) {
            for (ResolveInfo resolveInfo : listQueryIntentActivities) {
                if (resolveInfo.activityInfo != null && !readableMap.getArray("excludedActivityTypes").toString().contains(resolveInfo.activityInfo.packageName)) {
                    ...
                    map.put(RemoteConfigConstants.RequestFieldKey.PACKAGE_NAME, resolveInfo.activityInfo.packageName);
            ...
            if (!arrayList2.isEmpty()) {
                ...
                for (HashMap map2 : arrayList2) {
                    ...
                    intent3.setPackage((String) map2.get(RemoteConfigConstants.RequestFieldKey.PACKAGE_NAME));
                    intent3.setClassName((String) map2.get(RemoteConfigConstants.RequestFieldKey.PACKAGE_NAME), (String) map2.get("className"));
                    arrayList.add(intent3);
                }
                Intent intentCreateChooser = Intent.createChooser((Intent) arrayList.remove(arrayList.size() - 1), FirebaseAnalytics.Event.SHARE);
    ...
    public void open(ReadableMap readableMap) throws ActivityNotFoundException {
        ...
        String string = hasValidKey(StackTraceHelper.MESSAGE_KEY, readableMap) ? readableMap.getString(StackTraceHelper.MESSAGE_KEY) : "";
        ...
        if (string2.equals("sms")) {
            ...
            if (!string3.isEmpty()) {
                getIntent().putExtra(PlaceTypes.ADDRESS, string3);
            }
        }
        ...
        if (hasValidKey("urls", readableMap)) {
            ...
            getIntent().putExtra("android.intent.extra.TEXT", string + StringUtils.SPACE + readableMap.getArray("urls").getString(0));
            return;
        }
        if (hasValidKey(ImagesContract.URL, readableMap)) {
            ...
            getIntent().putExtra("android.intent.extra.TEXT", string + StringUtils.SPACE + readableMap.getString(ImagesContract.URL));
            return;
        }
        if (TextUtils.isEmpty(string)) {
            return;
    ...
    protected void openIntentChooser() throws ActivityNotFoundException {
        ...
        if (sharingSenderIntent == null) {
            ...
            writableMapCreateMap.putString(StackTraceHelper.MESSAGE_KEY, "OK");
