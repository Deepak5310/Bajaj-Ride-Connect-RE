    ...
    public static IntentSender getSharingSenderIntent(ReactContext reactContext) {
        synchronized (LOCK) {
            if (sTargetChosenReceiveAction == null) {
                sTargetChosenReceiveAction = reactContext.getPackageName() + RemoteSettings.FORWARD_SLASH_STRING + TargetChosenReceiver.class.getName() + "_ACTION";
            }
        ...
        synchronized (LOCK) {
            ...
            if (intent.hasExtra(EXTRA_RECEIVER_TOKEN) && intent.getIntExtra(EXTRA_RECEIVER_TOKEN, 0) == hashCode()) {
                ...
                if (componentName != null) {
                    writableMapCreateMap.putString(StackTraceHelper.MESSAGE_KEY, componentName.flattenToString());
                } else {
