...
class OkHttpClientTransport implements ConnectionClientTransport, ExceptionHandlingFrameWriter.TransportExceptionHandler, OutboundFlowController.Transport {
    ...
    private OkHttpClientTransport(OkHttpChannelBuilder.OkHttpTransportFactory okHttpTransportFactory, InetSocketAddress inetSocketAddress, String str, @Nullable String str2, Attributes attributes, Supplier<Stopwatch> supplier, Variant variant, @Nullable HttpConnectProxiedSocketAddress httpConnectProxiedSocketAddress, Runnable runnable) {
        ...
        this.address = (InetSocketAddress) Preconditions.checkNotNull(inetSocketAddress, PlaceTypes.ADDRESS);
            ...
            public void run() {
                ...
                try {
                    try {
                        ...
                        okHttpClientTransport4.clientFrameHandler = okHttpClientTransport4.new ClientFrameHandler(okHttpClientTransport4.variant.newReader(bufferedSourceBuffer2, true));
                        synchronized (OkHttpClientTransport.this.lock) {
                        ...
                        okHttpClientTransport5.clientFrameHandler = okHttpClientTransport5.new ClientFrameHandler(okHttpClientTransport5.variant.newReader(bufferedSourceBuffer, true));
                        throw th;
                    }
    ...
    private Request createHttpProxyRequest(InetSocketAddress inetSocketAddress, String str, String str2) {
        HttpUrl httpUrlBuild = new HttpUrl.Builder().scheme(UriUtil.HTTPS_SCHEME).host(inetSocketAddress.getHostName()).port(inetSocketAddress.getPort()).build();
    ...
    public String toString() {
        return MoreObjects.toStringHelper(this).add("logId", this.logId.getId()).add(PlaceTypes.ADDRESS, this.address).toString();
    }
    ...
    public OutboundFlowController.StreamState[] getActiveStreams() {
        ...
        synchronized (this.lock) {
            ...
            Iterator<OkHttpClientStream> it2 = this.streams.values().iterator();
            int i = 0;
            while (it2.hasNext()) {
                streamStateArr[i] = it2.next().transportState().getOutboundFlowState();
                i++;
            }
        }
