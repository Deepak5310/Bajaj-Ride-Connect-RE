...
public final class ObservableFromIterable<T> extends Observable<T> {
    ...
    static final class FromIterableDisposable<T> extends BasicQueueDisposable<T> {
        ...
        final Iterator<? extends T> f737it;
        ...
        void run() {
            while (!isDisposed()) {
                try {
                    this.downstream.onNext(ObjectHelper.requireNonNull(this.f737it.next(), "The iterator returned a null value"));
                    if (isDisposed()) {
                        return;
                    }
                    try {
                        if (!this.f737it.hasNext()) {
                            if (isDisposed()) {
                                return;
                            }
        ...
        public T poll() {
            ...
            return (T) ObjectHelper.requireNonNull(this.f737it.next(), "The iterator returned a null value");
        }
