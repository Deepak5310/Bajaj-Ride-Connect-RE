...
public class UniversalFirebaseConfigModule extends UniversalFirebaseModule {
    ...
    Task<Void> setDefaultsFromResource(String str, final String str2) {
        final FirebaseApp firebaseApp = FirebaseApp.getInstance(str);
        return Tasks.call(getExecutor(), new Callable() { // from class: io.invertase.firebase.config.UniversalFirebaseConfigModule$$ExternalSyntheticLambda3
            @Override // java.util.concurrent.Callable
            public final Object call() {
    ...
    Task<Void> setCustomSignals(String str, final HashMap<String, Object> map) {
        final TaskCompletionSource taskCompletionSource = new TaskCompletionSource();
        final FirebaseApp firebaseApp = FirebaseApp.getInstance(str);
        getExecutor().execute(new Runnable() { // from class: io.invertase.firebase.config.UniversalFirebaseConfigModule$$ExternalSyntheticLambda2
            @Override // java.lang.Runnable
            public final void run() {
                UniversalFirebaseConfigModule.lambda$setCustomSignals$3(map, firebaseApp, taskCompletionSource);
            }
        });
        return taskCompletionSource.getTask();
    }

    static /* synthetic */ void lambda$setCustomSignals$3(HashMap map, FirebaseApp firebaseApp, TaskCompletionSource taskCompletionSource) {
        try {
