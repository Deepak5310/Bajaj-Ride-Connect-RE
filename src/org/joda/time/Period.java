...
public final class Period extends BasePeriod implements ReadablePeriod, Serializable {
    ...
    public Days toStandardDays() {
        ...
        return Days.days(FieldUtils.safeToInt(FieldUtils.safeAdd(FieldUtils.safeAdd((((((long) getMillis()) + (((long) getSeconds()) * 1000)) + (((long) getMinutes()) * 60000)) + (((long) getHours()) * DateUtils.MILLIS_PER_HOUR)) / DateUtils.MILLIS_PER_DAY, getDays()), ((long) getWeeks()) * 7)));
    }

    public Hours toStandardHours() {
        checkYearsAndMonths("Hours");
        return Hours.hours(FieldUtils.safeToInt(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd(((((long) getMillis()) + (((long) getSeconds()) * 1000)) + (((long) getMinutes()) * 60000)) / DateUtils.MILLIS_PER_HOUR, getHours()), ((long) getDays()) * 24), ((long) getWeeks()) * 168)));
    }
    ...
    public Period normalizedStandard(PeriodType periodType) {
        PeriodType periodType2 = DateTimeUtils.getPeriodType(periodType);
        Period period = new Period(((long) getMillis()) + (((long) getSeconds()) * 1000) + (((long) getMinutes()) * 60000) + (((long) getHours()) * DateUtils.MILLIS_PER_HOUR) + (((long) getDays()) * DateUtils.MILLIS_PER_DAY) + (((long) getWeeks()) * 604800000), periodType2, ISOChronology.getInstanceUTC());
        int years = getYears();
