...
public class PdfView extends PDFView implements OnPageChangeListener, OnLoadCompleteListener, OnErrorListener, OnTapListener, OnDrawListener, OnPageScrollListener, LinkHandler {
    ...
    public void onPageChanged(int i, int i2) {
        ...
        writableMapCreateMap.putString(StackTraceHelper.MESSAGE_KEY, "pageChanged|" + i3 + "|" + i2);
    ...
    public void loadComplete(int i) {
        ...
        zoomTo(this.scale);
        WritableMap writableMapCreateMap = Arguments.createMap();
        writableMapCreateMap.putString(StackTraceHelper.MESSAGE_KEY, "loadComplete|" + i + "|" + width + "|" + height + "|" + new Gson().toJson(getTableOfContents()));
        ((RCTEventEmitter) ((ReactContext) getContext()).getJSModule(RCTEventEmitter.class)).receiveEvent(getId(), "topChange", writableMapCreateMap);
    }
    ...
    public void onError(Throwable th) {
        ...
        if (th.getMessage().contains("Password required or incorrect password")) {
            writableMapCreateMap.putString(StackTraceHelper.MESSAGE_KEY, "error|Password required or incorrect password.");
        } else {
            writableMapCreateMap.putString(StackTraceHelper.MESSAGE_KEY, "error|" + th.getMessage());
        }
        ((RCTEventEmitter) ((ReactContext) getContext()).getJSModule(RCTEventEmitter.class)).receiveEvent(getId(), "topChange", writableMapCreateMap);
    }
    ...
    public boolean onTap(MotionEvent motionEvent) {
        WritableMap writableMapCreateMap = Arguments.createMap();
        writableMapCreateMap.putString(StackTraceHelper.MESSAGE_KEY, "pageSingleTap|" + this.page + "|" + motionEvent.getX() + "|" + motionEvent.getY());
        ((RCTEventEmitter) ((ReactContext) getContext()).getJSModule(RCTEventEmitter.class)).receiveEvent(getId(), "topChange", writableMapCreateMap);
        return true;
    }
    ...
    public void onLayerDrawn(Canvas canvas, float f, float f2, int i) {
        ...
        if (f3 > 0.0f) {
            ...
            if (f4 > 0.0f && (f != f3 || f2 != f4)) {
                ...
                Constants.Pinch.MAXIMUM_ZOOM = this.maxScale;
                WritableMap writableMapCreateMap = Arguments.createMap();
                writableMapCreateMap.putString(StackTraceHelper.MESSAGE_KEY, "scaleChanged|" + (f / this.originalWidth));
                ((RCTEventEmitter) ((ReactContext) getContext()).getJSModule(RCTEventEmitter.class)).receiveEvent(getId(), "topChange", writableMapCreateMap);
            }
        }
    ...
    private void handleUri(String str) {
        WritableMap writableMapCreateMap = Arguments.createMap();
        writableMapCreateMap.putString(StackTraceHelper.MESSAGE_KEY, "linkPressed|" + str);
        ((RCTEventEmitter) ((ReactContext) getContext()).getJSModule(RCTEventEmitter.class)).receiveEvent(getId(), "topChange", writableMapCreateMap);
    }
