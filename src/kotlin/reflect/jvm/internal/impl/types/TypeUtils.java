...
public class TypeUtils {
    ...
    public static List<TypeProjection> getDefaultTypeProjections(List<TypeParameterDescriptor> list) {
        ...
        while (it2.hasNext()) {
            arrayList.add(new TypeProjectionImpl(it2.next().getDefaultType()));
        }
    ...
    public static List<KotlinType> getImmediateSupertypes(KotlinType kotlinType) {
        ...
        Collection<KotlinType> collectionMo3640getSupertypes = kotlinType.getConstructor().mo3640getSupertypes();
        ArrayList arrayList = new ArrayList(collectionMo3640getSupertypes.size());
        Iterator<KotlinType> it2 = collectionMo3640getSupertypes.iterator();
        while (it2.hasNext()) {
            KotlinType kotlinTypeCreateSubstitutedSupertype = createSubstitutedSupertype(kotlinType, it2.next(), typeSubstitutorCreate);
    ...
    public static boolean isNullableType(KotlinType kotlinType) {
        ...
        if (constructor instanceof IntersectionTypeConstructor) {
            Iterator<KotlinType> it2 = constructor.mo3640getSupertypes().iterator();
            while (it2.hasNext()) {
    ...
    public static boolean hasNullableSuperType(KotlinType kotlinType) {
        ...
        if (kotlinType.getConstructor().mo3639getDeclarationDescriptor() instanceof ClassDescriptor) {
            return false;
        }
        Iterator<KotlinType> it2 = getImmediateSupertypes(kotlinType).iterator();
        while (it2.hasNext()) {
            if (isNullableType(it2.next())) {
                return true;
            }
    ...
    private static boolean contains(KotlinType kotlinType, Function1<UnwrappedType, Boolean> function1, SmartSet<KotlinType> smartSet) {
        ...
        TypeConstructor constructor = kotlinType.getConstructor();
        if (constructor instanceof IntersectionTypeConstructor) {
            Iterator<KotlinType> it2 = ((IntersectionTypeConstructor) constructor).mo3640getSupertypes().iterator();
            while (it2.hasNext()) {
                if (contains(it2.next(), function1, smartSet)) {
                    return true;
                }
            }
            return false;
    ...
    public static TypeParameterDescriptor getTypeParameterDescriptorOrNull(KotlinType kotlinType) {
        ...
        if (kotlinType.getConstructor().mo3639getDeclarationDescriptor() instanceof TypeParameterDescriptor) {
            return (TypeParameterDescriptor) kotlinType.getConstructor().mo3639getDeclarationDescriptor();
        }
        return null;
    }
}
