        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(simpleTypeMarkerPop)) {
                ...
                if (none == null) {
                    ...
                    while (it2.hasNext()) {
                        SimpleTypeMarker simpleTypeMarkerMo3643transformType = none.mo3643transformType(typeCheckerState, it2.next());
                        if ((typeSystemContext.isClassType(simpleTypeMarkerMo3643transformType) && !typeSystemContext.isMarkedNullable(simpleTypeMarkerMo3643transformType)) || typeSystemContext.isDefinitelyNotNullType(simpleTypeMarkerMo3643transformType)) {
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(simpleTypeMarkerPop)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    Iterator<KotlinTypeMarker> it2 = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(simpleTypeMarkerPop)).iterator();
                    while (it2.hasNext()) {
                        SimpleTypeMarker simpleTypeMarkerMo3643transformType = supertypesPolicy.mo3643transformType(state, it2.next());
                        if (INSTANCE.isApplicableAsEndNode(state, simpleTypeMarkerMo3643transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(simpleTypeMarkerMo3643transformType);
