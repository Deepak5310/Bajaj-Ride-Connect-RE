...
public class DescriptorUtils {
    ...
    private static /* synthetic */ void $$$reportNull$$$0(int i) {
        ...
        switch (i) {
            ...
            case KodakMakernoteDirectory.TAG_FLASH_MODE /* 92 */:
                objArr[0] = AccountsQueryParameters.SCOPE;
                break;
            case 88:
            case 91:
            case 93:
    ...
    public static ModuleDescriptor getContainingModuleOrNull(KotlinType kotlinType) {
        ...
        return getContainingModuleOrNull(classifierDescriptorMo3639getDeclarationDescriptor);
    }
    ...
    public static boolean isDirectSubclass(ClassDescriptor classDescriptor, ClassDescriptor classDescriptor2) {
        ...
        Iterator<KotlinType> it2 = classDescriptor.getTypeConstructor().mo3640getSupertypes().iterator();
        while (it2.hasNext()) {
            if (isSameClass(it2.next(), classDescriptor2.getOriginal())) {
                return true;
            }
        }
        return false;
    ...
    private static boolean isSameClass(KotlinType kotlinType, DeclarationDescriptor declarationDescriptor) {
        ...
        DeclarationDescriptor original = classifierDescriptorMo3639getDeclarationDescriptor.getOriginal();
    ...
    public static boolean isSubtypeOfClass(KotlinType kotlinType, DeclarationDescriptor declarationDescriptor) {
        ...
        Iterator<KotlinType> it2 = kotlinType.getConstructor().mo3640getSupertypes().iterator();
        while (it2.hasNext()) {
            if (isSubtypeOfClass(it2.next(), declarationDescriptor)) {
                return true;
            }
        }
        return false;
    ...
    public static ClassDescriptor getSuperClassDescriptor(ClassDescriptor classDescriptor) {
        ...
        Iterator<KotlinType> it2 = classDescriptor.getTypeConstructor().mo3640getSupertypes().iterator();
        while (it2.hasNext()) {
            ClassDescriptor classDescriptorForType = getClassDescriptorForType(it2.next());
    ...
    public static ClassDescriptor getClassDescriptorForTypeConstructor(TypeConstructor typeConstructor) {
        ...
        ClassDescriptor classDescriptor = (ClassDescriptor) typeConstructor.mo3639getDeclarationDescriptor();
        if (classDescriptor == null) {
            $$$reportNull$$$0(47);
        }
    ...
    private static <D extends CallableDescriptor> void collectAllOverriddenDescriptors(D d, Set<D> set) {
        ...
        Iterator<? extends CallableDescriptor> it2 = d.getOriginal().getOverriddenDescriptors().iterator();
        while (it2.hasNext()) {
            CallableDescriptor original = it2.next().getOriginal();
