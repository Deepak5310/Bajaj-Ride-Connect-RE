...
public class ReactImageView extends GenericDraweeView {
    ...
    private class TilePostprocessor extends BasePostprocessor {
        ...
        public CloseableReference<Bitmap> process(Bitmap bitmap, PlatformBitmapFactory platformBitmapFactory) {
            ...
            try {
                ...
                return closeableReferenceCreateBitmap.mo996clone();
            } finally {
    ...
    public void setResizeMultiplier(float f) {
        if (Math.abs(this.mResizeMultiplier - f) > 1.0E-4f) {
            this.mResizeMultiplier = f;
            this.mIsDirty = true;
        }
    }
    ...
    public void setSource(ReadableArray readableArray) {
        ...
        while (it2.hasNext()) {
            this.mSources.add((ImageSource) it2.next());
        }
        this.mIsDirty = true;
    }
            ...
            if (!hasMultipleSources() || (getWidth() > 0 && getHeight() > 0)) {
                ...
                if (!zShouldResize || (getWidth() > 0 && getHeight() > 0)) {
                    if (!isTiled() || (getWidth() > 0 && getHeight() > 0)) {
                        ...
                        if (i2 < 0) {
                            i2 = this.mImageSource.get_isResource() ? 0 : 300;
                        }
                        ...
                        ResizeOptions resizeOptions = zShouldResize ? getResizeOptions() : null;
    ...
    private ResizeOptions getResizeOptions() {
        int iRound = Math.round(getWidth() * this.mResizeMultiplier);
        int iRound2 = Math.round(getHeight() * this.mResizeMultiplier);
