...
public class SurfaceMountingManager {
    ...
    public void createViewUnsafe(String str, int i, ReadableMap readableMap, StateWrapper stateWrapper, EventEmitterWrapper eventEmitterWrapper, boolean z) {
        ...
        if (z) {
            ...
            viewCreateView = viewManager.createView(i, this.mThemedReactContext, reactStylesDiffMap, stateWrapper, this.mJSResponderHandler);
        } else {
            viewManager = null;
            viewCreateView = null;
        }
        ViewState viewState = new ViewState(i, viewCreateView, viewManager);
    ...
    public void updateEventEmitter(int i, EventEmitterWrapper eventEmitterWrapper) {
        ...
        if (queue != null) {
            Iterator<PendingViewEvent> it2 = queue.iterator();
            while (it2.hasNext()) {
                it2.next().dispatch(eventEmitterWrapper);
            }
            ...
            while (!SurfaceMountingManager.this.mReactTagsToRemove.empty()) {
                try {
                    ...
                    if (SurfaceMountingManager.this.mErroneouslyReaddedReactTags.contains(num)) {
                        ReactSoftExceptionLogger.logSoftException(SurfaceMountingManager.TAG, new IllegalViewOperationException("RemoveDeleteTree recursively tried to remove a React View that was actually reused. This indicates a bug in the Differ. [" + iIntValue + "]"));
                    } else {
                        stack.clear();
