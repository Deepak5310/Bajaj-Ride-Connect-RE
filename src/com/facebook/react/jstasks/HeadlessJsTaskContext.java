...
public class HeadlessJsTaskContext {
    ...
    public synchronized void addTaskEventListener(HeadlessJsTaskEventListener headlessJsTaskEventListener) {
        ...
        Iterator<Integer> it2 = this.mActiveTasks.iterator();
        while (it2.hasNext()) {
            headlessJsTaskEventListener.onHeadlessJsTaskStart(it2.next().intValue());
        }
    }
    ...
    public synchronized void startTask(HeadlessJsTaskConfig headlessJsTaskConfig, int i) {
        ...
        Iterator<HeadlessJsTaskEventListener> it2 = this.mHeadlessJsTaskEventListeners.iterator();
        while (it2.hasNext()) {
    ...
    /* JADX INFO: renamed from: finishTask, reason: merged with bridge method [inline-methods] */
    public synchronized void lambda$scheduleTaskTimeout$0(final int i) {
        ...
        if (zRemove) {
            UiThreadUtil.runOnUiThread(new Runnable() { // from class: com.facebook.react.jstasks.HeadlessJsTaskContext.2
                @Override // java.lang.Runnable
                public void run() {
                    Iterator it2 = HeadlessJsTaskContext.this.mHeadlessJsTaskEventListeners.iterator();
                    while (it2.hasNext()) {
