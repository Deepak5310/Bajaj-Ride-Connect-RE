...
public final class ImageLoaderModule extends NativeImageLoaderAndroidSpec implements LifecycleEventListener {
    ...
    public void getSizeWithHeaders(String uriString, ReadableMap headers, final Promise promise) {
        ...
        ReactNetworkImageRequest reactNetworkImageRequestFromBuilderWithHeaders = ReactNetworkImageRequest.fromBuilderWithHeaders(imageRequestBuilderNewBuilderWithSource, headers);
        Intrinsics.checkNotNullExpressionValue(reactNetworkImageRequestFromBuilderWithHeaders, "fromBuilderWithHeaders(...)");
        getImagePipeline().fetchDecodedImage(reactNetworkImageRequestFromBuilderWithHeaders, getCallerContext()).subscribe(new BaseDataSubscriber<CloseableReference<CloseableImage>>() { // from class: com.facebook.react.modules.image.ImageLoaderModule$getSizeWithHeaders$dataSubscriber$1
            @Override // com.facebook.datasource.BaseDataSubscriber
            protected void onNewResultImpl(DataSource<CloseableReference<CloseableImage>> dataSource) {
                Intrinsics.checkNotNullParameter(dataSource, "dataSource");
                if (dataSource.isFinished()) {
