...
public class DevServerHelper {
    ...
    private String createBundleURL(String str, BundleType bundleType, String str2, boolean z, boolean z2) {
        ...
        sb.append(String.format(Locale.US, "http://%s/%s.%s?platform=android&dev=%s&lazy=%s&minify=%s&app=%s&modulesOnly=%s&runModule=%s", str2, str, bundleType.typeID(), Boolean.valueOf(devMode), Boolean.valueOf(devMode), Boolean.valueOf(getJSMinifyMode()), this.mPackageName, z ? "true" : "false", z2 ? "true" : "false"));
    ...
    public File downloadBundleResourceFromUrlSync(String str, File file) {
        try {
            Response responseExecute = FirebasePerfOkHttpClient.execute(this.mClient.newCall(new Request.Builder().url(createResourceURL(this.mPackagerConnectionSettings.getDebugServerHost(), str)).build()));
            try {
    ...
    public void openDebugger(final ReactContext reactContext, final String str) {
        FirebasePerfOkHttpClient.enqueue(this.mClient.newCall(new Request.Builder().url(String.format(Locale.US, "http://%s/open-debugger?appId=%s&device=%s", this.mPackagerConnectionSettings.getDebugServerHost(), Uri.encode(this.mPackageName), Uri.encode(getInspectorDeviceId()))).method("POST", RequestBody.create((MediaType) null, "")).build()), new Callback() { // from class: com.facebook.react.devsupport.DevServerHelper.6
            @Override // okhttp3.Callback
            public void onResponse(Call call, Response response) {
            }
