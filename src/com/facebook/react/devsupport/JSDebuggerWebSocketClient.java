...
public class JSDebuggerWebSocketClient extends WebSocketListener {
    ...
    public void loadBundle(String str, HashMap<String, String> map, JSDebuggerCallback jSDebuggerCallback) {
        ...
        try {
            StringWriter stringWriter = new StringWriter();
            JsonWriter jsonWriterBeginObject = new JsonWriter(stringWriter).beginObject().name("id").value(andIncrement).name(FirebaseAnalytics.Param.METHOD).value("executeApplicationScript").name(ImagesContract.URL).value(str).name("inject").beginObject();
    ...
    void abort(String str, Throwable th) {
        ...
        Iterator<JSDebuggerCallback> it2 = this.mCallbacks.values().iterator();
        while (it2.hasNext()) {
