    ...
    protected void applyUpdatesRecursive(ReactShadowNode reactShadowNode, float f, float f2, List<ReactShadowNode> list) {
        if (reactShadowNode.hasUpdates()) {
            ...
            if (iterableCalculateLayoutOnChildren != null) {
                Iterator<? extends ReactShadowNode> it2 = iterableCalculateLayoutOnChildren.iterator();
                while (it2.hasNext()) {
                    applyUpdatesRecursive(it2.next(), reactShadowNode.getLayoutX() + f, reactShadowNode.getLayoutY() + f2, list);
                }
            }
