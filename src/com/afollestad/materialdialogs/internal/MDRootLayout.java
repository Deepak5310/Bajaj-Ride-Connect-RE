    ...
    public void onMeasure(int i, int i2) {
        ...
        if (z) {
            if (this.isStacked) {
                ...
                int i6 = this.buttonPaddingFull;
                i3 = i6 * 2;
                i4 = i6 * 2;
            } else {
                measuredHeight2 = size2 - this.buttonBarHeight;
                i3 = this.buttonPaddingFull * 2;
            }
            if (isVisible(this.titleBar)) {
                this.titleBar.measure(View.MeasureSpec.makeMeasureSpec(size, 1073741824), 0);
                measuredHeight2 -= this.titleBar.getMeasuredHeight();
            } else if (!this.noTitleNoPadding) {
                i3 += this.noTitlePaddingFull;
            }
            if (isVisible(this.content)) {
                this.content.measure(View.MeasureSpec.makeMeasureSpec(size, 1073741824), View.MeasureSpec.makeMeasureSpec(measuredHeight2 - i4, Integer.MIN_VALUE));
                if (this.content.getMeasuredHeight() <= measuredHeight2 - i3) {
                    if (this.reducePaddingNoTitleNoButtons || isVisible(this.titleBar) || z) {
                        this.useFullPadding = true;
                        measuredHeight3 = measuredHeight2 - (this.content.getMeasuredHeight() + i3);
                    } else {
                        this.useFullPadding = false;
                        measuredHeight3 = measuredHeight2 - (this.content.getMeasuredHeight() + i4);
                    }
                } else {
                    this.useFullPadding = false;
                }
        ...
        if (isVisible(this.content)) {
            this.content.measure(View.MeasureSpec.makeMeasureSpec(size, 1073741824), View.MeasureSpec.makeMeasureSpec(measuredHeight2 - i4, Integer.MIN_VALUE));
            if (this.content.getMeasuredHeight() <= measuredHeight2 - i3) {
                if (this.reducePaddingNoTitleNoButtons) {
                    this.useFullPadding = true;
                    measuredHeight3 = measuredHeight2 - (this.content.getMeasuredHeight() + i3);
                } else {
                    this.useFullPadding = true;
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (view != null) {
            if (this.drawTopDivider) {
                int top = view.getTop();
                canvas.drawRect(0.0f, top - this.dividerWidth, getMeasuredWidth(), top, this.dividerPaint);
            }
        ...
        if (viewGroup instanceof RecyclerView) {
            RecyclerView.OnScrollListener onScrollListener = new RecyclerView.OnScrollListener() { // from class: com.afollestad.materialdialogs.internal.MDRootLayout.2
                ...
                public void onScrolled(RecyclerView recyclerView, int i, int i2) {
                    ...
                    boolean z3 = false;
                    for (MDButton mDButton : MDRootLayout.this.buttons) {
