...
public final class TurfMeasurement {
    ...
    public static double length(MultiLineString multiLineString, String str) {
        Iterator<List<Point>> it2 = multiLineString.coordinates().iterator();
        double length = 0.0d;
        while (it2.hasNext()) {
    ...
    public static double length(MultiPolygon multiPolygon, String str) {
        Iterator<List<List<Point>>> it2 = multiPolygon.coordinates().iterator();
        double length = 0.0d;
        while (it2.hasNext()) {
            Iterator<List<Point>> it3 = it2.next().iterator();
            while (it3.hasNext()) {
                length += length(it3.next(), str);
            }
        }
        return length;
    }
