...
public class SphericalMercator {
    ...
    public LatLng getLatLng(PointF pointF, int i) {
        double dDoubleValue = (((double) pointF.y) - cache.get("zc").get(i).doubleValue()) / (-cache.get("Cc").get(i).doubleValue());
        return new LatLng(((Math.atan(Math.exp(dDoubleValue)) * 2.0d) - 1.5707963267948966d) * 57.29577951308232d, (((double) pointF.x) - cache.get("zc").get(i).doubleValue()) / cache.get("Bc").get(i).doubleValue());
    }
}
