...
public class SunriseSunset {
    ...
    public SunriseSunset(double d, double d2, Date date, TimeZone timeZone) {
        double offset = (((double) timeZone.getOffset(date.getTime())) * 1.0d) / 3600000.0d;
        this.dfLat = d;
        this.dfLon = d2;
        this.dateInput = date;
        this.dfTimeZone = offset;
        doCalculations();
    }

    /* JADX WARN: Code duplicated, block: B:35:0x0329  */
    private void doCalculations() {
        ...
        this.dfJ = (((double) this.iYear) * 367.0d) + Math.floor((((double) this.iMonth) * 275.0d) / 9.0d) + (-Math.floor(((Math.floor((((double) this.iMonth) + 9.0d) / 12.0d) + ((double) this.iYear)) * 7.0d) / 4.0d)) + ((double) this.iDay) + 1721027.0d;
        double d2 = 1.0d;
        double d3 = 2.0d;
        if (this.bGregorian) {
            double d4 = ((double) this.iMonth) - 9.0d;
            if (d4 < 0.0d) {
                this.iSign = -1;
            } else {
                this.iSign = 1;
            ...
            this.dfA = dAbs;
            double d5 = -Math.floor((Math.floor(Math.floor((Math.floor(dAbs / 7.0d) * ((double) this.iSign)) + ((double) this.iYear)) / 100.0d) + 1.0d) * 0.75d);
            this.dfJ3 = d5;
            this.dfJ = this.dfJ + d5 + 2.0d;
        }
        int i = ((int) this.dfJ) - 1;
        this.iJulian = i;
        double d6 = (((double) i) - 2451545.0d) + 0.5d;
        this.dfT = d6;
        ...
        for (int i2 = 1; this.iCount <= i2; i2 = 1) {
            ...
            double dFloor = (d8 - Math.floor(d8)) * 2.0d * 3.141592653589793d;
            double d9 = (this.dfT * 0.0027377785d) + 0.993126d;
            double dFloor2 = (d9 - Math.floor(d9)) * 2.0d * 3.141592653589793d;
            double dSin = ((Math.sin(dFloor + dFloor2) * 0.00333d) + ((Math.sin(dFloor) * 0.39785d) - (Math.sin(dFloor - dFloor2) * 0.01d))) - ((Math.sin(dFloor) * 2.1E-4d) * this.dfTT);
            double d10 = dFloor * 2.0d;
            double dCos = (Math.cos(dFloor) * 8.0E-5d) + ((1.0d - (Math.cos(dFloor2) * 0.03349d)) - (Math.cos(d10) * 1.4E-4d));
            double dSin2 = (((((Math.sin(dFloor2) * 0.03211d) + ((-1.0E-4d) - (Math.sin(d10) * 0.04129d))) - (Math.sin(d10 - dFloor2) * 0.00104d)) - (Math.sin(d10 + dFloor2) * 3.5E-4d)) - ((Math.sin(dFloor2) * 8.0E-5d) * this.dfTT)) / Math.sqrt(dCos - (dSin * dSin));
            this.dfA5 = Math.atan(dSin2 / Math.sqrt(1.0d - (dSin2 * dSin2))) + dFloor;
            double dSqrt = dSin / Math.sqrt(dCos);
            double dAtan = Math.atan(dSqrt / Math.sqrt(1.0d - (dSqrt * dSqrt)));
            this.dfD5 = dAtan;
            int i3 = this.iCount;
        ...
        while (true) {
            ...
            double dCos2 = ((Math.cos(this.dfH2) * (Math.cos(this.dfD2) * this.dfCosLat)) + (Math.sin(this.dfD2) * this.dfSinLat)) - Math.cos(this.dfZenith);
            this.dfV2 = dCos2;
            double d26 = this.dfV0;
            if ((d26 < 0.0d || dCos2 < 0.0d) && (d26 >= 0.0d || dCos2 >= 0.0d)) {
                double dCos3 = ((Math.cos(this.dfH1) * (Math.cos(this.dfD1) * this.dfCosLat)) + (Math.sin(this.dfD1) * this.dfSinLat)) - Math.cos(this.dfZenith);
                this.dfV1 = dCos3;
                double d27 = this.dfV2;
                double d28 = dCos3 * 4.0d;
                ...
                if (d32 < 0.0d) {
                    ...
                    if (this.bSunrise) {
                        ...
                        int i5 = (int) d36;
                        this.dfHourRise = i5;
                        this.dfMinRise = (int) ((d36 - ((double) i5)) * 60.0d);
                    }
                    if (this.bSunset) {
                        double d37 = this.dfC0 + d35 + 0.008333333333333333d;
                        int i6 = (int) d37;
                        this.dfHourSet = i6;
                        this.dfMinSet = (int) ((d37 - ((double) i6)) * 60.0d);
                    }
                }
            } else {
                d = 1.0d;
    ...
    public boolean isDaytime() {
        ...
        if (!z || !this.bSunsetToday) {
            if (this.bSunUpAllDay) {
                ...
                if (z) {
                    zBefore = !this.dateInput.before(this.dateSunrise);
                } else if (this.bSunsetToday) {
                    zBefore = this.dateInput.before(this.dateSunset);
                }
                this.bDaytime = zBefore;
            }
            return this.bDaytime;
        }
        if (!this.dateSunrise.before(this.dateSunset) ? this.dateInput.after(this.dateSunrise) || this.dateInput.equals(this.dateSunrise) || this.dateInput.before(this.dateSunset) : (this.dateInput.after(this.dateSunrise) || this.dateInput.equals(this.dateSunrise)) && this.dateInput.before(this.dateSunset)) {
            z2 = true;
        }
        this.bDaytime = z2;
        return this.bDaytime;
