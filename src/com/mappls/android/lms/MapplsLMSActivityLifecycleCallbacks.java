        ...
        Runnable runnable2 = new Runnable() { // from class: com.mappls.android.lms.MapplsLMSActivityLifecycleCallbacks.1
            ...
            public void run() {
                if (MapplsLMSActivityLifecycleCallbacks.this.mIsForeground && MapplsLMSActivityLifecycleCallbacks.this.mPaused) {
                    ...
                    try {
                        double dCurrentTimeMillis = System.currentTimeMillis() - MapplsLMSActivityLifecycleCallbacks.sStartSessionTime.doubleValue();
                        if (dCurrentTimeMillis >= MapplsLMSActivityLifecycleCallbacks.this.mConfig.getMinimumSessionDuration() && dCurrentTimeMillis < MapplsLMSActivityLifecycleCallbacks.this.mConfig.getSessionTimeoutDuration() && MapplsLMSActivityLifecycleCallbacks.this.mMpInstance.getTrackAutomaticEvents().booleanValue()) {
                            ...
                            MapplsLMSActivityLifecycleCallbacks.this.mMpInstance.getPeople().increment(AutomaticEvents.TOTAL_SESSIONS_LENGTH, dRound);
