...
public class IccReader implements JpegSegmentMetadataReader, MetadataReader {
    ...
    public void extract(RandomAccessReader randomAccessReader, Metadata metadata, Directory directory) {
        ...
        try {
            ...
            int int33 = randomAccessReader.getInt32(128);
            iccDirectory.setInt(128, int33);
            for (int i = 0; i < int33; i++) {
                int i2 = i * 12;
                iccDirectory.setByteArray(randomAccessReader.getInt32(i2 + NikonType2MakernoteDirectory.TAG_LENS), randomAccessReader.getBytes(randomAccessReader.getInt32(i2 + 136), randomAccessReader.getInt32(i2 + 140)));
            }
        } catch (IOException e) {
    ...
    private void setDate(IccDirectory iccDirectory, int i, RandomAccessReader randomAccessReader) throws IOException {
        ...
        int uInt17 = randomAccessReader.getUInt16(i + 2);
        int uInt18 = randomAccessReader.getUInt16(i + 4);
        int uInt19 = randomAccessReader.getUInt16(i + 6);
        int uInt110 = randomAccessReader.getUInt16(i + 8);
        int uInt111 = randomAccessReader.getUInt16(i + 10);
        if (DateUtil.isValidDate(uInt16, uInt17 - 1, uInt18) && DateUtil.isValidTime(uInt19, uInt110, uInt111)) {
            iccDirectory.setString(i, String.format("%04d:%02d:%02d %02d:%02d:%02d", Integer.valueOf(uInt16), Integer.valueOf(uInt17), Integer.valueOf(uInt18), Integer.valueOf(uInt19), Integer.valueOf(uInt110), Integer.valueOf(uInt111)));
        } else {
            iccDirectory.addError(String.format("ICC data describes an invalid date/time: year=%d month=%d day=%d hour=%d minute=%d second=%d", Integer.valueOf(uInt16), Integer.valueOf(uInt17), Integer.valueOf(uInt18), Integer.valueOf(uInt19), Integer.valueOf(uInt110), Integer.valueOf(uInt111)));
        }
    }
