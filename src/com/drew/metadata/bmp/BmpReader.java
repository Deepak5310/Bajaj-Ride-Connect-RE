    ...
    protected void readBitmapHeader(SequentialReader sequentialReader, BmpHeaderDirectory bmpHeaderDirectory, Metadata metadata) {
        try {
            ...
            if (int32 != 16 && int32 != 64) {
                ...
                long uInt33 = sequentialReader.getUInt32();
                int int33 = sequentialReader.getInt32();
                long j = position + uInt33;
                if (sequentialReader.getPosition() > j) {
                    ...
                    if (uInt32 == BmpHeaderDirectory.ColorSpaceType.PROFILE_LINKED.getValue()) {
                        bmpHeaderDirectory.setString(21, sequentialReader.getNullTerminatedString(int33, Charsets.WINDOWS_1252));
                        return;
                    } else {
                        new IccReader().extract(new ByteArrayReader(sequentialReader.getBytes(int33)), metadata, bmpHeaderDirectory);
                        return;
                    }
                }
            }
