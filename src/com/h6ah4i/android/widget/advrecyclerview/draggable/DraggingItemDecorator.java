...
class DraggingItemDecorator extends BaseDraggableItemDecorator {
    ...
    public void onDrawOver(Canvas canvas, RecyclerView recyclerView, RecyclerView.State state) {
        ...
        int iMin = (int) Math.min(System.currentTimeMillis() - this.mStartMillis, this.mStartAnimationDurationMillis);
        long j = this.mStartAnimationDurationMillis;
        float f = j > 0 ? iMin / j : 1.0f;
    ...
    private void updateTranslationOffset() {
        ...
        if (recyclerView.getChildCount() > 0) {
            ...
            if (!this.mIsScrolling) {
                int iFindFirstVisibleItemPosition = CustomRecyclerViewUtils.findFirstVisibleItemPosition(recyclerView, true);
                int iFindLastVisibleItemPosition = CustomRecyclerViewUtils.findLastVisibleItemPosition(recyclerView, true);
                View viewFindRangeFirstItem = findRangeFirstItem(recyclerView, this.mRange, iFindFirstVisibleItemPosition, iFindLastVisibleItemPosition);
                View viewFindRangeLastItem = findRangeLastItem(recyclerView, this.mRange, iFindFirstVisibleItemPosition, iFindLastVisibleItemPosition);
                int i2 = this.mLayoutOrientation;
                if (i2 == 0) {
                    if (viewFindRangeFirstItem != null) {
                        this.mTranslationLeftLimit = Math.min(this.mTranslationLeftLimit, viewFindRangeFirstItem.getLeft());
                    }
                    if (viewFindRangeLastItem != null) {
                        this.mTranslationRightLimit = Math.min(this.mTranslationRightLimit, Math.max(0, viewFindRangeLastItem.getRight() - this.mDraggingItemInfo.width));
                    }
                } else if (i2 == 1) {
                    if (viewFindRangeFirstItem != null) {
                        this.mTranslationTopLimit = Math.min(this.mTranslationBottomLimit, viewFindRangeFirstItem.getTop());
                    }
                    if (viewFindRangeLastItem != null) {
                        this.mTranslationBottomLimit = Math.min(this.mTranslationBottomLimit, Math.max(0, viewFindRangeLastItem.getBottom() - this.mDraggingItemInfo.height));
                    }
                }
            }
        } else {
