...
class SwapTargetItemOperator extends BaseDraggableItemDecorator {
    ...
    public void setSwapTargetItem(RecyclerView.ViewHolder viewHolder) {
        ...
        if (viewHolder2 != null) {
            ViewPropertyAnimatorCompat viewPropertyAnimatorCompatAnimate = ViewCompat.animate(viewHolder2.itemView);
            viewPropertyAnimatorCompatAnimate.cancel();
            viewPropertyAnimatorCompatAnimate.setDuration(10L).translationX(0.0f).translationY(0.0f).setListener(RESET_TRANSLATION_LISTENER).start();
        }
    ...
    public void onDraw(Canvas canvas, RecyclerView recyclerView, RecyclerView.State state) {
        ...
        float fCalculateTranslationPhase = calculateTranslationPhase(viewHolder, viewHolder2);
        this.mReqTranslationPhase = fCalculateTranslationPhase;
        if (this.mSwapTargetItemChanged) {
            this.mSwapTargetItemChanged = false;
            this.mCurTranslationPhase = fCalculateTranslationPhase;
        } else {
            this.mCurTranslationPhase = calculateCurrentTranslationPhase(this.mCurTranslationPhase, fCalculateTranslationPhase);
        }
