...
class RemovingItemDecorator extends RecyclerView.ItemDecoration {
    ...
    private void fillSwipingItemBackground(Canvas canvas, Drawable drawable, float f) {
        ...
        int iWidth = (int) ((f2 * rect.width()) + 0.5f);
        int iHeight = (int) ((f * rect.height()) + 0.5f);
        if (iHeight == 0 || iWidth == 0 || drawable == null) {
            return;
        }
        int iSave = canvas.save();
        canvas.clipRect(rect.left + i, rect.top + i2, rect.left + i + iWidth, rect.top + i2 + iHeight);
        canvas.translate((rect.left + i) - ((rect.width() - iWidth) / 2), (rect.top + i2) - ((rect.height() - iHeight) / 2));
