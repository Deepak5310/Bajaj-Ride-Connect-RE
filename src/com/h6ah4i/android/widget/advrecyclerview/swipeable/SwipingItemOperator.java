...
class SwipingItemOperator {
    ...
    private static final Interpolator RUBBER_BAND_INTERPOLATOR = new RubberBandInterpolator(0.15f);
    ...
    public void start() {
        ...
        int iMax = Math.max(0, this.mSwipingItemWidth - i);
        int iMax2 = Math.max(0, this.mSwipingItemHeight - i);
        this.mInitialTranslateAmountX = clip(this.mSwipeManager.getSwipeContainerViewTranslationX(this.mSwipingItem), -iMax, iMax);
        this.mInitialTranslateAmountY = clip(this.mSwipeManager.getSwipeContainerViewTranslationY(this.mSwipingItem), -iMax2, iMax2);
    }
    ...
    public void update(int i, int i2, int i3) {
        ...
        int i5 = z ? i2 + this.mInitialTranslateAmountX : this.mInitialTranslateAmountY + i3;
        int i6 = z ? this.mSwipingItemWidth : this.mSwipingItemHeight;
        float f = z ? this.mInvSwipingItemWidth : this.mInvSwipingItemHeight;
        if (z) {
            i4 = i5 > 0 ? this.mRightSwipeReactionType : this.mLeftSwipeReactionType;
        } else {
            i4 = i5 > 0 ? this.mDownSwipeReactionType : this.mUpSwipeReactionType;
        }
        if (i4 == 1) {
            fSignum = Math.signum(i5) * RUBBER_BAND_INTERPOLATOR.getInterpolation(Math.min(Math.abs(i5), i6) * f);
        } else {
            fSignum = i4 != 2 ? 0.0f : Math.min(Math.max(i5 * f, -1.0f), 1.0f);
        }
        this.mSwipeManager.applySlideItem(this.mSwipingItem, i, this.mPrevTranslateAmount, fSignum, true, this.mSwipeHorizontal, false, true);
        this.mPrevTranslateAmount = fSignum;
    }
