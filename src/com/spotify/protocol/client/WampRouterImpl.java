...
public class WampRouterImpl implements WampClient.Router, WampClient.RequestType {
    ...
    private void routeGoodbye(WampMessage wampMessage) {
        Iterator<WampClient.Receiver> it2 = this.mReceivers.iterator();
        while (it2.hasNext()) {
            it2.next().onGoodbye(wampMessage.getObjectAt(1), wampMessage.getStringAt(2));
        }
    }
    ...
    private void routeResult(WampMessage wampMessage) {
        Iterator<WampClient.Receiver> it2 = this.mReceivers.iterator();
        while (it2.hasNext()) {
            it2.next().onResult(Types.RequestId.from(wampMessage.getIntAt(1)), wampMessage.getObjectAt(2), wampMessage.getObjectAt(3), wampMessage.getObjectAt(4));
        }
    }
    ...
    private void routeWelcome(WampMessage wampMessage) {
        Iterator<WampClient.Receiver> it2 = this.mReceivers.iterator();
        while (it2.hasNext()) {
            it2.next().onWelcome(wampMessage.getIntAt(1), wampMessage.getObjectAt(2));
        }
    }
    ...
    private void routeEvent(WampMessage wampMessage) {
        Iterator<WampClient.Receiver> it2 = this.mReceivers.iterator();
        while (it2.hasNext()) {
            it2.next().onEvent(Types.SubscriptionId.from(wampMessage.getIntAt(1)), wampMessage.getIntAt(2), wampMessage.getObjectAt(5));
        }
    }
    ...
    private void routeSubscribed(WampMessage wampMessage) {
        Iterator<WampClient.Receiver> it2 = this.mReceivers.iterator();
        while (it2.hasNext()) {
            it2.next().onSubscribed(Types.RequestId.from(wampMessage.getIntAt(1)), Types.SubscriptionId.from(wampMessage.getIntAt(2)));
        }
    }
}
