...
public class AuthorizationResponse implements Parcelable {
    ...
    public enum Type {
        ...
        TOKEN(MapplsLMSDbAdapter.KEY_TOKEN),
        ERROR("error"),
        EMPTY("empty"),
    ...
    public static class Builder {
        ...
        Builder setAccessToken(String str) {
        ...
        Builder setError(String str) {
            this.mError = str;
            return this;
        }

        Builder setExpiresIn(int i) {
            this.mExpiresIn = i;
            return this;
        }
        ...
        if (encodedFragment != null && encodedFragment.length() > 0) {
            ...
            for (String str : encodedFragment.split("&")) {
                ...
                if (strArrSplit.length == 2) {
                    if (strArrSplit[0].startsWith(AccountsQueryParameters.ACCESS_TOKEN)) {
                        strDecode = Uri.decode(strArrSplit[1]);
                    }
                    if (strArrSplit[0].startsWith("state")) {
                        strDecode2 = Uri.decode(strArrSplit[1]);
                    }
                    if (strArrSplit[0].startsWith(AccountsQueryParameters.EXPIRES_IN)) {
