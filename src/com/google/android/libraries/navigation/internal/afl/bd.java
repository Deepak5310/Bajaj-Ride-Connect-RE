package com.google.android.libraries.navigation.internal.afl;

import androidx.constraintlayout.core.motion.utils.TypedValues;
import com.bajajconnect.wifi.StreamingImageHandler;
import com.drew.metadata.avi.AviDirectory;
import com.drew.metadata.exif.ExifDirectoryBase;
import com.drew.metadata.exif.makernotes.CanonMakernoteDirectory;
import com.drew.metadata.exif.makernotes.KodakMakernoteDirectory;
import com.drew.metadata.exif.makernotes.NikonType2MakernoteDirectory;
import com.drew.metadata.exif.makernotes.OlympusImageProcessingMakernoteDirectory;
import com.drew.metadata.exif.makernotes.OlympusMakernoteDirectory;
import com.drew.metadata.exif.makernotes.OlympusRawInfoMakernoteDirectory;
import com.drew.metadata.exif.makernotes.PanasonicMakernoteDirectory;
import com.drew.metadata.exif.makernotes.ReconyxHyperFireMakernoteDirectory;
import com.drew.metadata.exif.makernotes.SanyoMakernoteDirectory;
import com.drew.metadata.iptc.IptcDirectory;
import com.drew.metadata.mp4.media.Mp4VideoDirectory;
import com.facebook.imagepipeline.memory.BitmapCounterConfig;
import com.facebook.imagepipeline.transcoder.JpegTranscoderUtils;
import com.facebook.imageutils.JfifUtil;
import com.mappls.sdk.maps.rctmgl.components.styles.sources.RCTMGLShapeSourceManager;
import com.mappls.sdk.navigation.camera.NavigationConstants;
import io.grpc.internal.GrpcUtil;
import okhttp3.internal.ws.WebSocketProtocol;

/* JADX INFO: compiled from: PG */
/* JADX INFO: loaded from: classes7.dex */
public enum bd implements com.google.android.libraries.navigation.internal.ael.bm {
    UNKNOWN_REQUEST_ID(0),
    MAP_TILE_REQUEST(1),
    LOCAL_SEARCH_REQUEST(2),
    ROUTE_REQUEST(3),
    MOST_RECENT_VERSION_REQUEST(4),
    UNUSED_5(5),
    AD_IMPRESSION_REQUEST(6),
    BILLING_POINT_REQUEST(7),
    EXCEPTION_REQUEST(8),
    FILE_DOWNLOAD_REQUEST(9),
    USER_EVENT_REQUEST(10),
    TRAFFIC_TILE_REQUEST(11),
    TEXT_LOG_REQUEST(12),
    HAS_TRAFFIC_REQUEST(13),
    CELLID_1_REQUEST(14),
    COOKIE_REQUEST(15),
    START_SESSION_REQUEST(16),
    MAP_TILE_2_REQUEST(17),
    LOCAL_SEARCH_2_REQUEST(18),
    ROUTE_2_REQUEST(19),
    GPS_REPORT_REQUEST(20),
    OPERATOR_BILLING_EVENT_REQUEST(21),
    CELLID_2_REQUEST(22),
    LOCAL_SEARCH_3_REQUEST(23),
    MOST_RECENT_VERSION_2_REQUEST(24),
    CLIENT_PROPERTIES_REQUEST(25),
    MAP_TILE_3_REQUEST(26),
    CELLID_LOCATION_REQUEST(27),
    DIRECTIONS_REQUEST(28),
    CELLID_3_REQUEST(29),
    LOCAL_SEARCH_4_REQUEST(30),
    CELLID_4_REQUEST(31),
    LOCATION_UPDATE_REQUEST(32),
    SET_HTTP_COOKIE_REQUEST(33),
    CREATE_SHORT_URL(34),
    LAYER_LIST_REQUEST(35),
    LAYER_TILE_REQUEST(36),
    LAYER_ITEM_REQUEST(37),
    USER_PROFILE_UPDATE_REQUEST(38),
    RESOURCE_REQUEST(39),
    STREETVIEW_REQUEST(40),
    LOCATION_REQUEST(41),
    LOCAL_DETAILS_REQUEST(42),
    DIRECTIONS_OPTION_REQUEST(43),
    GET_FRIEND_FINDER_USERS_REQUEST(44),
    STREET_VIEW_REPORT(45),
    LOCAL_SEARCH_5_REQUEST(46),
    UPDATE_FRIEND_STATE_REQUEST(47),
    PROFILE_PHOTO_REQUEST(48),
    STREET_VIEW_TAKE_DOWN_REQUEST(49),
    REVERSE_GEOCODE_REQUEST(50),
    SET_LOCATION_PRIVACY_SETTING_REQUEST(51),
    GET_USER_STATE_REQUEST(52),
    LOCATION_SHIFT_FIX_REQUEST(53),
    UPDATE_LOCATION_ACL(54),
    AVAILABLE_55(55),
    SHARE_WITH_FRIEND_REQUEST(56),
    LAYER_CATEGORY_REQUEST(57),
    GET_INVITE_GROUP_LIST_REQUEST(58),
    GET_INVITE_CONTACT_LIST_REQUEST(59),
    USER_CONTENT_REQUEST(60),
    TEXT_TO_SPEECH_REQUEST(61),
    CLIENT_PROPERTIES_2_REQUEST(62),
    SNAP_TO_PLACE_REQUEST(63),
    ACTIVITY_STREAM_CREATE_ACTIVITY_REQUEST(64),
    ACTIVITY_STREAM_CREATE_COMMENT_REQUEST(65),
    NOTIFICATION_CHECK_REQUEST(66),
    GET_ACTIVITY_REQUEST(67),
    ACTIVITY_STREAM_GET_REQUEST(68),
    AD_EVENT_REPORT_REQUEST(69),
    GET_INVITE_GROUP_LIST_PROTO_REQUEST(70),
    GET_USER_STATE_PROTO_REQUEST(71),
    JOIN_EXPERIMENTS_PROTO_REQUEST(72),
    CLIENT_FEATURE_RESTRICTIONS_REQUEST(73),
    LAYER_EVENT_REPORT_REQUEST(74),
    CLIENT_PARAMETERS_REQUEST(75),
    SUGGEST_REQUEST(76),
    USER_DATA_SYNC_REQUEST(77),
    REPORT_LOCAL_ISSUE_REQUEST(78),
    REPORT_MAP_ISSUE_REQUEST(79),
    URL_REQUEST(80),
    SEARCH_CID_PUBLIC_ACTIVITIES(81),
    SEARCH_VIEWPORT_PUBLIC_ACTIVITIES(82),
    SET_USER_STATE_PROTO_REQUEST(83),
    GET_USER_PROFILE_REQUEST(84),
    GET_PRODUCTION_STREAM_REQUEST(85),
    NOTIFICATIONS_REQUEST(86),
    GET_FRIENDS_LOCATIONS_REQUEST(87),
    GET_FRIENDS_PROFILES_REQUEST(88),
    GET_SUGGESTED_FRIEND_LIST_REQUEST(89),
    DISMISS_SUGGESTED_FRIEND_REQUEST(90),
    SET_NETWORK_INITIATED_PREFERENCE_REQUEST(91),
    UPDATE_ACTIVITY_REQUEST(92),
    SUBMIT_REVIEW_REQUEST(93),
    UPDATE_CHECKIN_PREFERENCE_REQUEST(94),
    GET_CHECKIN_PREFERENCE_REQUEST(95),
    ACCEPT_SUGGESTED_FRIEND_REQUEST(96),
    DELETE_REVIEW_REQUEST(97),
    SUBMIT_REVIEW_2_REQUEST(98),
    LOCATION_POLLING_INTERVALS_REQUEST(99),
    GET_CIRCLES_REQUEST(100),
    CREATE_GOOGLE_PLUS_CHECKIN_REQUEST(101),
    DEPRECATED_SET_CIRCLES_REQUEST(102),
    AD_REQUEST(103),
    GET_LATITUDE_PREFERENCES_REQUEST(104),
    SET_LATITUDE_PREFERENCES_REQUEST(105),
    CREATE_BUZZ_CHECKIN_REQUEST(RCTMGLShapeSourceManager.METHOD_GET_CLUSTER_CHILDREN),
    THIRD_PARTY_OAUTH_REQUEST(107),
    MAP_TILE_4_REQUEST(108),
    PING_ME_REQUEST(109),
    PING_TELL_REQUEST(110),
    GET_LOCATION_HISTORY_SUMMARY_REQUEST(PanasonicMakernoteDirectory.TAG_LANDMARK),
    LEGACY_TRANSIT_STATION_REQUEST(PanasonicMakernoteDirectory.TAG_INTELLIGENT_RESOLUTION),
    DEPRECATED_SET_HOME_LOCATION(113),
    DEPRECATED_GET_HOME_LOCATION(114),
    GET_USER_PHOTO_DETAILS_REQUEST(115),
    CREATE_PLACE(116),
    PHOTO_UPLOAD_REQUEST(117),
    INDOOR_BUILDING_REQUEST(118),
    PLATTERS_REQUEST(PanasonicMakernoteDirectory.TAG_BURST_SPEED),
    DELETE_REVIEW_PHOTO_REQUEST(120),
    BATCH_LOCATION_UPDATE_REQUEST(PanasonicMakernoteDirectory.TAG_INTELLIGENT_D_RANGE),
    GET_CHECKIN_LEADERBOARD_REQUEST(PanasonicMakernoteDirectory.TAG_CLEAR_RETOUCH),
    USER_EVENT_2_REQUEST(NavigationConstants.NAVIGATION_LOW_ALERT_DURATION),
    NEARBY_TRANSIT_STATIONS_REQUEST(WebSocketProtocol.PAYLOAD_SHORT),
    TRANSIT_LINE_REQUEST(127),
    SUGGEST_LOGGING_REQUEST(128),
    ELEVATION_REQUEST(129),
    GET_MOBILE_LOCATION_HISTORY_REQUEST(NikonType2MakernoteDirectory.TAG_ADAPTER),
    API_TOKEN_REQUEST(NikonType2MakernoteDirectory.TAG_LENS),
    GET_PSUGGEST_ACTIVITY_SOURCE_REQUEST(NikonType2MakernoteDirectory.TAG_MANUAL_FOCUS_DISTANCE),
    DELETE_PSUGGEST_ACTIVITY_REQUEST(NikonType2MakernoteDirectory.TAG_DIGITAL_ZOOM),
    CORRECT_PLACE_REQUEST(136),
    GET_VISITED_PLACES_REQUEST(NikonType2MakernoteDirectory.TAG_LENS_STOPS),
    DEPRECATED_TACTILE_SEARCH_REQUEST(140),
    TRAFFIC_INCIDENT_REQUEST(141),
    TACTILE_DIRECTIONS_REQUEST(142),
    START_PAGE_REQUEST(143),
    MY_PROFILE_REQUEST(144),
    USER_EVENT_3_REQUEST(145),
    DEPRECATED_TACTILE_PLACE_REQUEST(146),
    API_QUOTA_EVENT_REQUEST(147),
    LAYER_METADATA_REQUEST(148),
    MAPS_ENGINE_FEATURE_REQUEST(149),
    TACTILE_SEARCH_REQUEST(150),
    TACTILE_PLACE_DETAILS_REQUEST(151),
    TACTILE_STARRING_REQUEST(152),
    TACTILE_REVEAL_REQUEST(153),
    DEPRECATED_TACTILE_IMAGERY_REQUEST(154),
    TACTILE_REVIEWS_REQUEST(NikonType2MakernoteDirectory.TAG_UNKNOWN_10),
    TACTILE_SUGGEST_REQUEST(NikonType2MakernoteDirectory.TAG_SCENE_ASSIST),
    TACTILE_VIEWPORT_METADATA_REQUEST(157),
    TACTILE_PHOTO_REQUEST(158),
    TACTILE_UPDATE_ALIAS_REQUEST(159),
    APP_START_REQUEST(160),
    TACTILE_LOCATION_DETAILS_REQUEST(CanonMakernoteDirectory.TAG_TONE_CURVE_TABLE),
    TACTILE_PLACE_ATTRIBUTE_UPDATE_REQUEST(162),
    DEPRECATED_QUERYLESS_ADS_REQUEST(163),
    CREATE_PLUS_ONE_REQUEST(164),
    DELETE_PLUS_ONE_REQUEST(NikonType2MakernoteDirectory.TAG_IMAGE_COUNT),
    EXTERNAL_INVOCATION_REQUEST(NikonType2MakernoteDirectory.TAG_DELETED_IMAGE_COUNT),
    DIRECTORY_REQUEST(NikonType2MakernoteDirectory.TAG_EXPOSURE_SEQUENCE_NUMBER),
    MAPS_ACTIVITIES_REQUEST(168),
    SUBMIT_OWNER_RESPONSE_REQUEST(169),
    DELETE_OWNER_RESPONSE_REQUEST(170),
    TACTILE_SNAP_TO_PLACE_REQUEST(171),
    TACTILE_REPORT_DATA_PROBLEM_REQUEST(NikonType2MakernoteDirectory.TAG_IMAGE_STABILISATION),
    MAP_PER_TILE_REQUEST(NikonType2MakernoteDirectory.TAG_AF_RESPONSE),
    TACTILE_DISMISS_TODO_REQUEST(174),
    OFFLINE_UPDATE_REQUEST(NikonType2MakernoteDirectory.TAG_UNKNOWN_30),
    DEPRECATED_RESTAURANT_RESERVATION_SEARCH_REQUEST(176),
    DEPRECATED_RESTAURANT_RESERVATION_BOOK_REQUEST(177),
    REPORT_TRACK_REQUEST(178),
    ADD_TO_PREDEFINED_CIRCLE_REQUEST(179),
    REMOVE_FROM_CIRCLES_REQUEST(180),
    PLUS_SETTINGS_MESSAGE_REQUEST(NikonType2MakernoteDirectory.TAG_UNKNOWN_48),
    USER_REVIEWS_REQUEST(182),
    TACTILE_CATEGORIES_REQUEST(NikonType2MakernoteDirectory.TAG_AF_INFO_2),
    REPORT_TRACK_PARAMETERS_REQUEST(NikonType2MakernoteDirectory.TAG_FILE_INFO),
    TACTILE_GEOCODE_REQUEST(NikonType2MakernoteDirectory.TAG_AF_TUNE),
    TACTILE_FACEPILE_DETAILS_REQUEST(186),
    GUNS_REGISTER_DEVICE_REQUEST(NikonType2MakernoteDirectory.TAG_UNKNOWN_49),
    GUNS_UNREGISTER_DEVICE_REQUEST(188),
    MAPS_ENGINE_MAP_REQUEST(NikonType2MakernoteDirectory.TAG_UNKNOWN_50),
    PROFILE_REQUEST(190),
    GUNS_FETCH_NOTIFICATIONS_BY_KEY(191),
    TRANSIT_TRIP_DETAILS_REQUEST(JfifUtil.MARKER_SOFn),
    TACTILE_WRITE_RIDDLER_ANSWER_REQUEST(193),
    GUNS_SET_READ_STATE(194),
    GUNS_MARK_ALL_AS_READ(195),
    GUNS_FETCH_NOTIFICATIONS(196),
    MAPS_ACTIVITIES_TIMELINE_REQUEST(197),
    MAPS_ACTIVITIES_TIMELINE_SEGMENT_REQUEST(222),
    MAPS_ACTIVITIES_HISTORY_EDIT_REQUEST(198),
    MAPS_ACTIVITIES_DELETE_DATA_REQUEST(199),
    TACTILE_DIRECTIONS_ASSIST_REQUEST(200),
    PERSONAL_PLACES_REQUEST(201),
    GUNS_FETCH_NOTIFICATIONS_COUNT_REQUEST(202),
    TACTILE_PERSONAL_INTELLIGENCE_REQUEST(203),
    ON_MAP_ADS_REQUEST(204),
    USER_PREFS_READ_REQUEST(205),
    USER_PREFS_WRITE_REQUEST(206),
    USER_PHOTOS_REQUEST(207),
    USER_PHOTO_EDIT_REQUEST(208),
    TACTILE_USER_TO_USER_BLOCKING_REQUEST(255),
    TRANSIT_STATION_REQUEST(Mp4VideoDirectory.TAG_DEPTH),
    RIDDLER_FOLLOW_ON_REQUEST(Mp4VideoDirectory.TAG_COMPRESSION_TYPE),
    LIST_RIDDLER_QUESTIONS_REQUEST(460),
    LIST_RIDDLER_QUESTIONS_DEIDENTIFIED_REQUEST(487),
    LOCATION_SHARING_ASK_FOR_LOCATION_REQUEST(272),
    LOCATION_SHARING_CREATE_SHARES_REQUEST(282),
    DEPRECATED_LOCATION_SHARING_CREATE_GEOFENCE_ALERT_REQUEST(334),
    DEPRECATED_LOCATION_SHARING_DELETE_GEOFENCE_ALERT_REQUEST(337),
    DEPRECATED_LOCATION_SHARING_MUTE_GEOFENCE_ALERT_REQUEST(IptcDirectory.TAG_TIME_SENT),
    LOCATION_SHARING_RECORD_CONSENT_REQUEST(418),
    DEPRECATED_LOCATION_SHARING_UPDATE_GEOFENCE_ALERT_REQUEST(335),
    LOCATION_SHARING_UPDATE_SHARE_REQUEST(317),
    LOCATION_SHARING_UPLOAD_LOCATION_REQUEST(TypedValues.CycleType.TYPE_CUSTOM_WAVE_SHAPE),
    TACTILE_LOCATION_SHARING_CANCEL_SHARE_REQUEST(250),
    TACTILE_LOCATION_SHARING_CANCEL_SHARED_JOURNEY_REQUEST(ExifDirectoryBase.TAG_NEW_SUBFILE_TYPE),
    TACTILE_LOCATION_SHARING_CREATE_SHARED_JOURNEY_REQUEST(268),
    TACTILE_LOCATION_SHARING_LOCATION_READ_REQUEST(Mp4VideoDirectory.TAG_GRAPHICS_MODE),
    TACTILE_LOCATION_SHARING_UPDATE_SHARED_JOURNEY_REQUEST(253),
    USER_INFO_REQUEST(Mp4VideoDirectory.TAG_OPCOLOR),
    TACTILE_PHOTO_TAKEDOWN_REQUEST(Mp4VideoDirectory.TAG_COLOR_TABLE),
    USER_CONTRIBUTIONS_REQUEST(Mp4VideoDirectory.TAG_FRAME_RATE),
    SUBMIT_RATING_REQUEST(JfifUtil.MARKER_RST7),
    PERSONAL_NOTES_REQUEST(JfifUtil.MARKER_SOI),
    PEOPLE_AUTOCOMPLETE_REQUEST(JfifUtil.MARKER_EOI),
    PEOPLE_GET_REQUEST(227),
    PEOPLE_WRITE_REQUEST(228),
    TODO_LIST_REQUEST(JfifUtil.MARKER_SOS),
    DISMISS_EDIT_STATE_NOTIFICATION_REQUEST(219),
    YOUR_PLACES_LIST_DATA_REQUEST(220),
    PASSIVE_ASSIST_REQUEST(221),
    MAPS_ACTIVITY_SYNC_REQUEST(223),
    ALIAS_STICKER_REQUEST(CanonMakernoteDirectory.TAG_SENSOR_INFO_ARRAY),
    LOCAL_GUIDES_PREFS_WRITE_REQUEST(JfifUtil.MARKER_APP1),
    LOCAL_GUIDES_SIGN_UP_PAGE_REQUEST(226),
    TODO_PHOTO_REQUEST(230),
    NEARBY_TRANSIT_REQUEST(229),
    DISMISS_TODO_PHOTO_REQUEST(231),
    VOTE_ON_PENDING_EDIT_REQUEST(232),
    MAPSHOP_REQUEST(233),
    CREATE_RALLY_POINT_REQUEST(234),
    DELETE_RALLY_POINT_REQUEST(235),
    GET_RALLY_POINT_REQUEST(236),
    GET_RALLY_POINTS_FOR_USER_REQUEST(237),
    UGC_TASK_SETS_REQUEST(238),
    POST_CONTRIBUTION_THANKS_PAGE_REQUEST(239),
    TRAFFIC_NOTIFICATION_REQUEST(240),
    DISMISS_UGC_TASK_REQUEST(241),
    TAXI_BOOK_RIDE_REQUEST(242),
    TAXI_CANCEL_RIDE_REQUEST(243),
    TAXI_GET_AVAILABLE_PRODUCTS_REQUEST(244),
    TAXI_GET_PAYMENT_METHODS_REQUEST(259),
    TAXI_GET_RIDE_ESTIMATE_REQUEST(245),
    TAXI_GET_RIDE_STATUS_REQUEST(246),
    TAXI_GET_TRIP_ESTIMATES_REQUEST(345),
    TAXI_GET_USER_CONSENT_REQUEST(327),
    TAXI_GET_USER_DETAILS_REQUEST(260),
    TAXI_MODIFY_RIDE_REQUEST(252),
    TAXI_UPDATE_SANDBOX_RIDE_STATUS_REQUEST(IptcDirectory.TAG_DATE_SENT),
    TAXI_UPDATE_USER_CONSENT_REQUEST(328),
    USER_FACTUAL_EDITS_REQUEST(247),
    THUMBS_VOTE_REQUEST(248),
    UGC_TASK_ANSWER_REQUEST(249),
    KNOWLEDGE_DETAILS_REQUEST(251),
    MAPS_ACTIVITY_PLACE_LIST_SHARE_REQUEST(256),
    PROMOTED_PIN_ADS_REQUEST(257),
    OFFERING_WRITE_REQUEST(258),
    MAPS_ACTIVITY_PLACE_LIST_FOLLOW_REQUEST(261),
    MAPS_ACTIVITY_PLACE_LIST_GET_REQUEST(262),
    TRAFFIC_TO_PLACE_NOTIFICATION_REQUEST(263),
    AD_BLOCK_REQUEST(264),
    OFFLINE_GET_REGION_SIZE_REQUEST(265),
    OFFERING_DETAILS_REQUEST(266),
    TACTILE_LOCATION_EVENT_BATCH_REQUEST(267),
    DEPRECATED_TUTORIAL_PARAMETERS_REQUEST(269),
    OFFERING_SUGGEST_REQUEST(270),
    ADDRESS_DATA_FEEDBACK_REQUEST(271),
    GET_BUILDING_ENTRANCE_EDITING_DATA_REQUEST(527),
    PLACE_QA_REQUEST(273),
    PLACE_QA_ANSWER_WRITE_REQUEST(275),
    PLACE_QA_QUESTION_WRITE_REQUEST(276),
    SUGGEST_PLACE_QA_TAGGABLE_ENTITIES_REQUEST(452),
    PLACE_QA_USER_CONTRIBUTION_REQUEST(310),
    PLACE_QA_VOTE_REQUEST(AviDirectory.TAG_DATETIME_ORIGINAL),
    PLACE_QA_SET_BEST_ANSWER_REQUEST(371),
    PLACE_QA_REPLIES_REQUEST(391),
    WRITE_PLACE_QA_REPLY_REQUEST(396),
    PLACE_QA_GET_SMART_ANSWERS_REQUEST(393),
    LOCAL_STREAM_REQUEST(274),
    USER_INCIDENT_REPORT_REQUEST(277),
    LOCAL_POSTS_REQUEST(278),
    DEPRECATED_LOCAL_STORY_REQUEST(OlympusRawInfoMakernoteDirectory.TagWbRbLevelsFineWeather),
    LOCAL_STREAM_FOLLOW_REQUEST(279),
    LOCAL_PRODUCTS_INSTORE_SUGGEST(408),
    DEPRECATED_LOCAL_STREAM_ACTION_REQUEST(280),
    EXPLORE_REQUEST(281),
    LOCAL_STREAM_LIST_FOLLOW_ENTITIES_REQUEST(283),
    AREA_TRAFFIC_REQUEST(284),
    REPORT_NAVIGATION_SESSION_EVENTS_REQUEST(285),
    CREATE_UGC_TASK_PLACE_REMINDER_REQUEST(OlympusImageProcessingMakernoteDirectory.TagWbGLevel),
    DELETE_UGC_TASK_PLACE_REMINDER_REQUEST(288),
    RECOMMENDED_PLACES_REQUEST(289),
    DEPRECATED_GET_NAVIGATION_SATELLITE_EPHEMERIS_REQUEST(291),
    GET_UGC_TASK_PLACE_REMINDERS_REQUEST(OlympusRawInfoMakernoteDirectory.TagWbRbLevelsEveningSunlight),
    TRANSACTIONS_GET_USER_STREAM_REQUEST(293),
    DEPRECATED_LIST_SEARCH_CONVERSION_QUESTIONS_REQUEST(294),
    DEPRECATED_CREATE_SEARCH_CONVERSION_ANSWER_REQUEST(295),
    SEND_POST_TRIP_NOTIFICATION_REQUEST(464),
    WRITE_POST_TRIP_ANSWER_REQUEST(296),
    GET_POST_TRIP_QUESTIONS_REQUEST(377),
    CREATE_SHAREABLE_URL_REQUEST(ExifDirectoryBase.TAG_PAGE_NUMBER),
    CREATE_ENTITY_LIST_REQUEST(300),
    DELETE_ENTITY_LIST_REQUEST(301),
    CREATE_ENTITY_LIST_ITEM_REQUEST(302),
    DELETE_ENTITY_LIST_ITEM_REQUEST(303),
    CREATE_ENTITY_LIST_ITEM_CUSTOM_DATA_REQUEST(304),
    DELETE_ENTITY_LIST_ITEM_CUSTOM_DATA_REQUEST(305),
    GET_ENTITY_LIST_PARTICIPANTS_REQUEST(315),
    GET_ENTITY_LIST_RECOMMENDATIONS_REQUEST(386),
    GET_ENTITY_LIST_REQUEST(306),
    LIST_ENTITY_LISTS_REQUEST(309),
    SHARE_ENTITY_LIST_REQUEST(314),
    UPDATE_ENTITY_LIST_REQUEST(308),
    UPDATE_ENTITY_LIST_ITEM_REQUEST(375),
    UPDATE_ENTITY_LIST_ITEM_CUSTOM_DATA_REQUEST(318),
    UPDATE_ENTITY_LIST_ROLE_REQUEST(307),
    UPDATE_ENTITY_LIST_VISIBILITY_REQUEST(313),
    CREATE_ENTITY_LIST_CUSTOM_DATA_REQUEST(355),
    DELETE_ENTITY_LIST_CUSTOM_DATA_REQUEST(IptcDirectory.TAG_UNIQUE_OBJECT_NAME),
    UPDATE_ENTITY_LIST_CUSTOM_DATA_REQUEST(357),
    KNOWLEDGE_ENTITY_EDIT_REQUEST(311),
    KNOWLEDGE_ENTITY_EDIT_SUMMARY_REQUEST(312),
    KNOWLEDGE_ENTITY_FEEDBACK_REQUEST(341),
    LIST_EXPERIENCES_REQUEST(316),
    MARK_PHOTO_AS_RECEIPT_REQUEST(ExifDirectoryBase.TAG_PRIMARY_CHROMATICITIES),
    GET_VISUAL_EXPLORE_PHOTOS_REQUEST(321),
    PHOTO_VOTE_REQUEST(ExifDirectoryBase.TAG_TILE_WIDTH),
    TRANSIT_CREATE_CROWDEDNESS_FEEDBACK_REQUEST(ExifDirectoryBase.TAG_TILE_LENGTH),
    LOCAL_STREAM_FEEDBACK_REQUEST(ExifDirectoryBase.TAG_TILE_OFFSETS),
    LOCAL_STREAM_DELETE_CARD_REQUEST(ExifDirectoryBase.TAG_TILE_BYTE_COUNTS),
    FOLLOW_PLACE_REQUEST(329),
    LOCAL_STREAM_CREATE_MUTED_PLACES_REQUEST(ExifDirectoryBase.TAG_SUB_IFD_OFFSET),
    LOCAL_STREAM_DELETE_MUTED_PLACES_REQUEST(331),
    LOCAL_STREAM_UPDATE_AREAS_REQUEST(332),
    LOCAL_STREAM_DELETE_TRIP_DESTINATIONS_REQUEST(363),
    LOCAL_STREAM_VERIFY_AREA_REQUEST(333),
    LOCAL_STREAM_ENABLE_PLACE_LISTS_REQUEST(338),
    LOCAL_STREAM_DISABLE_PLACE_LISTS_REQUEST(339),
    LIST_EDITABLE_FEATURES_REQUEST(340),
    DEPRECATED_LIST_TODO_BUNDLES_REQUEST(ExifDirectoryBase.TAG_TRANSFER_RANGE),
    PLACE_VISIT_STATS_REQUEST(343),
    ASSOCIATE_PHOTO_REQUEST(344),
    TRANSIT_DESCRIBE_TRANSIT_PATTERN_REQUEST(ExifDirectoryBase.TAG_JPEG_TABLES),
    TRANSIT_MERGE_SEGMENTS_REQUEST(348),
    UPDATE_RECEIPT_USER_CONSENT_REQUEST(349),
    DELETE_RECEIPTS_REQUEST(350),
    GET_RECEIPT_USER_CONSENT_REQUEST(351),
    GET_ENTITY_LIST_HERO_IMAGES_REQUEST(352),
    TRANSIT_LINES_REQUEST(353),
    DONATE_RECEIPT_REQUEST(354),
    GET_ENTITY_LIST_METADATA_REQUEST(358),
    CINEMA_DATA_REQUEST(359),
    CREATE_RECEIPT_REQUEST(360),
    SEND_SHARE_REQUEST(361),
    GET_ROAD_RAP_INFO_REQUEST(362),
    SYNTHESIZE_TEXT_REQUEST(364),
    SEARCH_TRANSIT_STATIONS_REQUEST(365),
    GET_CREATOR_PROFILE_REQUEST(366),
    EDIT_CREATOR_PROFILE_REQUEST(367),
    ALLOW_COMMUTE_NOTIFICATION_SESSION(368),
    CANCEL_COMMUTE_NOTIFICATION_SESSION(369),
    GET_GOOGLE_WALLET_TRANSIT_CARD_REQUEST(370),
    LIST_NOTIFICATIONS_REQUEST(372),
    UPDATE_NOTIFICATIONS_REQUEST(373),
    UPDATE_DISCOVERY_PREFS_REQUEST(374),
    GET_COMMUTE_IMMERSIVE_CONTENT(IptcDirectory.TAG_ARM_IDENTIFIER),
    APPROVE_PEOPLE_FOLLOWER_REQUEST(IptcDirectory.TAG_ARM_VERSION),
    FOLLOW_PEOPLE_REQUEST(379),
    GET_PEOPLE_FOLLOW_CREATOR_RECOMMENDATIONS_REQUEST(380),
    LIST_PEOPLE_FOLLOWS_REQUEST(381),
    REJECT_PEOPLE_FOLLOWER_REQUEST(382),
    REJECT_PEOPLE_FOLLOW_CREATOR_RECOMMENDATION_REQUEST(383),
    UNFOLLOW_PEOPLE_REQUEST(BitmapCounterConfig.DEFAULT_MAX_BITMAP_COUNT),
    GET_PLACE_INSIGHTS_REQUEST(385),
    LIST_EV_CONNECTOR_TYPES_REQUEST(387),
    GET_TRANSIT_POLYLINES_REQUEST(388),
    GET_ROAD_EDITING_TILES_REQUEST(389),
    GET_REVIEWS_REQUEST(390),
    GET_TRANSIT_ATTRIBUTE_QUESTIONS_REQUEST(392),
    GET_EXHAUSTIVE_SEARCH_REQUEST(394),
    REPORT_TRANSIT_ATTRIBUTES_REQUEST(395),
    CREATE_INCOGNITO_COOKIE_REQUEST(397),
    DEPRECATED_GET_AREA_SHOPPING_DATA_REQUEST(398),
    GET_PROXY_PHONE_NUMBER_REQUEST(399),
    CREATE_UGC_POST_REQUEST(400),
    DELETE_UGC_POST_REQUEST(TypedValues.CycleType.TYPE_CURVE_FIT),
    GET_UGC_POST_REQUEST(TypedValues.CycleType.TYPE_VISIBILITY),
    LIST_UGC_POSTS_REQUEST(TypedValues.CycleType.TYPE_ALPHA),
    UPDATE_UGC_POST_REQUEST(404),
    VOTE_UGC_POST_REQUEST(405),
    CREATE_OR_UPDATE_OWNER_RESPONSE_TO_UGC_POST_REQUEST(406),
    DELETE_OWNER_RESPONSE_TO_UGC_POST_REQUEST(407),
    WRITE_OWNER_RESPONSE_TO_REVIEW_REQUEST(409),
    DELETE_OWNER_RESPONSE_TO_REVIEW_REQUEST(410),
    BUSINESS_CATEGORIES_REQUEST(411),
    DELETE_LOCAL_POST_REQUEST(412),
    CREATE_LOCAL_POST_REQUEST(413),
    UPDATE_LOCAL_POST_REQUEST(414),
    GET_BUSINESS_MESSAGING_STATE_REQUEST(415),
    ENABLE_BUSINESS_MESSAGING_REQUEST(TypedValues.CycleType.TYPE_PATH_ROTATE),
    DISABLE_BUSINESS_MESSAGING_REQUEST(417),
    LIST_MERCHANT_RECOMMENDATIONS_REQUEST(419),
    DISMISS_MERCHANT_RECOMMENDATION_REQUEST(TypedValues.CycleType.TYPE_EASING),
    BATCH_GET_MERCHANT_LISTING_METADATA_REQUEST(421),
    GET_CALL_INSIGHTS_REQUEST(TypedValues.CycleType.TYPE_WAVE_PERIOD),
    LIST_CALL_DETAILS_REQUEST(TypedValues.CycleType.TYPE_WAVE_OFFSET),
    UPDATE_LISTING_CALLS_STATUS_REQUEST(TypedValues.CycleType.TYPE_WAVE_PHASE),
    GET_TIMELINE_ENTRY_POINTS_REQUEST(426),
    MULTI_TODO_LIST_REQUEST(427),
    DISMISS_UGC_INFO_CARD_REQUEST(428),
    GET_CONTRIBUTOR_ZONE_CONTENT_REQUEST(429),
    GET_PEOPLE_FOLLOW_INFO_REQUEST(430),
    GET_BATCH_BUSINESS_MESSAGING_STATE_REQUEST(431),
    GET_UGC_POST_EDITOR_INFO_REQUEST(432),
    GET_MESSAGING_AUTO_ENABLE_STATE_REQUEST(433),
    UPDATE_MESSAGING_PREFERENCE_REQUEST(434),
    GET_CALL_TRANSCRIPT_REQUEST(435),
    DELETE_CALL_RECORD_REQUEST(436),
    LIST_MERCHANT_BUSINESS_SUMMARIES_REQUEST(437),
    LIST_EV_PAYMENT_NETWORKS_REQUEST(438),
    LIST_CALL_SURVEYS_REQUEST(439),
    LIST_USER_PUBLIC_ENTITY_LISTS_REQUEST(440),
    GET_MERCHANT_MANAGEMENT_DISABLED_MESSAGE_REQUEST(441),
    GET_UGC_CAMPAIGN_REQUEST(453),
    RECORD_UGC_CAMPAIGN_CONTRIBUTIONS_REQUEST(442),
    GET_PARKING_AVAILABILITY_REQUEST(GrpcUtil.DEFAULT_PORT_SSL),
    LIST_MERCHANT_BUSINESS_ADMINS_REQUEST(444),
    UPDATE_MERCHANT_BUSINESS_ADMIN_ROLE_REQUEST(445),
    CREATE_MERCHANT_USER_BUSINESS_ADMIN_REQUEST(446),
    DELETE_MERCHANT_BUSINESS_ADMIN_REQUEST(447),
    GET_BUSINESS_WELCOME_MESSAGE_REQUEST(448),
    UPDATE_BUSINESS_WELCOME_MESSAGE_REQUEST(449),
    GET_PLACE_PREVIEW_REQUEST(450),
    UPDATE_CUSTOMER_PICKUP_ETA_REQUEST(451),
    GET_NOTIFICATION_TYPE_METADATA_REQUEST(454),
    REDEEM_UGC_CAMPAIGNS_COUPON_REQUEST(455),
    GET_MERCHANT_SETTINGS_REQUEST(456),
    UPDATE_MERCHANT_SETTINGS_REQUEST(457),
    ANSWER_TODO_LIST_QUESTION_TASK_REQUEST(458),
    GET_OFFLINE_TIMELINE_MANIFEST_REQUEST(459),
    TACTILE_GET_SEARCH_RESULT_PREVIEW_REQUEST(461),
    CREATE_UGC_CHALLENGE_USER_PLACE_REQUEST(462),
    GET_UGC_CHALLENGE_INFO_REQUEST(463),
    SUBMIT_GOOGLE_FEEDBACK_REQUEST(465),
    CREATE_COMMENT_REQUEST(466),
    DELETE_COMMENT_REQUEST(467),
    LIST_COMMENTS_BY_PARENT_CONTENT_ID_REQUEST(468),
    LIST_COMMENTS_BY_USER_REQUEST(469),
    LIST_MERCHANT_NOTIFICATION_OVERLAYS_REQUEST(470),
    REPORT_MERCHANT_NOTIFICATION_OVERLAY_REQUEST(471),
    DEDUCT_QUOTA_REQUEST(472),
    UPDATE_BUSINESS_MESSAGING_STATE_REQUEST(473),
    DELETE_SEARCH_HISTORY_SUGGEST_REQUEST(474),
    GET_MERCHANT_BUSINESS_PROFILE_DETAILS_REQUEST(475),
    GET_EXPERIENCES_BY_PLACE_REQUEST(476),
    GET_UGC_ELIGIBILITY_REQUEST(477),
    SEND_NOTIFICATION_TO_DEVICE_REQUEST(478),
    GET_BATCH_TIMELINE_PLACE_INFOS_REQUEST(479),
    GET_CONTRIBUTOR_IDENTITY_REQUEST(StreamingImageHandler.TFT_HEIGHT),
    GET_MERCHANT_STATUS_REQUEST(481),
    TRIGGER_AT_A_PLACE_NOTIFICATION_REQUEST(482),
    GET_EV_TRIP_WAYPOINTS_REQUEST(483),
    WRITE_MULTIPLE_CHOICE_ANSWER_REQUEST(484),
    DELETE_PERSONAL_ACTIVITIES_BY_PLACE_REQUEST(485),
    LIST_PERSONAL_ACTIVITIES_BY_PLACE_REQUEST(486),
    OFFLINE_SNAPTILE_PACKAGE_METADATA_REQUEST(488),
    GET_CONTRIBUTOR_ZONE_CONTRIBUTION_SUGGESTION_BY_GAIA_REQUEST(489),
    GET_CONTRIBUTOR_ZONE_CONTRIBUTION_SUGGESTION_BY_ODLH_REQUEST(490),
    GET_CREATOR_ZONE_CONTRIBUTION_SUGGESTION_BY_GAIA_REQUEST(491),
    GET_CREATOR_ZONE_CONTRIBUTION_SUGGESTION_BY_ODLH_REQUEST(492),
    GET_THANKS_PAGE_CONTRIBUTION_SUGGESTION_BY_GAIA_REQUEST(493),
    GET_THANKS_PAGE_CONTRIBUTION_SUGGESTION_BY_ODLH_REQUEST(494),
    DELETE_UGC_REACITON_REQUEST(495),
    LIST_ATTRIBUTED_REACTIONS_BY_PARENT_CONTENT_ID_REQUEST(496),
    UPDATE_UGC_REACTION_REQUEST(497),
    GET_GEN_AI_SEARCH_OUTPUT_REQUEST(498),
    GET_RIDDLER_FAMILIAR_PLACES_REQUEST(499),
    ENABLE_SMS_NOTIFICATION_REQUEST(500),
    LIST_VEHICLE_MAKES_REQUEST(TypedValues.PositionType.TYPE_TRANSITION_EASING),
    LIST_VEHICLE_MODELS_REQUEST(TypedValues.PositionType.TYPE_DRAWPATH),
    LIST_VEHICLE_BUILDS_REQUEST(TypedValues.PositionType.TYPE_PERCENT_WIDTH),
    LIST_TASK_COMPLETIONS_REQUEST(TypedValues.PositionType.TYPE_PERCENT_HEIGHT),
    GET_MERCHANT_PROFILE_STRENGTH_REQUEST(TypedValues.PositionType.TYPE_SIZE_PERCENT),
    SUGGEST_EDIT_BY_CHAT_REQUEST(TypedValues.PositionType.TYPE_PERCENT_X),
    SUBMIT_GEN_AI_USER_FEEDBACK_REQUEST(TypedValues.PositionType.TYPE_PERCENT_Y),
    GET_GEN_AI_LANDING_PAGE_REQUEST(TypedValues.PositionType.TYPE_CURVE_FIT),
    REPORT_ROUTING_FEEDBACK_REQUEST(509),
    GET_TIMELINE_STATIC_MAP_REQUEST(TypedValues.PositionType.TYPE_POSITION_TYPE),
    UPDATE_MERCHANT_LISTING_REQUEST(511),
    GET_EOM_TREATMENT_REQUEST(512),
    REPORT_MERCHANT_RECOMMENDATION_INTERACTION_REQUEST(513),
    BLOCK_UGC_USER_TO_USER_REQUEST(514),
    EXPLORE_ALONG_ROUTE_REQUEST(515),
    OFFLINE_GET_DATA_VIEW_REQUEST(516),
    LIST_UGC_FLAGS_BY_USER_REQUEST(517),
    GET_INFERRED_EV_METADATA_REQUEST(518),
    LIST_MERCHANT_PRODUCT_ANSWERS_REQUEST(519),
    REPORT_MERCHANT_PRODUCT_QUESTION_REQUEST(520),
    UPDATE_MERCHANT_PRODUCT_ANSWER_REQUEST(521),
    LIST_TICKET_ORDERS_REQUEST(522),
    DECODE_URL(523),
    EDIT_MERCHANT_MEDIA_ITEM_REQUEST(524),
    GET_UGC_POST_EDITOR_FEEDBACK_REQUEST(OlympusMakernoteDirectory.TAG_ORIGINAL_MANUFACTURER_MODEL),
    GET_TRAFFIC_CONDITIONS_REQUEST(SanyoMakernoteDirectory.TAG_SEQUENTIAL_SHOT),
    FULFILL_FEATURE_REQUEST(528),
    EXTRACT_PLACE_LIST_FROM_TEXT_REQUEST(529),
    REPORT_EV_CHARGING_STATION_ETA_REQUEST(530),
    LIST_PROMPT_TEMPLATES_REQUEST(531),
    GET_MULTI_MODAL_SEARCH_LANDING_PAGE_REQUEST(532),
    GET_EXTRACT_PLACE_LIST_STATUS_REQUEST(533),
    FETCH_LLM_RESULT_REQUEST(534),
    GET_MID_TRIP_UGC_IMPACT_REQUEST(SanyoMakernoteDirectory.TAG_RECORD_SHUTTER_RELEASE),
    DELETE_USER_FACTUAL_EDIT_REQUEST(SanyoMakernoteDirectory.TAG_FLICKER_REDUCE),
    SUBMIT_FEEDBACK_ON_INFERRED_PLACES_REQUEST(537),
    LIST_INTERACTED_PLACES_REQUEST(IptcDirectory.TAG_CONTENT_LOCATION_CODE),
    LIST_ROADLER_QUESTIONS_REQUEST(539),
    GET_ALLOWED_EV_INTEGRATIONS_REQUEST(540),
    GET_PLACES_AGGREGATION_FOR_USER_PROTOTYPE_REQUEST(SanyoMakernoteDirectory.TAG_LIGHT_SOURCE_SPECIAL);

    public final int jm;

    bd(int i) {
        this.jm = i;
    }

    public static bd b(int i) {
        switch (i) {
            case 0:
                return UNKNOWN_REQUEST_ID;
            case 1:
                return MAP_TILE_REQUEST;
            case 2:
                return LOCAL_SEARCH_REQUEST;
            case 3:
                return ROUTE_REQUEST;
            case 4:
                return MOST_RECENT_VERSION_REQUEST;
            case 5:
                return UNUSED_5;
            case 6:
                return AD_IMPRESSION_REQUEST;
            case 7:
                return BILLING_POINT_REQUEST;
            case 8:
                return EXCEPTION_REQUEST;
            case 9:
                return FILE_DOWNLOAD_REQUEST;
            case 10:
                return USER_EVENT_REQUEST;
            case 11:
                return TRAFFIC_TILE_REQUEST;
            case 12:
                return TEXT_LOG_REQUEST;
            case 13:
                return HAS_TRAFFIC_REQUEST;
            case 14:
                return CELLID_1_REQUEST;
            case 15:
                return COOKIE_REQUEST;
            case 16:
                return START_SESSION_REQUEST;
            case 17:
                return MAP_TILE_2_REQUEST;
            case 18:
                return LOCAL_SEARCH_2_REQUEST;
            case 19:
                return ROUTE_2_REQUEST;
            case 20:
                return GPS_REPORT_REQUEST;
            case 21:
                return OPERATOR_BILLING_EVENT_REQUEST;
            case 22:
                return CELLID_2_REQUEST;
            case 23:
                return LOCAL_SEARCH_3_REQUEST;
            case 24:
                return MOST_RECENT_VERSION_2_REQUEST;
            case 25:
                return CLIENT_PROPERTIES_REQUEST;
            case 26:
                return MAP_TILE_3_REQUEST;
            case 27:
                return CELLID_LOCATION_REQUEST;
            case 28:
                return DIRECTIONS_REQUEST;
            case 29:
                return CELLID_3_REQUEST;
            case 30:
                return LOCAL_SEARCH_4_REQUEST;
            case 31:
                return CELLID_4_REQUEST;
            case 32:
                return LOCATION_UPDATE_REQUEST;
            case 33:
                return SET_HTTP_COOKIE_REQUEST;
            case 34:
                return CREATE_SHORT_URL;
            case 35:
                return LAYER_LIST_REQUEST;
            case 36:
                return LAYER_TILE_REQUEST;
            case 37:
                return LAYER_ITEM_REQUEST;
            case 38:
                return USER_PROFILE_UPDATE_REQUEST;
            case 39:
                return RESOURCE_REQUEST;
            case 40:
                return STREETVIEW_REQUEST;
            case 41:
                return LOCATION_REQUEST;
            case 42:
                return LOCAL_DETAILS_REQUEST;
            case 43:
                return DIRECTIONS_OPTION_REQUEST;
            case 44:
                return GET_FRIEND_FINDER_USERS_REQUEST;
            case 45:
                return STREET_VIEW_REPORT;
            case 46:
                return LOCAL_SEARCH_5_REQUEST;
            case 47:
                return UPDATE_FRIEND_STATE_REQUEST;
            case 48:
                return PROFILE_PHOTO_REQUEST;
            case 49:
                return STREET_VIEW_TAKE_DOWN_REQUEST;
            case 50:
                return REVERSE_GEOCODE_REQUEST;
            case 51:
                return SET_LOCATION_PRIVACY_SETTING_REQUEST;
            case 52:
                return GET_USER_STATE_REQUEST;
            case 53:
                return LOCATION_SHIFT_FIX_REQUEST;
            case 54:
                return UPDATE_LOCATION_ACL;
            case 55:
                return AVAILABLE_55;
            case 56:
                return SHARE_WITH_FRIEND_REQUEST;
            case 57:
                return LAYER_CATEGORY_REQUEST;
            case 58:
                return GET_INVITE_GROUP_LIST_REQUEST;
            case 59:
                return GET_INVITE_CONTACT_LIST_REQUEST;
            case 60:
                return USER_CONTENT_REQUEST;
            case 61:
                return TEXT_TO_SPEECH_REQUEST;
            case 62:
                return CLIENT_PROPERTIES_2_REQUEST;
            case 63:
                return SNAP_TO_PLACE_REQUEST;
            case 64:
                return ACTIVITY_STREAM_CREATE_ACTIVITY_REQUEST;
            case 65:
                return ACTIVITY_STREAM_CREATE_COMMENT_REQUEST;
            case 66:
                return NOTIFICATION_CHECK_REQUEST;
            case 67:
                return GET_ACTIVITY_REQUEST;
            case 68:
                return ACTIVITY_STREAM_GET_REQUEST;
            case 69:
                return AD_EVENT_REPORT_REQUEST;
            case 70:
                return GET_INVITE_GROUP_LIST_PROTO_REQUEST;
            case PanasonicMakernoteDirectory.TAG_WB_ADJUST_GM /* 71 */:
                return GET_USER_STATE_PROTO_REQUEST;
            case 72:
                return JOIN_EXPERIMENTS_PROTO_REQUEST;
            case 73:
                return CLIENT_FEATURE_RESTRICTIONS_REQUEST;
            case ReconyxHyperFireMakernoteDirectory.TAG_BRIGHTNESS /* 74 */:
                return LAYER_EVENT_REPORT_REQUEST;
            case 75:
                return CLIENT_PARAMETERS_REQUEST;
            case 76:
                return SUGGEST_REQUEST;
            case PanasonicMakernoteDirectory.TAG_AF_POINT_POSITION /* 77 */:
                return USER_DATA_SYNC_REQUEST;
            case 78:
                return REPORT_LOCAL_ISSUE_REQUEST;
            case 79:
                return REPORT_MAP_ISSUE_REQUEST;
            case 80:
                return URL_REQUEST;
            case PanasonicMakernoteDirectory.TAG_LENS_TYPE /* 81 */:
                return SEARCH_CID_PUBLIC_ACTIVITIES;
            case 82:
                return SEARCH_VIEWPORT_PUBLIC_ACTIVITIES;
            case PanasonicMakernoteDirectory.TAG_ACCESSORY_TYPE /* 83 */:
                return SET_USER_STATE_PROTO_REQUEST;
            case 84:
                return GET_USER_PROFILE_REQUEST;
            case JpegTranscoderUtils.DEFAULT_JPEG_QUALITY /* 85 */:
                return GET_PRODUCTION_STREAM_REQUEST;
            case ReconyxHyperFireMakernoteDirectory.TAG_USER_LABEL /* 86 */:
                return NOTIFICATIONS_REQUEST;
            case 87:
                return GET_FRIENDS_LOCATIONS_REQUEST;
            case 88:
                return GET_FRIENDS_PROFILES_REQUEST;
            case PanasonicMakernoteDirectory.TAG_TRANSFORM /* 89 */:
                return GET_SUGGESTED_FRIEND_LIST_REQUEST;
            case 90:
                return DISMISS_SUGGESTED_FRIEND_REQUEST;
            case 91:
                return SET_NETWORK_INITIATED_PREFERENCE_REQUEST;
            case KodakMakernoteDirectory.TAG_FLASH_MODE /* 92 */:
                return UPDATE_ACTIVITY_REQUEST;
            case 93:
                return SUBMIT_REVIEW_REQUEST;
            case KodakMakernoteDirectory.TAG_ISO_SETTING /* 94 */:
                return UPDATE_CHECKIN_PREFERENCE_REQUEST;
            case 95:
                return GET_CHECKIN_PREFERENCE_REQUEST;
            case 96:
                return ACCEPT_SUGGESTED_FRIEND_REQUEST;
            case PanasonicMakernoteDirectory.TAG_FACE_RECOGNITION_INFO /* 97 */:
                return DELETE_REVIEW_REQUEST;
            case 98:
                return SUBMIT_REVIEW_2_REQUEST;
            case 99:
                return LOCATION_POLLING_INTERVALS_REQUEST;
            case 100:
                return GET_CIRCLES_REQUEST;
            case 101:
                return CREATE_GOOGLE_PLUS_CHECKIN_REQUEST;
            case 102:
                return DEPRECATED_SET_CIRCLES_REQUEST;
            case 103:
                return AD_REQUEST;
            case 104:
                return GET_LATITUDE_PREFERENCES_REQUEST;
            case 105:
                return SET_LATITUDE_PREFERENCES_REQUEST;
            case RCTMGLShapeSourceManager.METHOD_GET_CLUSTER_CHILDREN /* 106 */:
                return CREATE_BUZZ_CHECKIN_REQUEST;
            case 107:
                return THIRD_PARTY_OAUTH_REQUEST;
            case 108:
                return MAP_TILE_4_REQUEST;
            case 109:
                return PING_ME_REQUEST;
            case 110:
                return PING_TELL_REQUEST;
            case PanasonicMakernoteDirectory.TAG_LANDMARK /* 111 */:
                return GET_LOCATION_HISTORY_SUMMARY_REQUEST;
            case PanasonicMakernoteDirectory.TAG_INTELLIGENT_RESOLUTION /* 112 */:
                return LEGACY_TRANSIT_STATION_REQUEST;
            case 113:
                return DEPRECATED_SET_HOME_LOCATION;
            case 114:
                return DEPRECATED_GET_HOME_LOCATION;
            case 115:
                return GET_USER_PHOTO_DETAILS_REQUEST;
            case 116:
                return CREATE_PLACE;
            case 117:
                return PHOTO_UPLOAD_REQUEST;
            case 118:
                return INDOOR_BUILDING_REQUEST;
            case PanasonicMakernoteDirectory.TAG_BURST_SPEED /* 119 */:
                return PLATTERS_REQUEST;
            case 120:
                return DELETE_REVIEW_PHOTO_REQUEST;
            case PanasonicMakernoteDirectory.TAG_INTELLIGENT_D_RANGE /* 121 */:
                return BATCH_LOCATION_UPDATE_REQUEST;
            case 122:
            case 123:
            case 131:
            case NikonType2MakernoteDirectory.TAG_FLASH_USED /* 135 */:
            case 137:
            case 138:
            case 286:
            case 298:
            case 299:
            case IptcDirectory.TAG_CODED_CHARACTER_SET /* 346 */:
            default:
                return null;
            case PanasonicMakernoteDirectory.TAG_CLEAR_RETOUCH /* 124 */:
                return GET_CHECKIN_LEADERBOARD_REQUEST;
            case NavigationConstants.NAVIGATION_LOW_ALERT_DURATION /* 125 */:
                return USER_EVENT_2_REQUEST;
            case WebSocketProtocol.PAYLOAD_SHORT /* 126 */:
                return NEARBY_TRANSIT_STATIONS_REQUEST;
            case 127:
                return TRANSIT_LINE_REQUEST;
            case 128:
                return SUGGEST_LOGGING_REQUEST;
            case 129:
                return ELEVATION_REQUEST;
            case NikonType2MakernoteDirectory.TAG_ADAPTER /* 130 */:
                return GET_MOBILE_LOCATION_HISTORY_REQUEST;
            case NikonType2MakernoteDirectory.TAG_LENS /* 132 */:
                return API_TOKEN_REQUEST;
            case NikonType2MakernoteDirectory.TAG_MANUAL_FOCUS_DISTANCE /* 133 */:
                return GET_PSUGGEST_ACTIVITY_SOURCE_REQUEST;
            case NikonType2MakernoteDirectory.TAG_DIGITAL_ZOOM /* 134 */:
                return DELETE_PSUGGEST_ACTIVITY_REQUEST;
            case 136:
                return CORRECT_PLACE_REQUEST;
            case NikonType2MakernoteDirectory.TAG_LENS_STOPS /* 139 */:
                return GET_VISITED_PLACES_REQUEST;
            case 140:
                return DEPRECATED_TACTILE_SEARCH_REQUEST;
            case 141:
                return TRAFFIC_INCIDENT_REQUEST;
            case 142:
                return TACTILE_DIRECTIONS_REQUEST;
            case 143:
                return START_PAGE_REQUEST;
            case 144:
                return MY_PROFILE_REQUEST;
            case 145:
                return USER_EVENT_3_REQUEST;
            case 146:
                return DEPRECATED_TACTILE_PLACE_REQUEST;
            case 147:
                return API_QUOTA_EVENT_REQUEST;
            case 148:
                return LAYER_METADATA_REQUEST;
            case 149:
                return MAPS_ENGINE_FEATURE_REQUEST;
            case 150:
                return TACTILE_SEARCH_REQUEST;
            case 151:
                return TACTILE_PLACE_DETAILS_REQUEST;
            case 152:
                return TACTILE_STARRING_REQUEST;
            case 153:
                return TACTILE_REVEAL_REQUEST;
            case 154:
                return DEPRECATED_TACTILE_IMAGERY_REQUEST;
            case NikonType2MakernoteDirectory.TAG_UNKNOWN_10 /* 155 */:
                return TACTILE_REVIEWS_REQUEST;
            case NikonType2MakernoteDirectory.TAG_SCENE_ASSIST /* 156 */:
                return TACTILE_SUGGEST_REQUEST;
            case 157:
                return TACTILE_VIEWPORT_METADATA_REQUEST;
            case 158:
                return TACTILE_PHOTO_REQUEST;
            case 159:
                return TACTILE_UPDATE_ALIAS_REQUEST;
            case 160:
                return APP_START_REQUEST;
            case CanonMakernoteDirectory.TAG_TONE_CURVE_TABLE /* 161 */:
                return TACTILE_LOCATION_DETAILS_REQUEST;
            case 162:
                return TACTILE_PLACE_ATTRIBUTE_UPDATE_REQUEST;
            case 163:
                return DEPRECATED_QUERYLESS_ADS_REQUEST;
            case 164:
                return CREATE_PLUS_ONE_REQUEST;
            case NikonType2MakernoteDirectory.TAG_IMAGE_COUNT /* 165 */:
                return DELETE_PLUS_ONE_REQUEST;
            case NikonType2MakernoteDirectory.TAG_DELETED_IMAGE_COUNT /* 166 */:
                return EXTERNAL_INVOCATION_REQUEST;
            case NikonType2MakernoteDirectory.TAG_EXPOSURE_SEQUENCE_NUMBER /* 167 */:
                return DIRECTORY_REQUEST;
            case 168:
                return MAPS_ACTIVITIES_REQUEST;
            case 169:
                return SUBMIT_OWNER_RESPONSE_REQUEST;
            case 170:
                return DELETE_OWNER_RESPONSE_REQUEST;
            case 171:
                return TACTILE_SNAP_TO_PLACE_REQUEST;
            case NikonType2MakernoteDirectory.TAG_IMAGE_STABILISATION /* 172 */:
                return TACTILE_REPORT_DATA_PROBLEM_REQUEST;
            case NikonType2MakernoteDirectory.TAG_AF_RESPONSE /* 173 */:
                return MAP_PER_TILE_REQUEST;
            case 174:
                return TACTILE_DISMISS_TODO_REQUEST;
            case NikonType2MakernoteDirectory.TAG_UNKNOWN_30 /* 175 */:
                return OFFLINE_UPDATE_REQUEST;
            case 176:
                return DEPRECATED_RESTAURANT_RESERVATION_SEARCH_REQUEST;
            case 177:
                return DEPRECATED_RESTAURANT_RESERVATION_BOOK_REQUEST;
            case 178:
                return REPORT_TRACK_REQUEST;
            case 179:
                return ADD_TO_PREDEFINED_CIRCLE_REQUEST;
            case 180:
                return REMOVE_FROM_CIRCLES_REQUEST;
            case NikonType2MakernoteDirectory.TAG_UNKNOWN_48 /* 181 */:
                return PLUS_SETTINGS_MESSAGE_REQUEST;
            case 182:
                return USER_REVIEWS_REQUEST;
            case NikonType2MakernoteDirectory.TAG_AF_INFO_2 /* 183 */:
                return TACTILE_CATEGORIES_REQUEST;
            case NikonType2MakernoteDirectory.TAG_FILE_INFO /* 184 */:
                return REPORT_TRACK_PARAMETERS_REQUEST;
            case NikonType2MakernoteDirectory.TAG_AF_TUNE /* 185 */:
                return TACTILE_GEOCODE_REQUEST;
            case 186:
                return TACTILE_FACEPILE_DETAILS_REQUEST;
            case NikonType2MakernoteDirectory.TAG_UNKNOWN_49 /* 187 */:
                return GUNS_REGISTER_DEVICE_REQUEST;
            case 188:
                return GUNS_UNREGISTER_DEVICE_REQUEST;
            case NikonType2MakernoteDirectory.TAG_UNKNOWN_50 /* 189 */:
                return MAPS_ENGINE_MAP_REQUEST;
            case 190:
                return PROFILE_REQUEST;
            case 191:
                return GUNS_FETCH_NOTIFICATIONS_BY_KEY;
            case JfifUtil.MARKER_SOFn /* 192 */:
                return TRANSIT_TRIP_DETAILS_REQUEST;
            case 193:
                return TACTILE_WRITE_RIDDLER_ANSWER_REQUEST;
            case 194:
                return GUNS_SET_READ_STATE;
            case 195:
                return GUNS_MARK_ALL_AS_READ;
            case 196:
                return GUNS_FETCH_NOTIFICATIONS;
            case 197:
                return MAPS_ACTIVITIES_TIMELINE_REQUEST;
            case 198:
                return MAPS_ACTIVITIES_HISTORY_EDIT_REQUEST;
            case 199:
                return MAPS_ACTIVITIES_DELETE_DATA_REQUEST;
            case 200:
                return TACTILE_DIRECTIONS_ASSIST_REQUEST;
            case 201:
                return PERSONAL_PLACES_REQUEST;
            case 202:
                return GUNS_FETCH_NOTIFICATIONS_COUNT_REQUEST;
            case 203:
                return TACTILE_PERSONAL_INTELLIGENCE_REQUEST;
            case 204:
                return ON_MAP_ADS_REQUEST;
            case 205:
                return USER_PREFS_READ_REQUEST;
            case 206:
                return USER_PREFS_WRITE_REQUEST;
            case 207:
                return USER_PHOTOS_REQUEST;
            case 208:
                return USER_PHOTO_EDIT_REQUEST;
            case Mp4VideoDirectory.TAG_DEPTH /* 209 */:
                return TRANSIT_STATION_REQUEST;
            case Mp4VideoDirectory.TAG_COMPRESSION_TYPE /* 210 */:
                return RIDDLER_FOLLOW_ON_REQUEST;
            case Mp4VideoDirectory.TAG_GRAPHICS_MODE /* 211 */:
                return TACTILE_LOCATION_SHARING_LOCATION_READ_REQUEST;
            case Mp4VideoDirectory.TAG_OPCOLOR /* 212 */:
                return USER_INFO_REQUEST;
            case Mp4VideoDirectory.TAG_COLOR_TABLE /* 213 */:
                return TACTILE_PHOTO_TAKEDOWN_REQUEST;
            case Mp4VideoDirectory.TAG_FRAME_RATE /* 214 */:
                return USER_CONTRIBUTIONS_REQUEST;
            case JfifUtil.MARKER_RST7 /* 215 */:
                return SUBMIT_RATING_REQUEST;
            case JfifUtil.MARKER_SOI /* 216 */:
                return PERSONAL_NOTES_REQUEST;
            case JfifUtil.MARKER_EOI /* 217 */:
                return PEOPLE_AUTOCOMPLETE_REQUEST;
            case JfifUtil.MARKER_SOS /* 218 */:
                return TODO_LIST_REQUEST;
            case 219:
                return DISMISS_EDIT_STATE_NOTIFICATION_REQUEST;
            case 220:
                return YOUR_PLACES_LIST_DATA_REQUEST;
            case 221:
                return PASSIVE_ASSIST_REQUEST;
            case 222:
                return MAPS_ACTIVITIES_TIMELINE_SEGMENT_REQUEST;
            case 223:
                return MAPS_ACTIVITY_SYNC_REQUEST;
            case CanonMakernoteDirectory.TAG_SENSOR_INFO_ARRAY /* 224 */:
                return ALIAS_STICKER_REQUEST;
            case JfifUtil.MARKER_APP1 /* 225 */:
                return LOCAL_GUIDES_PREFS_WRITE_REQUEST;
            case 226:
                return LOCAL_GUIDES_SIGN_UP_PAGE_REQUEST;
            case 227:
                return PEOPLE_GET_REQUEST;
            case 228:
                return PEOPLE_WRITE_REQUEST;
            case 229:
                return NEARBY_TRANSIT_REQUEST;
            case 230:
                return TODO_PHOTO_REQUEST;
            case 231:
                return DISMISS_TODO_PHOTO_REQUEST;
            case 232:
                return VOTE_ON_PENDING_EDIT_REQUEST;
            case 233:
                return MAPSHOP_REQUEST;
            case 234:
                return CREATE_RALLY_POINT_REQUEST;
            case 235:
                return DELETE_RALLY_POINT_REQUEST;
            case 236:
                return GET_RALLY_POINT_REQUEST;
            case 237:
                return GET_RALLY_POINTS_FOR_USER_REQUEST;
            case 238:
                return UGC_TASK_SETS_REQUEST;
            case 239:
                return POST_CONTRIBUTION_THANKS_PAGE_REQUEST;
            case 240:
                return TRAFFIC_NOTIFICATION_REQUEST;
            case 241:
                return DISMISS_UGC_TASK_REQUEST;
            case 242:
                return TAXI_BOOK_RIDE_REQUEST;
            case 243:
                return TAXI_CANCEL_RIDE_REQUEST;
            case 244:
                return TAXI_GET_AVAILABLE_PRODUCTS_REQUEST;
            case 245:
                return TAXI_GET_RIDE_ESTIMATE_REQUEST;
            case 246:
                return TAXI_GET_RIDE_STATUS_REQUEST;
            case 247:
                return USER_FACTUAL_EDITS_REQUEST;
            case 248:
                return THUMBS_VOTE_REQUEST;
            case 249:
                return UGC_TASK_ANSWER_REQUEST;
            case 250:
                return TACTILE_LOCATION_SHARING_CANCEL_SHARE_REQUEST;
            case 251:
                return KNOWLEDGE_DETAILS_REQUEST;
            case 252:
                return TAXI_MODIFY_RIDE_REQUEST;
            case 253:
                return TACTILE_LOCATION_SHARING_UPDATE_SHARED_JOURNEY_REQUEST;
            case ExifDirectoryBase.TAG_NEW_SUBFILE_TYPE /* 254 */:
                return TACTILE_LOCATION_SHARING_CANCEL_SHARED_JOURNEY_REQUEST;
            case 255:
                return TACTILE_USER_TO_USER_BLOCKING_REQUEST;
            case 256:
                return MAPS_ACTIVITY_PLACE_LIST_SHARE_REQUEST;
            case 257:
                return PROMOTED_PIN_ADS_REQUEST;
            case 258:
                return OFFERING_WRITE_REQUEST;
            case 259:
                return TAXI_GET_PAYMENT_METHODS_REQUEST;
            case 260:
                return TAXI_GET_USER_DETAILS_REQUEST;
            case 261:
                return MAPS_ACTIVITY_PLACE_LIST_FOLLOW_REQUEST;
            case 262:
                return MAPS_ACTIVITY_PLACE_LIST_GET_REQUEST;
            case 263:
                return TRAFFIC_TO_PLACE_NOTIFICATION_REQUEST;
            case 264:
                return AD_BLOCK_REQUEST;
            case 265:
                return OFFLINE_GET_REGION_SIZE_REQUEST;
            case 266:
                return OFFERING_DETAILS_REQUEST;
            case 267:
                return TACTILE_LOCATION_EVENT_BATCH_REQUEST;
            case 268:
                return TACTILE_LOCATION_SHARING_CREATE_SHARED_JOURNEY_REQUEST;
            case 269:
                return DEPRECATED_TUTORIAL_PARAMETERS_REQUEST;
            case 270:
                return OFFERING_SUGGEST_REQUEST;
            case 271:
                return ADDRESS_DATA_FEEDBACK_REQUEST;
            case 272:
                return LOCATION_SHARING_ASK_FOR_LOCATION_REQUEST;
            case 273:
                return PLACE_QA_REQUEST;
            case 274:
                return LOCAL_STREAM_REQUEST;
            case 275:
                return PLACE_QA_ANSWER_WRITE_REQUEST;
            case 276:
                return PLACE_QA_QUESTION_WRITE_REQUEST;
            case 277:
                return USER_INCIDENT_REPORT_REQUEST;
            case 278:
                return LOCAL_POSTS_REQUEST;
            case 279:
                return LOCAL_STREAM_FOLLOW_REQUEST;
            case 280:
                return DEPRECATED_LOCAL_STREAM_ACTION_REQUEST;
            case 281:
                return EXPLORE_REQUEST;
            case 282:
                return LOCATION_SHARING_CREATE_SHARES_REQUEST;
            case 283:
                return LOCAL_STREAM_LIST_FOLLOW_ENTITIES_REQUEST;
            case 284:
                return AREA_TRAFFIC_REQUEST;
            case 285:
                return REPORT_NAVIGATION_SESSION_EVENTS_REQUEST;
            case OlympusImageProcessingMakernoteDirectory.TagWbGLevel /* 287 */:
                return CREATE_UGC_TASK_PLACE_REMINDER_REQUEST;
            case 288:
                return DELETE_UGC_TASK_PLACE_REMINDER_REQUEST;
            case 289:
                return RECOMMENDED_PLACES_REQUEST;
            case OlympusRawInfoMakernoteDirectory.TagWbRbLevelsFineWeather /* 290 */:
                return DEPRECATED_LOCAL_STORY_REQUEST;
            case 291:
                return DEPRECATED_GET_NAVIGATION_SATELLITE_EPHEMERIS_REQUEST;
            case OlympusRawInfoMakernoteDirectory.TagWbRbLevelsEveningSunlight /* 292 */:
                return GET_UGC_TASK_PLACE_REMINDERS_REQUEST;
            case 293:
                return TRANSACTIONS_GET_USER_STREAM_REQUEST;
            case 294:
                return DEPRECATED_LIST_SEARCH_CONVERSION_QUESTIONS_REQUEST;
            case 295:
                return DEPRECATED_CREATE_SEARCH_CONVERSION_ANSWER_REQUEST;
            case 296:
                return WRITE_POST_TRIP_ANSWER_REQUEST;
            case ExifDirectoryBase.TAG_PAGE_NUMBER /* 297 */:
                return CREATE_SHAREABLE_URL_REQUEST;
            case 300:
                return CREATE_ENTITY_LIST_REQUEST;
            case 301:
                return DELETE_ENTITY_LIST_REQUEST;
            case 302:
                return CREATE_ENTITY_LIST_ITEM_REQUEST;
            case 303:
                return DELETE_ENTITY_LIST_ITEM_REQUEST;
            case 304:
                return CREATE_ENTITY_LIST_ITEM_CUSTOM_DATA_REQUEST;
            case 305:
                return DELETE_ENTITY_LIST_ITEM_CUSTOM_DATA_REQUEST;
            case 306:
                return GET_ENTITY_LIST_REQUEST;
            case 307:
                return UPDATE_ENTITY_LIST_ROLE_REQUEST;
            case 308:
                return UPDATE_ENTITY_LIST_REQUEST;
            case 309:
                return LIST_ENTITY_LISTS_REQUEST;
            case 310:
                return PLACE_QA_USER_CONTRIBUTION_REQUEST;
            case 311:
                return KNOWLEDGE_ENTITY_EDIT_REQUEST;
            case 312:
                return KNOWLEDGE_ENTITY_EDIT_SUMMARY_REQUEST;
            case 313:
                return UPDATE_ENTITY_LIST_VISIBILITY_REQUEST;
            case 314:
                return SHARE_ENTITY_LIST_REQUEST;
            case 315:
                return GET_ENTITY_LIST_PARTICIPANTS_REQUEST;
            case 316:
                return LIST_EXPERIENCES_REQUEST;
            case 317:
                return LOCATION_SHARING_UPDATE_SHARE_REQUEST;
            case 318:
                return UPDATE_ENTITY_LIST_ITEM_CUSTOM_DATA_REQUEST;
            case ExifDirectoryBase.TAG_PRIMARY_CHROMATICITIES /* 319 */:
                return MARK_PHOTO_AS_RECEIPT_REQUEST;
            case AviDirectory.TAG_DATETIME_ORIGINAL /* 320 */:
                return PLACE_QA_VOTE_REQUEST;
            case 321:
                return GET_VISUAL_EXPLORE_PHOTOS_REQUEST;
            case ExifDirectoryBase.TAG_TILE_WIDTH /* 322 */:
                return PHOTO_VOTE_REQUEST;
            case ExifDirectoryBase.TAG_TILE_LENGTH /* 323 */:
                return TRANSIT_CREATE_CROWDEDNESS_FEEDBACK_REQUEST;
            case ExifDirectoryBase.TAG_TILE_OFFSETS /* 324 */:
                return LOCAL_STREAM_FEEDBACK_REQUEST;
            case ExifDirectoryBase.TAG_TILE_BYTE_COUNTS /* 325 */:
                return LOCAL_STREAM_DELETE_CARD_REQUEST;
            case IptcDirectory.TAG_DATE_SENT /* 326 */:
                return TAXI_UPDATE_SANDBOX_RIDE_STATUS_REQUEST;
            case 327:
                return TAXI_GET_USER_CONSENT_REQUEST;
            case 328:
                return TAXI_UPDATE_USER_CONSENT_REQUEST;
            case 329:
                return FOLLOW_PLACE_REQUEST;
            case ExifDirectoryBase.TAG_SUB_IFD_OFFSET /* 330 */:
                return LOCAL_STREAM_CREATE_MUTED_PLACES_REQUEST;
            case 331:
                return LOCAL_STREAM_DELETE_MUTED_PLACES_REQUEST;
            case 332:
                return LOCAL_STREAM_UPDATE_AREAS_REQUEST;
            case 333:
                return LOCAL_STREAM_VERIFY_AREA_REQUEST;
            case 334:
                return DEPRECATED_LOCATION_SHARING_CREATE_GEOFENCE_ALERT_REQUEST;
            case 335:
                return DEPRECATED_LOCATION_SHARING_UPDATE_GEOFENCE_ALERT_REQUEST;
            case IptcDirectory.TAG_TIME_SENT /* 336 */:
                return DEPRECATED_LOCATION_SHARING_MUTE_GEOFENCE_ALERT_REQUEST;
            case 337:
                return DEPRECATED_LOCATION_SHARING_DELETE_GEOFENCE_ALERT_REQUEST;
            case 338:
                return LOCAL_STREAM_ENABLE_PLACE_LISTS_REQUEST;
            case 339:
                return LOCAL_STREAM_DISABLE_PLACE_LISTS_REQUEST;
            case 340:
                return LIST_EDITABLE_FEATURES_REQUEST;
            case 341:
                return KNOWLEDGE_ENTITY_FEEDBACK_REQUEST;
            case ExifDirectoryBase.TAG_TRANSFER_RANGE /* 342 */:
                return DEPRECATED_LIST_TODO_BUNDLES_REQUEST;
            case 343:
                return PLACE_VISIT_STATS_REQUEST;
            case 344:
                return ASSOCIATE_PHOTO_REQUEST;
            case 345:
                return TAXI_GET_TRIP_ESTIMATES_REQUEST;
            case ExifDirectoryBase.TAG_JPEG_TABLES /* 347 */:
                return TRANSIT_DESCRIBE_TRANSIT_PATTERN_REQUEST;
            case 348:
                return TRANSIT_MERGE_SEGMENTS_REQUEST;
            case 349:
                return UPDATE_RECEIPT_USER_CONSENT_REQUEST;
            case 350:
                return DELETE_RECEIPTS_REQUEST;
            case 351:
                return GET_RECEIPT_USER_CONSENT_REQUEST;
            case 352:
                return GET_ENTITY_LIST_HERO_IMAGES_REQUEST;
            case 353:
                return TRANSIT_LINES_REQUEST;
            case 354:
                return DONATE_RECEIPT_REQUEST;
            case 355:
                return CREATE_ENTITY_LIST_CUSTOM_DATA_REQUEST;
            case IptcDirectory.TAG_UNIQUE_OBJECT_NAME /* 356 */:
                return DELETE_ENTITY_LIST_CUSTOM_DATA_REQUEST;
            case 357:
                return UPDATE_ENTITY_LIST_CUSTOM_DATA_REQUEST;
            case 358:
                return GET_ENTITY_LIST_METADATA_REQUEST;
            case 359:
                return CINEMA_DATA_REQUEST;
            case 360:
                return CREATE_RECEIPT_REQUEST;
            case 361:
                return SEND_SHARE_REQUEST;
            case 362:
                return GET_ROAD_RAP_INFO_REQUEST;
            case 363:
                return LOCAL_STREAM_DELETE_TRIP_DESTINATIONS_REQUEST;
            case 364:
                return SYNTHESIZE_TEXT_REQUEST;
            case 365:
                return SEARCH_TRANSIT_STATIONS_REQUEST;
            case 366:
                return GET_CREATOR_PROFILE_REQUEST;
            case 367:
                return EDIT_CREATOR_PROFILE_REQUEST;
            case 368:
                return ALLOW_COMMUTE_NOTIFICATION_SESSION;
            case 369:
                return CANCEL_COMMUTE_NOTIFICATION_SESSION;
            case 370:
                return GET_GOOGLE_WALLET_TRANSIT_CARD_REQUEST;
            case 371:
                return PLACE_QA_SET_BEST_ANSWER_REQUEST;
            case 372:
                return LIST_NOTIFICATIONS_REQUEST;
            case 373:
                return UPDATE_NOTIFICATIONS_REQUEST;
            case 374:
                return UPDATE_DISCOVERY_PREFS_REQUEST;
            case 375:
                return UPDATE_ENTITY_LIST_ITEM_REQUEST;
            case IptcDirectory.TAG_ARM_IDENTIFIER /* 376 */:
                return GET_COMMUTE_IMMERSIVE_CONTENT;
            case 377:
                return GET_POST_TRIP_QUESTIONS_REQUEST;
            case IptcDirectory.TAG_ARM_VERSION /* 378 */:
                return APPROVE_PEOPLE_FOLLOWER_REQUEST;
            case 379:
                return FOLLOW_PEOPLE_REQUEST;
            case 380:
                return GET_PEOPLE_FOLLOW_CREATOR_RECOMMENDATIONS_REQUEST;
            case 381:
                return LIST_PEOPLE_FOLLOWS_REQUEST;
            case 382:
                return REJECT_PEOPLE_FOLLOWER_REQUEST;
            case 383:
                return REJECT_PEOPLE_FOLLOW_CREATOR_RECOMMENDATION_REQUEST;
            case BitmapCounterConfig.DEFAULT_MAX_BITMAP_COUNT /* 384 */:
                return UNFOLLOW_PEOPLE_REQUEST;
            case 385:
                return GET_PLACE_INSIGHTS_REQUEST;
            case 386:
                return GET_ENTITY_LIST_RECOMMENDATIONS_REQUEST;
            case 387:
                return LIST_EV_CONNECTOR_TYPES_REQUEST;
            case 388:
                return GET_TRANSIT_POLYLINES_REQUEST;
            case 389:
                return GET_ROAD_EDITING_TILES_REQUEST;
            case 390:
                return GET_REVIEWS_REQUEST;
            case 391:
                return PLACE_QA_REPLIES_REQUEST;
            case 392:
                return GET_TRANSIT_ATTRIBUTE_QUESTIONS_REQUEST;
            case 393:
                return PLACE_QA_GET_SMART_ANSWERS_REQUEST;
            case 394:
                return GET_EXHAUSTIVE_SEARCH_REQUEST;
            case 395:
                return REPORT_TRANSIT_ATTRIBUTES_REQUEST;
            case 396:
                return WRITE_PLACE_QA_REPLY_REQUEST;
            case 397:
                return CREATE_INCOGNITO_COOKIE_REQUEST;
            case 398:
                return DEPRECATED_GET_AREA_SHOPPING_DATA_REQUEST;
            case 399:
                return GET_PROXY_PHONE_NUMBER_REQUEST;
            case 400:
                return CREATE_UGC_POST_REQUEST;
            case TypedValues.CycleType.TYPE_CURVE_FIT /* 401 */:
                return DELETE_UGC_POST_REQUEST;
            case TypedValues.CycleType.TYPE_VISIBILITY /* 402 */:
                return GET_UGC_POST_REQUEST;
            case TypedValues.CycleType.TYPE_ALPHA /* 403 */:
                return LIST_UGC_POSTS_REQUEST;
            case 404:
                return UPDATE_UGC_POST_REQUEST;
            case 405:
                return VOTE_UGC_POST_REQUEST;
            case 406:
                return CREATE_OR_UPDATE_OWNER_RESPONSE_TO_UGC_POST_REQUEST;
            case 407:
                return DELETE_OWNER_RESPONSE_TO_UGC_POST_REQUEST;
            case 408:
                return LOCAL_PRODUCTS_INSTORE_SUGGEST;
            case 409:
                return WRITE_OWNER_RESPONSE_TO_REVIEW_REQUEST;
            case 410:
                return DELETE_OWNER_RESPONSE_TO_REVIEW_REQUEST;
            case 411:
                return BUSINESS_CATEGORIES_REQUEST;
            case 412:
                return DELETE_LOCAL_POST_REQUEST;
            case 413:
                return CREATE_LOCAL_POST_REQUEST;
            case 414:
                return UPDATE_LOCAL_POST_REQUEST;
            case 415:
                return GET_BUSINESS_MESSAGING_STATE_REQUEST;
            case TypedValues.CycleType.TYPE_PATH_ROTATE /* 416 */:
                return ENABLE_BUSINESS_MESSAGING_REQUEST;
            case 417:
                return DISABLE_BUSINESS_MESSAGING_REQUEST;
            case 418:
                return LOCATION_SHARING_RECORD_CONSENT_REQUEST;
            case 419:
                return LIST_MERCHANT_RECOMMENDATIONS_REQUEST;
            case TypedValues.CycleType.TYPE_EASING /* 420 */:
                return DISMISS_MERCHANT_RECOMMENDATION_REQUEST;
            case 421:
                return BATCH_GET_MERCHANT_LISTING_METADATA_REQUEST;
            case TypedValues.CycleType.TYPE_CUSTOM_WAVE_SHAPE /* 422 */:
                return LOCATION_SHARING_UPLOAD_LOCATION_REQUEST;
            case TypedValues.CycleType.TYPE_WAVE_PERIOD /* 423 */:
                return GET_CALL_INSIGHTS_REQUEST;
            case TypedValues.CycleType.TYPE_WAVE_OFFSET /* 424 */:
                return LIST_CALL_DETAILS_REQUEST;
            case TypedValues.CycleType.TYPE_WAVE_PHASE /* 425 */:
                return UPDATE_LISTING_CALLS_STATUS_REQUEST;
            case 426:
                return GET_TIMELINE_ENTRY_POINTS_REQUEST;
            case 427:
                return MULTI_TODO_LIST_REQUEST;
            case 428:
                return DISMISS_UGC_INFO_CARD_REQUEST;
            case 429:
                return GET_CONTRIBUTOR_ZONE_CONTENT_REQUEST;
            case 430:
                return GET_PEOPLE_FOLLOW_INFO_REQUEST;
            case 431:
                return GET_BATCH_BUSINESS_MESSAGING_STATE_REQUEST;
            case 432:
                return GET_UGC_POST_EDITOR_INFO_REQUEST;
            case 433:
                return GET_MESSAGING_AUTO_ENABLE_STATE_REQUEST;
            case 434:
                return UPDATE_MESSAGING_PREFERENCE_REQUEST;
            case 435:
                return GET_CALL_TRANSCRIPT_REQUEST;
            case 436:
                return DELETE_CALL_RECORD_REQUEST;
            case 437:
                return LIST_MERCHANT_BUSINESS_SUMMARIES_REQUEST;
            case 438:
                return LIST_EV_PAYMENT_NETWORKS_REQUEST;
            case 439:
                return LIST_CALL_SURVEYS_REQUEST;
            case 440:
                return LIST_USER_PUBLIC_ENTITY_LISTS_REQUEST;
            case 441:
                return GET_MERCHANT_MANAGEMENT_DISABLED_MESSAGE_REQUEST;
            case 442:
                return RECORD_UGC_CAMPAIGN_CONTRIBUTIONS_REQUEST;
            case GrpcUtil.DEFAULT_PORT_SSL /* 443 */:
                return GET_PARKING_AVAILABILITY_REQUEST;
            case 444:
                return LIST_MERCHANT_BUSINESS_ADMINS_REQUEST;
            case 445:
                return UPDATE_MERCHANT_BUSINESS_ADMIN_ROLE_REQUEST;
            case 446:
                return CREATE_MERCHANT_USER_BUSINESS_ADMIN_REQUEST;
            case 447:
                return DELETE_MERCHANT_BUSINESS_ADMIN_REQUEST;
            case 448:
                return GET_BUSINESS_WELCOME_MESSAGE_REQUEST;
            case 449:
                return UPDATE_BUSINESS_WELCOME_MESSAGE_REQUEST;
            case 450:
                return GET_PLACE_PREVIEW_REQUEST;
            case 451:
                return UPDATE_CUSTOMER_PICKUP_ETA_REQUEST;
            case 452:
                return SUGGEST_PLACE_QA_TAGGABLE_ENTITIES_REQUEST;
            case 453:
                return GET_UGC_CAMPAIGN_REQUEST;
            case 454:
                return GET_NOTIFICATION_TYPE_METADATA_REQUEST;
            case 455:
                return REDEEM_UGC_CAMPAIGNS_COUPON_REQUEST;
            case 456:
                return GET_MERCHANT_SETTINGS_REQUEST;
            case 457:
                return UPDATE_MERCHANT_SETTINGS_REQUEST;
            case 458:
                return ANSWER_TODO_LIST_QUESTION_TASK_REQUEST;
            case 459:
                return GET_OFFLINE_TIMELINE_MANIFEST_REQUEST;
            case 460:
                return LIST_RIDDLER_QUESTIONS_REQUEST;
            case 461:
                return TACTILE_GET_SEARCH_RESULT_PREVIEW_REQUEST;
            case 462:
                return CREATE_UGC_CHALLENGE_USER_PLACE_REQUEST;
            case 463:
                return GET_UGC_CHALLENGE_INFO_REQUEST;
            case 464:
                return SEND_POST_TRIP_NOTIFICATION_REQUEST;
            case 465:
                return SUBMIT_GOOGLE_FEEDBACK_REQUEST;
            case 466:
                return CREATE_COMMENT_REQUEST;
            case 467:
                return DELETE_COMMENT_REQUEST;
            case 468:
                return LIST_COMMENTS_BY_PARENT_CONTENT_ID_REQUEST;
            case 469:
                return LIST_COMMENTS_BY_USER_REQUEST;
            case 470:
                return LIST_MERCHANT_NOTIFICATION_OVERLAYS_REQUEST;
            case 471:
                return REPORT_MERCHANT_NOTIFICATION_OVERLAY_REQUEST;
            case 472:
                return DEDUCT_QUOTA_REQUEST;
            case 473:
                return UPDATE_BUSINESS_MESSAGING_STATE_REQUEST;
            case 474:
                return DELETE_SEARCH_HISTORY_SUGGEST_REQUEST;
            case 475:
                return GET_MERCHANT_BUSINESS_PROFILE_DETAILS_REQUEST;
            case 476:
                return GET_EXPERIENCES_BY_PLACE_REQUEST;
            case 477:
                return GET_UGC_ELIGIBILITY_REQUEST;
            case 478:
                return SEND_NOTIFICATION_TO_DEVICE_REQUEST;
            case 479:
                return GET_BATCH_TIMELINE_PLACE_INFOS_REQUEST;
            case StreamingImageHandler.TFT_HEIGHT /* 480 */:
                return GET_CONTRIBUTOR_IDENTITY_REQUEST;
            case 481:
                return GET_MERCHANT_STATUS_REQUEST;
            case 482:
                return TRIGGER_AT_A_PLACE_NOTIFICATION_REQUEST;
            case 483:
                return GET_EV_TRIP_WAYPOINTS_REQUEST;
            case 484:
                return WRITE_MULTIPLE_CHOICE_ANSWER_REQUEST;
            case 485:
                return DELETE_PERSONAL_ACTIVITIES_BY_PLACE_REQUEST;
            case 486:
                return LIST_PERSONAL_ACTIVITIES_BY_PLACE_REQUEST;
            case 487:
                return LIST_RIDDLER_QUESTIONS_DEIDENTIFIED_REQUEST;
            case 488:
                return OFFLINE_SNAPTILE_PACKAGE_METADATA_REQUEST;
            case 489:
                return GET_CONTRIBUTOR_ZONE_CONTRIBUTION_SUGGESTION_BY_GAIA_REQUEST;
            case 490:
                return GET_CONTRIBUTOR_ZONE_CONTRIBUTION_SUGGESTION_BY_ODLH_REQUEST;
            case 491:
                return GET_CREATOR_ZONE_CONTRIBUTION_SUGGESTION_BY_GAIA_REQUEST;
            case 492:
                return GET_CREATOR_ZONE_CONTRIBUTION_SUGGESTION_BY_ODLH_REQUEST;
            case 493:
                return GET_THANKS_PAGE_CONTRIBUTION_SUGGESTION_BY_GAIA_REQUEST;
            case 494:
                return GET_THANKS_PAGE_CONTRIBUTION_SUGGESTION_BY_ODLH_REQUEST;
            case 495:
                return DELETE_UGC_REACITON_REQUEST;
            case 496:
                return LIST_ATTRIBUTED_REACTIONS_BY_PARENT_CONTENT_ID_REQUEST;
            case 497:
                return UPDATE_UGC_REACTION_REQUEST;
            case 498:
                return GET_GEN_AI_SEARCH_OUTPUT_REQUEST;
            case 499:
                return GET_RIDDLER_FAMILIAR_PLACES_REQUEST;
            case 500:
                return ENABLE_SMS_NOTIFICATION_REQUEST;
            case TypedValues.PositionType.TYPE_TRANSITION_EASING /* 501 */:
                return LIST_VEHICLE_MAKES_REQUEST;
            case TypedValues.PositionType.TYPE_DRAWPATH /* 502 */:
                return LIST_VEHICLE_MODELS_REQUEST;
            case TypedValues.PositionType.TYPE_PERCENT_WIDTH /* 503 */:
                return LIST_VEHICLE_BUILDS_REQUEST;
            case TypedValues.PositionType.TYPE_PERCENT_HEIGHT /* 504 */:
                return LIST_TASK_COMPLETIONS_REQUEST;
            case TypedValues.PositionType.TYPE_SIZE_PERCENT /* 505 */:
                return GET_MERCHANT_PROFILE_STRENGTH_REQUEST;
            case TypedValues.PositionType.TYPE_PERCENT_X /* 506 */:
                return SUGGEST_EDIT_BY_CHAT_REQUEST;
            case TypedValues.PositionType.TYPE_PERCENT_Y /* 507 */:
                return SUBMIT_GEN_AI_USER_FEEDBACK_REQUEST;
            case TypedValues.PositionType.TYPE_CURVE_FIT /* 508 */:
                return GET_GEN_AI_LANDING_PAGE_REQUEST;
            case 509:
                return REPORT_ROUTING_FEEDBACK_REQUEST;
            case TypedValues.PositionType.TYPE_POSITION_TYPE /* 510 */:
                return GET_TIMELINE_STATIC_MAP_REQUEST;
            case 511:
                return UPDATE_MERCHANT_LISTING_REQUEST;
            case 512:
                return GET_EOM_TREATMENT_REQUEST;
            case 513:
                return REPORT_MERCHANT_RECOMMENDATION_INTERACTION_REQUEST;
            case 514:
                return BLOCK_UGC_USER_TO_USER_REQUEST;
            case 515:
                return EXPLORE_ALONG_ROUTE_REQUEST;
            case 516:
                return OFFLINE_GET_DATA_VIEW_REQUEST;
            case 517:
                return LIST_UGC_FLAGS_BY_USER_REQUEST;
            case 518:
                return GET_INFERRED_EV_METADATA_REQUEST;
            case 519:
                return LIST_MERCHANT_PRODUCT_ANSWERS_REQUEST;
            case 520:
                return REPORT_MERCHANT_PRODUCT_QUESTION_REQUEST;
            case 521:
                return UPDATE_MERCHANT_PRODUCT_ANSWER_REQUEST;
            case 522:
                return LIST_TICKET_ORDERS_REQUEST;
            case 523:
                return DECODE_URL;
            case 524:
                return EDIT_MERCHANT_MEDIA_ITEM_REQUEST;
            case OlympusMakernoteDirectory.TAG_ORIGINAL_MANUFACTURER_MODEL /* 525 */:
                return GET_UGC_POST_EDITOR_FEEDBACK_REQUEST;
            case SanyoMakernoteDirectory.TAG_SEQUENTIAL_SHOT /* 526 */:
                return GET_TRAFFIC_CONDITIONS_REQUEST;
            case 527:
                return GET_BUILDING_ENTRANCE_EDITING_DATA_REQUEST;
            case 528:
                return FULFILL_FEATURE_REQUEST;
            case 529:
                return EXTRACT_PLACE_LIST_FROM_TEXT_REQUEST;
            case 530:
                return REPORT_EV_CHARGING_STATION_ETA_REQUEST;
            case 531:
                return LIST_PROMPT_TEMPLATES_REQUEST;
            case 532:
                return GET_MULTI_MODAL_SEARCH_LANDING_PAGE_REQUEST;
            case 533:
                return GET_EXTRACT_PLACE_LIST_STATUS_REQUEST;
            case 534:
                return FETCH_LLM_RESULT_REQUEST;
            case SanyoMakernoteDirectory.TAG_RECORD_SHUTTER_RELEASE /* 535 */:
                return GET_MID_TRIP_UGC_IMPACT_REQUEST;
            case SanyoMakernoteDirectory.TAG_FLICKER_REDUCE /* 536 */:
                return DELETE_USER_FACTUAL_EDIT_REQUEST;
            case 537:
                return SUBMIT_FEEDBACK_ON_INFERRED_PLACES_REQUEST;
            case IptcDirectory.TAG_CONTENT_LOCATION_CODE /* 538 */:
                return LIST_INTERACTED_PLACES_REQUEST;
            case 539:
                return LIST_ROADLER_QUESTIONS_REQUEST;
            case 540:
                return GET_ALLOWED_EV_INTEGRATIONS_REQUEST;
            case SanyoMakernoteDirectory.TAG_LIGHT_SOURCE_SPECIAL /* 541 */:
                return GET_PLACES_AGGREGATION_FOR_USER_PROTOTYPE_REQUEST;
        }
    }

    @Override // com.google.android.libraries.navigation.internal.ael.bm
    public final int a() {
        return this.jm;
    }

    @Override // java.lang.Enum
    public final String toString() {
        return Integer.toString(this.jm);
    }
}
