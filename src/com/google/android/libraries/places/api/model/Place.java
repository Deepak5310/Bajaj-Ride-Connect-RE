    ...
    public static abstract class Builder {
        public Place build() {
            ...
            if (attributions != null) {
                Iterator<String> it2 = attributions.iterator();
                while (it2.hasNext()) {
                    zzkt.zzo(!TextUtils.isEmpty(it2.next()), "Attributions must not contain null or empty values.");
                }
            }
            Integer priceLevel = placeZza.getPriceLevel();
            if (priceLevel != null) {
                zzkt.zzr(zzmk.zzb(0, 4).zzd(priceLevel), "Price Level must not be out-of-range: %s to %s, but was: %s.", 0, 4, priceLevel);
            }
            Double rating = placeZza.getRating();
            if (rating != null) {
                Double dValueOf = Double.valueOf(1.0d);
                Double dValueOf2 = Double.valueOf(5.0d);
                zzkt.zzr(zzmk.zzb(dValueOf, dValueOf2).zzd(rating), "Rating must not be out-of-range: %s to %s, but was: %s.", dValueOf, dValueOf2, rating);
            }
            Integer userRatingsTotal = placeZza.getUserRatingsTotal();
            if (userRatingsTotal != null) {
                zzkt.zzq(zzmk.zza(0).zzd(userRatingsTotal), "User Ratings Total must not be < 0, but was: %s.", userRatingsTotal);
            }
            if (attributions != null) {
                setAttributions(zzlx.zzj(attributions));
            }
            List<PhotoMetadata> photoMetadatas = placeZza.getPhotoMetadatas();
            if (photoMetadatas != null) {
                setPhotoMetadatas(zzlx.zzj(photoMetadatas));
            }
            List<String> placeTypes = placeZza.getPlaceTypes();
            if (placeTypes != null) {
                setPlaceTypes(zzlx.zzj(placeTypes));
            }
            List<Type> types = placeZza.getTypes();
            if (types != null) {
                setTypes(zzlx.zzj(types));
            }
            List<OpeningHours> secondaryOpeningHours = placeZza.getSecondaryOpeningHours();
            if (secondaryOpeningHours != null) {
                setSecondaryOpeningHours(zzlx.zzj(secondaryOpeningHours));
            }
            List<Review> reviews = placeZza.getReviews();
            if (reviews != null) {
                setReviews(zzlx.zzj(reviews));
            }
            return zza();
        }
