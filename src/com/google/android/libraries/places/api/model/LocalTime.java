...
public abstract class LocalTime implements Parcelable, Comparable<LocalTime> {
    public static LocalTime newInstance(int i, int i2) {
        try {
            ...
            zzkt.zzp(zzmk.zzb(0, 23).zzd(Integer.valueOf(hours)), "Hours must not be out-of-range: 0 to 23, but was: %s.", hours);
            int minutes = localTimeZzc.getMinutes();
            zzkt.zzp(zzmk.zzb(0, 59).zzd(Integer.valueOf(minutes)), "Minutes must not be out-of-range: 0 to 59, but was: %s.", minutes);
            return localTimeZzc;
    ...
    public int compareTo(LocalTime localTime) {
        int hours;
        int hours2;
        zzkt.zzc(localTime, "compare must not be null.");
        if (this == localTime) {
            return 0;
        }
