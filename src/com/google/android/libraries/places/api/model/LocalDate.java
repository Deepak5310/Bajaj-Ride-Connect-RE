...
public abstract class LocalDate implements Parcelable, Comparable<LocalDate> {
    public static LocalDate newInstance(int i, int i2, int i3) {
        ...
        zzmk zzmkVarZzb = zzmk.zzb(1, 12);
        Integer numValueOf = Integer.valueOf(month);
        zzkt.zzh(zzmkVarZzb.zzd(numValueOf), "Month must not be out of range of 1 to 12, but was: %s.", month);
        int day = localDateZzd.getDay();
        zzmk zzmkVarZzb2 = zzmk.zzb(1, 31);
        Integer numValueOf2 = Integer.valueOf(day);
        zzkt.zzh(zzmkVarZzb2.zzd(numValueOf2), "Day must not be out of range of 1 to 31, but was: %s.", day);
        if (Arrays.asList(4, 6, 9, 11).contains(numValueOf) && !zzmk.zzb(1, 30).zzd(numValueOf2)) {
            throw new IllegalArgumentException(zzlb.zzb("%s is not a valid day for month %s.", numValueOf2, numValueOf));
        }
        if (month == 2) {
            int year = localDateZzd.getYear();
            zzkt.zzl(zzmk.zzb(1, Integer.valueOf(year % 4 == 0 ? 29 : 28)).zzd(numValueOf2), "%s is not a valid day for month %s in year %s.", numValueOf2, 2, Integer.valueOf(year));
        }
        return localDateZzd;
    }
    ...
    public int compareTo(LocalDate localDate) {
        int day;
        int day2;
        zzkt.zzc(localDate, "dateToCompare must not be null.");
        if (this == localDate) {
            return 0;
        }
