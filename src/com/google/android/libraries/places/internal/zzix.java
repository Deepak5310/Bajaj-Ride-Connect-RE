...
public final class zzix {
    public static Status zza(Intent intent) {
        try {
            zzkt.zzc(intent, "Intent must not be null.");
            Status status = (Status) intent.getParcelableExtra("places/status");
            zzkt.zzc(status, "Intent expected to contain a Status, but doesn't.");
            return status;
    ...
    public static Place zzb(Intent intent) {
        try {
            ...
            zzkt.zzc(place, "Intent expected to contain a Place, but doesn't.");
            return place;
        } catch (Error | RuntimeException e) {
            zzif.zzb(e);
            throw e;
