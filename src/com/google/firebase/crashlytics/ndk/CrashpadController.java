...
public class CrashpadController {
    ...
    private CrashlyticsReport.ApplicationExitInfo getRelevantApplicationExitInfo(long j, List<ApplicationExitInfo> list) {
        ...
        while (it2.hasNext()) {
            ...
            if (applicationExitInfoM.getReason() == 5 && applicationExitInfoM.getTimestamp() >= j) {
                arrayList.add(applicationExitInfoM);
            }
        }
        if (arrayList.isEmpty()) {
            return null;
        }
        return convertApplicationExitInfoToModel(a$$ExternalSyntheticApiModelOutline0.m(arrayList.get(0)));
    }
