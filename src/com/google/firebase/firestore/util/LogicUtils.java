...
public class LogicUtils {
    ...
    private static Filter applyDistribution(CompositeFilter compositeFilter, CompositeFilter compositeFilter2) {
        ...
        ArrayList arrayList = new ArrayList();
        Iterator<Filter> it2 = compositeFilter3.getFilters().iterator();
        while (it2.hasNext()) {
            arrayList.add(applyDistribution(it2.next(), compositeFilter));
        }
        return new CompositeFilter(arrayList, CompositeFilter.Operator.OR);
    }
    ...
    protected static Filter computeInExpansion(Filter filter) {
        ...
        if (filter instanceof FieldFilter) {
            ...
            Iterator<Value> it2 = inFilter.getValue().getArrayValue().getValuesList().iterator();
            while (it2.hasNext()) {
                arrayList.add(FieldFilter.create(inFilter.getField(), FieldFilter.Operator.EQUAL, it2.next()));
            }
            return new CompositeFilter(arrayList, CompositeFilter.Operator.OR);
        }
        CompositeFilter compositeFilter = (CompositeFilter) filter;
        Iterator<Filter> it3 = compositeFilter.getFilters().iterator();
        while (it3.hasNext()) {
            arrayList.add(computeInExpansion(it3.next()));
        }
