...
public class SyncEngine implements RemoteStore.RemoteStoreCallback {
    ...
    public void handleOnlineStateChange(OnlineState onlineState) {
        ...
        Iterator<Map.Entry<Query, QueryView>> it2 = this.queryViewsByQuery.entrySet().iterator();
        while (it2.hasNext()) {
            ViewChange viewChangeApplyOnlineStateChange = it2.next().getValue().getView().applyOnlineStateChange(onlineState);
    ...
    private void resolvePendingWriteTasks(int i) {
        if (this.pendingWritesCallbacks.containsKey(Integer.valueOf(i))) {
            Iterator<TaskCompletionSource<Void>> it2 = this.pendingWritesCallbacks.get(Integer.valueOf(i)).iterator();
            while (it2.hasNext()) {
    ...
    private void failOutstandingPendingWritesAwaitingTasks() {
        Iterator<Map.Entry<Integer, List<TaskCompletionSource<Void>>>> it2 = this.pendingWritesCallbacks.entrySet().iterator();
        while (it2.hasNext()) {
