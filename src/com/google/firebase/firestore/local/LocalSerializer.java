...
public final class LocalSerializer {
    ...
    private com.google.firestore.v1.Document encodeDocument(Document document) {
        com.google.firestore.v1.Document.Builder builderNewBuilder = com.google.firestore.v1.Document.newBuilder();
    ...
    WriteBatch encodeMutationBatch(MutationBatch mutationBatch) {
        ...
        Iterator<Mutation> it2 = mutationBatch.getBaseMutations().iterator();
        while (it2.hasNext()) {
            builderNewBuilder.addBaseWrites(this.rpcSerializer.encodeMutation(it2.next()));
        }
        Iterator<Mutation> it3 = mutationBatch.getMutations().iterator();
        while (it3.hasNext()) {
            builderNewBuilder.addWrites(this.rpcSerializer.encodeMutation(it3.next()));
        }
        return builderNewBuilder.build();
    }
    ...
    MutationBatch decodeMutationBatch(WriteBatch writeBatch) {
        ...
        while (i2 < writeBatch.getWritesCount()) {
            ...
            if (i3 < writeBatch.getWritesCount() && writeBatch.getWrites(i3).hasTransform()) {
                ...
                Iterator<DocumentTransform.FieldTransform> it2 = writeBatch.getWrites(i3).getTransform().getFieldTransformsList().iterator();
                while (it2.hasNext()) {
                    builderNewBuilder.addUpdateTransforms(it2.next());
                }
    ...
    public BundledQuery encodeBundledQuery(com.google.firebase.firestore.bundle.BundledQuery bundledQuery) {
        ...
        com.google.firestore.v1.Target.QueryTarget queryTargetEncodeQueryTarget = this.rpcSerializer.encodeQueryTarget(bundledQuery.getTarget());
