...
class MemoryEagerReferenceDelegate implements ReferenceDelegate {
    ...
    public void removeTarget(TargetData targetData) {
        ...
        Iterator<DocumentKey> it2 = targetCache.getMatchingKeysForTargetId(targetData.getTargetId()).iterator();
        while (it2.hasNext()) {
            this.orphanedDocuments.add(it2.next());
        }
    ...
    private boolean mutationQueuesContainKey(DocumentKey documentKey) {
        Iterator<MemoryMutationQueue> it2 = this.persistence.getMutationQueues().iterator();
        while (it2.hasNext()) {
            if (it2.next().containsKey(documentKey)) {
                return true;
            }
        }
        return false;
