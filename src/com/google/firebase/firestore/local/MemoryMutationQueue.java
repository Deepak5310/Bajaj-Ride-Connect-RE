...
final class MemoryMutationQueue implements MutationQueue {
    ...
    private List<MutationBatch> lookupMutationBatches(ImmutableSortedSet<Integer> immutableSortedSet) {
        ...
        while (it2.hasNext()) {
            MutationBatch mutationBatchLookupMutationBatch = lookupMutationBatch(it2.next().intValue());
    ...
    long getByteSize(LocalSerializer localSerializer) {
        Iterator<MutationBatch> it2 = this.queue.iterator();
        long serializedSize = 0;
        while (it2.hasNext()) {
            serializedSize += (long) localSerializer.encodeMutationBatch(it2.next()).getSerializedSize();
        }
