...
public final class UserDataReader {
    ...
    private <T> Value parseList(List<T> list, UserData.ParseContext parseContext) {
        ...
        int i = 0;
        while (it2.hasNext()) {
            Value data = parseData(it2.next(), parseContext.childContext(i));
            if (data == null) {
    ...
    private Value parseVectorValue(VectorValue vectorValue, UserData.ParseContext parseContext) {
        MapValue.Builder builderNewBuilder = MapValue.newBuilder();
        builderNewBuilder.putFields(Values.TYPE_KEY, Values.VECTOR_VALUE_TYPE);
        builderNewBuilder.putFields("value", parseData(vectorValue.toList(), parseContext));
        return Value.newBuilder().setMapValue(builderNewBuilder).build();
    }
