...
final class FieldSet<T extends FieldDescriptorLite<T>> {
    ...
    public static <T extends FieldDescriptorLite<T>> SmallSortedMap<T, Object> cloneAllFieldsMap(SmallSortedMap<T, Object> fields, boolean copyList) {
        ...
        Iterator it2 = fields.getOverflowEntries().iterator();
        while (it2.hasNext()) {
            cloneFieldEntry(smallSortedMapNewFieldMap, (Map.Entry) it2.next(), copyList);
        }
    ...
    public void mergeFrom(final FieldSet<T> other) {
        ...
        Iterator it2 = other.fields.getOverflowEntries().iterator();
        while (it2.hasNext()) {
    ...
    public void writeMessageSetTo(final CodedOutputStream output) throws IOException {
        ...
        Iterator it2 = this.fields.getOverflowEntries().iterator();
        while (it2.hasNext()) {
            writeMessageSetTo((Map.Entry) it2.next(), output);
        }
    }
    ...
    public static void writeField(final FieldDescriptorLite<?> descriptor, final Object value, final CodedOutputStream output) throws IOException {
        ...
        if (descriptor.isRepeated()) {
            ...
            while (it4.hasNext()) {
                writeElement(output, liteType, number, it4.next());
            }
            return;
        }
        if (value instanceof LazyField) {
    ...
    public int getMessageSetSerializedSize() {
        ...
        Iterator it2 = this.fields.getOverflowEntries().iterator();
        while (it2.hasNext()) {
            messageSetSerializedSize += getMessageSetSerializedSize((Map.Entry) it2.next());
        }
    ...
    static final class Builder<T extends FieldDescriptorLite<T>> {
        ...
        private static <T extends FieldDescriptorLite<T>> void replaceBuilders(SmallSortedMap<T, Object> fieldMap, boolean partial) {
            ...
            Iterator it2 = fieldMap.getOverflowEntries().iterator();
            while (it2.hasNext()) {
                replaceBuilders((Map.Entry) it2.next(), partial);
            }
        }
        ...
        public boolean isInitialized() {
            ...
            Iterator it2 = this.fields.getOverflowEntries().iterator();
            while (it2.hasNext()) {
                if (!FieldSet.isInitialized((Map.Entry) it2.next())) {
                    return false;
                }
            }
            return true;
        ...
        public void mergeFrom(final FieldSet<T> other) {
            ...
            Iterator it2 = ((FieldSet) other).fields.getOverflowEntries().iterator();
            while (it2.hasNext()) {
                mergeFromField((Map.Entry) it2.next());
            }
        }
        ...
        private void mergeFromField(final Map.Entry<T, Object> entry) {
            ...
            if (key.isRepeated()) {
                ...
                Iterator it2 = ((List) value).iterator();
                while (it2.hasNext()) {
                    arrayList.add(FieldSet.cloneIfMutable(it2.next()));
                }
                return;
            }
            if (key.getLiteJavaType() != WireFormat.JavaType.MESSAGE) {
