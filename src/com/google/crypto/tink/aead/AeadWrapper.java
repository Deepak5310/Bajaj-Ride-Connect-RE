    ...
    private static class WrappedAead implements Aead {
        ...
        public byte[] decrypt(final byte[] ciphertext, final byte[] associatedData) throws GeneralSecurityException {
            if (ciphertext.length > 5) {
                ...
                Iterator<PrimitiveSet.Entry<Aead>> it2 = this.pSet.getPrimitive(bArrCopyOfRange).iterator();
                while (it2.hasNext()) {
                    try {
                        return it2.next().getPrimitive().decrypt(bArrCopyOfRange2, associatedData);
                    } catch (GeneralSecurityException e) {
                        AeadWrapper.logger.info("ciphertext prefix matches a key, but cannot decrypt: " + e.toString());
                    }
                }
            }
            Iterator<PrimitiveSet.Entry<Aead>> it3 = this.pSet.getRawPrimitives().iterator();
            while (it3.hasNext()) {
                try {
                    return it3.next().getPrimitive().decrypt(ciphertext, associatedData);
