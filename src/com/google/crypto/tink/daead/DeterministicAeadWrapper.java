    ...
    private static class WrappedDeterministicAead implements DeterministicAead {
        ...
        public byte[] decryptDeterministically(final byte[] ciphertext, final byte[] associatedData) throws GeneralSecurityException {
            if (ciphertext.length > 5) {
                ...
                Iterator<PrimitiveSet.Entry<DeterministicAead>> it2 = this.primitives.getPrimitive(bArrCopyOfRange).iterator();
                while (it2.hasNext()) {
                    try {
                        return it2.next().getPrimitive().decryptDeterministically(bArrCopyOfRange2, associatedData);
                    } catch (GeneralSecurityException e) {
                        DeterministicAeadWrapper.logger.info("ciphertext prefix matches a key, but cannot decrypt: " + e.toString());
                    }
                }
            }
            Iterator<PrimitiveSet.Entry<DeterministicAead>> it3 = this.primitives.getRawPrimitives().iterator();
            while (it3.hasNext()) {
                try {
                    return it3.next().getPrimitive().decryptDeterministically(ciphertext, associatedData);
