...
import com.ammarahmed.mmkv.Constants;
...
public final class EngineFactory<T_WRAPPER extends EngineWrapper<T_ENGINE>, T_ENGINE> {
    ...
    static {
        if (SubtleUtil.isAndroid()) {
            defaultPolicy = toProviderList(ProviderInstaller.PROVIDER_NAME, Constants.KEYSTORE_PROVIDER_3);
        } else {
    ...
    public T_ENGINE getInstance(String str) throws GeneralSecurityException {
        ...
        while (it2.hasNext()) {
            try {
                return (T_ENGINE) this.instanceBuilder.getInstance(str, it2.next());
            } catch (Exception e) {
                if (exc == null) {
                    exc = e;
                }
