    ...
    private static class WrappedHybridDecrypt implements HybridDecrypt {
        ...
        public byte[] decrypt(final byte[] ciphertext, final byte[] contextInfo) throws GeneralSecurityException {
            if (ciphertext.length > 5) {
                ...
                Iterator<PrimitiveSet.Entry<HybridDecrypt>> it2 = this.primitives.getPrimitive(bArrCopyOfRange).iterator();
                while (it2.hasNext()) {
                    try {
                        return it2.next().getPrimitive().decrypt(bArrCopyOfRange2, contextInfo);
                    } catch (GeneralSecurityException e) {
                        HybridDecryptWrapper.logger.info("ciphertext prefix matches a key, but cannot decrypt: " + e.toString());
                    }
                }
            }
            Iterator<PrimitiveSet.Entry<HybridDecrypt>> it3 = this.primitives.getRawPrimitives().iterator();
            while (it3.hasNext()) {
                try {
                    return it3.next().getPrimitive().decrypt(ciphertext, contextInfo);
