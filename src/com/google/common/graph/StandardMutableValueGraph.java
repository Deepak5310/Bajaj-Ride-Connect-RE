...
final class StandardMutableValueGraph<N, V> extends StandardValueGraph<N, V> implements MutableValueGraph<N, V> {
    ...
    public boolean removeNode(N n2) {
        ...
        UnmodifiableIterator it2 = ImmutableList.copyOf((Collection) graphConnections.successors()).iterator();
        while (it2.hasNext()) {
            E next = it2.next();
        ...
        if (isDirected()) {
            UnmodifiableIterator it3 = ImmutableList.copyOf((Collection) graphConnections.predecessors()).iterator();
            while (it3.hasNext()) {
                E next2 = it3.next();
