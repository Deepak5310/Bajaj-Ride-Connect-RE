...
public final class LongMath {
    ...
    static final int[] biggestBinomials = {Integer.MAX_VALUE, Integer.MAX_VALUE, Integer.MAX_VALUE, 3810779, 121977, 16175, 4337, 1733, 887, 534, 361, 265, 206, 169, 143, NavigationConstants.NAVIGATION_LOW_ALERT_DURATION, PanasonicMakernoteDirectory.TAG_LANDMARK, 101, 94, 88, 83, 79, 76, 74, 72, 70, 69, 68, 67, 67, 66, 66, 66, 66};
    static final int[] biggestSimpleBinomials = {Integer.MAX_VALUE, Integer.MAX_VALUE, Integer.MAX_VALUE, 2642246, 86251, 11724, 3218, OlympusCameraSettingsMakernoteDirectory.TagPictureModeSaturation, 684, 419, OlympusImageProcessingMakernoteDirectory.TagWbGLevel, Mp4VideoDirectory.TAG_FRAME_RATE, 169, NikonType2MakernoteDirectory.TAG_LENS_STOPS, PanasonicMakernoteDirectory.TAG_BURST_SPEED, 105, 95, 87, 81, 76, 73, 70, 68, 66, 64, 63, 62, 62, 61, 61, 61};
    ...
    public static long binomial(int n2, int k) {
        MathPreconditions.checkNonNegative("n", n2);
        MathPreconditions.checkNonNegative("k", k);
        Preconditions.checkArgument(k <= n2, "k (%s) > n (%s)", k, n2);
        if (k > (n2 >> 1)) {
            k = n2 - k;
        }
        ...
        long[] jArr = factorials;
        if (n2 < jArr.length) {
            return jArr[n2] / (jArr[k] * jArr[n2 - k]);
        }
        int[] iArr = biggestBinomials;
        if (k >= iArr.length || n2 > iArr[k]) {
            return Long.MAX_VALUE;
        }
        int[] iArr2 = biggestSimpleBinomials;
        if (k < iArr2.length && n2 <= iArr2[k]) {
            int i = n2 - 1;
            long j = n2;
    ...
    public static boolean isPrime(long n2) {
        ...
        if (n2 < 66) {
            return ((722865708377213483 >> (((int) n2) + (-2))) & 1) != 0;
        }
        ...
        for (long[] jArr : millerRabinBaseSets) {
            if (n2 <= jArr[0]) {
                for (int i = 1; i < jArr.length; i++) {
                    if (!MillerRabinTester.test(jArr[i], n2)) {
                        return false;
                    }
                }
                return true;
