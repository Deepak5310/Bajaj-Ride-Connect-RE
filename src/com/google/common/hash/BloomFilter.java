...
public final class BloomFilter<T> implements Predicate<T>, Serializable {
    ...
    static int optimalNumOfHashFunctions(long n2, long m) {
        return Math.max(1, (int) Math.round((m / n2) * Math.log(2.0d)));
    }

    static long optimalNumOfBits(long n2, double p) {
        if (p == 0.0d) {
            p = Double.MIN_VALUE;
        }
        return (long) (((-n2) * Math.log(p)) / (Math.log(2.0d) * Math.log(2.0d)));
    }
