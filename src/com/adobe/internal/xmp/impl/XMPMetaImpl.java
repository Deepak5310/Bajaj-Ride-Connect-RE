...
public class XMPMetaImpl implements XMPMeta, XMPConst {
    ...
    public void setLocalizedText(String str, String str2, String str3, String str4, String str5, PropertyOptions propertyOptions) throws XMPException {
        ...
        String strNormalizeLangValue = str3 != null ? Utils.normalizeLangValue(str3) : null;
        String strNormalizeLangValue2 = Utils.normalizeLangValue(str4);
        XMPNode xMPNodeFindNode = XMPNodeUtils.findNode(this.tree, XMPPathParser.expandXPath(str, str2), true, new PropertyOptions(7680));
        if (xMPNodeFindNode == null) {
            throw new XMPException("Failed to find or create array node", 102);
        }
        if (!xMPNodeFindNode.getOptions().isArrayAltText()) {
            if (!xMPNodeFindNode.hasChildren() && xMPNodeFindNode.getOptions().isArrayAlternate()) {
                xMPNodeFindNode.getOptions().setArrayAltText(true);
            } else {
        ...
        while (true) {
            ...
            xMPNode = (XMPNode) itIterateChildren.next();
        ...
        if (xMPNode != null && xMPNodeFindNode.getChildrenLength() > 1) {
            xMPNodeFindNode.removeChild(xMPNode);
            xMPNodeFindNode.addChild(1, xMPNode);
        }
        Object[] objArrChooseLocalizedText = XMPNodeUtils.chooseLocalizedText(xMPNodeFindNode, strNormalizeLangValue, strNormalizeLangValue2);
        int iIntValue = ((Integer) objArrChooseLocalizedText[0]).intValue();
        XMPNode xMPNode2 = (XMPNode) objArrChooseLocalizedText[1];
        boolean zEquals = XMPConst.X_DEFAULT.equals(strNormalizeLangValue2);
        if (iIntValue == 0) {
            XMPNodeUtils.appendLangItem(xMPNodeFindNode, XMPConst.X_DEFAULT, str5);
            if (!zEquals) {
                XMPNodeUtils.appendLangItem(xMPNodeFindNode, strNormalizeLangValue2, str5);
            }
        } else {
            if (iIntValue != 1) {
                if (iIntValue == 2) {
                ...
                Iterator itIterateChildren2 = xMPNodeFindNode.iterateChildren();
            ...
            if (z && xMPNodeFindNode.getChildrenLength() == 1) {
                XMPNodeUtils.appendLangItem(xMPNodeFindNode, XMPConst.X_DEFAULT, str5);
                return;
            }
        }
