    ...
    private static void rdf_EmptyPropertyElement(XMPMetaImpl xMPMetaImpl, XMPNode xMPNode, Node node, boolean z) throws XMPException {
        ...
        for (int i2 = 0; i2 < node.getAttributes().getLength(); i2++) {
            ...
            if (!"xmlns".equals(nodeItem2.getPrefix()) && (nodeItem2.getPrefix() != null || !"xmlns".equals(nodeItem2.getNodeName()))) {
                int rDFTermKind = getRDFTermKind(nodeItem2);
                if (rDFTermKind != 0) {
                    if (rDFTermKind == 2) {
                        continue;
                ...
                } else if ("value".equals(nodeItem2.getLocalName()) && XMPConst.NS_RDF.equals(nodeItem2.getNamespaceURI())) {
                    if (z4) {
                        throw new XMPException("Empty property element can't have both rdf:value and rdf:resource", 203);
                    }
                    node2 = nodeItem2;
                    z3 = true;
                } else if (!XMPConst.XML_LANG.equals(nodeItem2.getNodeName())) {
                    z5 = true;
                }
            }
        }
        ...
        if (!z3 && !z4) {
            ...
            for (i = 0; i < node.getAttributes().getLength(); i++) {
                ...
                if (nodeItem == node2 && !"xmlns".equals(nodeItem.getPrefix()) && (nodeItem.getPrefix() != null || !"xmlns".equals(nodeItem.getNodeName()))) {
                    int rDFTermKind2 = getRDFTermKind(nodeItem);
                    if (rDFTermKind2 != 0) {
                        if (rDFTermKind2 != 2) {
                            if (rDFTermKind2 == 5) {
                                addQualifierNode(xMPNodeAddChildNode, "rdf:resource", nodeItem.getNodeValue());
                            } else if (rDFTermKind2 != 6) {
                                throw new XMPException("Unrecognized attribute of empty property element", 202);
                            }
                        }
                    } else if (!z2) {
                        addQualifierNode(xMPNodeAddChildNode, nodeItem.getNodeName(), nodeItem.getNodeValue());
                    } else if (XMPConst.XML_LANG.equals(nodeItem.getNodeName())) {
                        addQualifierNode(xMPNodeAddChildNode, XMPConst.XML_LANG, nodeItem.getNodeValue());
                    } else {
                        addChildNode(xMPMetaImpl, xMPNodeAddChildNode, nodeItem, nodeItem.getNodeValue(), false);
                    }
                }
            }
        }
        ...
        xMPNodeAddChildNode.setValue(nodeValue);
        if (!z3) {
