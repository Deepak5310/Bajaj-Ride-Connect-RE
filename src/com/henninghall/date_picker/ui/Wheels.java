...
public class Wheels {
    ...
    void applyOnAll(WheelFunction wheelFunction) {
        Iterator<Wheel> it2 = getAll().iterator();
        while (it2.hasNext()) {
            wheelFunction.apply(it2.next());
        }
    }
    ...
    String getDisplayValue() {
        StringBuilder sb = new StringBuilder();
        Iterator<Wheel> it2 = getOrderedVisibleWheels().iterator();
        while (it2.hasNext()) {
            sb.append(it2.next().getDisplayValue());
        }
        return sb.toString();
    }

    private void addInOrder() {
        Iterator<WheelType> it2 = this.state.derived.getOrderedVisibleWheels().iterator();
        while (it2.hasNext()) {
            this.pickerWrapper.addPicker(getWheel(it2.next()).picker.getView());
        }
    }
    ...
    private ArrayList<Wheel> getOrderedVisibleWheels() {
        ...
        Iterator<WheelType> it2 = this.state.derived.getOrderedVisibleWheels().iterator();
        while (it2.hasNext()) {
            arrayList.add(getWheel(it2.next()));
        }
        return arrayList;
    }
